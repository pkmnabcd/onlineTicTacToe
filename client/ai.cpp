#include "ai.hpp"
#include "util.hpp"

#include <array>
#include <memory>
#include <print>
#include <random>
#include <cstdint>
#include <vector>

using ModelType = std::array<std::vector<std::array<char, 9>>, 9>;
const std::unique_ptr<ModelType> MODEL = std::make_unique<ModelType>(ModelType{{
    {
        {'1', 'X', 'O', 'X', 'O', 'X', 'X', 'O', 'O'},
        {'1', 'X', 'O', 'X', 'O', 'X', 'X', 'O', '9'},
        {'1', 'X', 'O', 'X', 'O', 'X', 'X', '8', 'O'},
        {'1', 'X', 'O', 'X', 'O', 'X', '7', 'X', 'O'},
        {'1', 'X', 'O', 'X', 'O', 'X', '7', '8', 'O'},
        {'1', 'X', 'O', 'X', 'O', 'O', 'X', 'O', 'X'},
        {'1', 'X', 'O', 'X', 'O', 'O', 'X', '8', 'X'},
        {'1', 'X', 'O', 'X', 'O', 'O', 'X', '8', '9'},
        {'1', 'X', 'O', 'X', 'O', '6', 'X', 'O', 'X'},
        {'1', 'X', 'O', 'X', 'O', '6', 'X', 'O', '9'},
        {'1', 'X', 'O', 'X', 'O', '6', 'X', 'X', 'O'},
        {'1', 'X', 'O', 'X', 'O', '6', 'X', '8', 'O'},
        {'1', 'X', 'O', 'X', 'O', '6', 'X', '8', '9'},
        {'1', 'X', 'O', 'X', 'O', 'O', '7', 'X', '9'},
        {'1', 'X', 'O', 'X', 'O', '6', '7', 'X', 'O'},
        {'1', 'X', 'O', 'X', 'X', 'O', 'O', 'O', 'X'},
        {'1', 'X', 'O', 'X', 'X', 'O', 'O', '8', 'X'},
        {'1', 'X', 'O', 'X', 'X', 'O', '7', 'O', 'X'},
        {'1', 'X', 'O', 'X', '5', 'O', 'X', 'O', 'X'},
        {'1', 'X', 'O', 'X', '5', 'O', 'X', 'O', '9'},
        {'1', 'X', 'O', 'X', '5', 'O', '7', 'O', 'X'},
        {'1', 'X', 'O', 'X', '5', 'O', '7', '8', 'X'},
        {'1', 'X', 'O', 'X', 'X', '6', 'O', 'O', 'X'},
        {'1', 'X', 'O', 'X', 'X', '6', 'O', '8', '9'},
        {'1', 'X', 'O', 'X', '5', 'X', 'X', 'O', 'O'},
        {'1', 'X', 'O', 'X', '5', '6', 'X', 'O', 'O'},
        {'1', 'X', 'O', 'X', '5', '6', 'X', 'O', '9'},
        {'1', 'X', 'O', 'X', '5', '6', '7', 'O', 'X'},
        {'1', 'X', 'O', 'X', '5', '6', '7', '8', 'O'},
        {'1', 'X', 'O', 'O', 'X', 'X', 'O', 'O', 'X'},
        {'1', 'X', 'O', 'O', 'X', 'X', 'O', '8', 'X'},
        {'1', 'X', 'O', 'O', 'X', 'X', 'X', 'O', 'O'},
        {'1', 'X', 'O', 'O', 'X', 'X', 'X', 'O', '9'},
        {'1', 'X', 'O', 'O', 'X', 'X', '7', 'O', 'X'},
        {'1', 'X', 'O', 'O', 'X', 'X', '7', 'O', '9'},
        {'1', 'X', 'O', 'O', 'X', 'O', 'X', 'O', 'X'},
        {'1', 'X', 'O', 'O', 'X', 'O', 'X', '8', 'X'},
        {'1', 'X', 'O', 'O', 'X', '6', 'X', 'O', 'X'},
        {'1', 'X', 'O', 'O', 'X', '6', 'X', 'O', '9'},
        {'1', 'X', 'O', 'O', 'X', 'O', '7', '8', 'X'},
        {'1', 'X', 'O', 'O', 'X', '6', 'O', '8', 'X'},
        {'1', 'X', 'O', 'O', 'X', '6', '7', 'O', 'X'},
        {'1', 'X', 'O', 'O', 'X', '6', '7', '8', 'X'},
        {'1', 'X', 'O', '4', 'X', 'O', 'X', 'O', 'X'},
        {'1', 'X', 'O', '4', 'X', 'O', 'O', '8', 'X'},
        {'1', 'X', 'O', '4', 'X', 'O', '7', 'O', 'X'},
        {'1', 'X', 'O', '4', 'X', 'O', '7', '8', 'X'},
        {'1', 'X', 'O', '4', 'X', 'X', 'O', 'O', 'X'},
        {'1', 'X', 'O', '4', 'X', 'X', 'O', '8', '9'},
        {'1', 'X', 'O', '4', 'X', '6', 'O', 'O', 'X'},
        {'1', 'X', 'O', '4', 'X', '6', 'O', '8', 'X'},
        {'1', 'X', 'O', '4', 'X', '6', 'X', 'O', '9'},
        {'1', 'X', 'O', '4', 'X', '6', '7', 'O', 'X'},
        {'1', 'X', 'O', '4', 'X', 'X', '7', '8', 'O'},
        {'1', 'X', 'O', 'O', 'O', 'X', 'X', 'X', 'O'},
        {'1', 'X', 'O', 'O', 'O', 'X', 'X', 'O', 'X'},
        {'1', 'X', 'O', 'O', 'O', 'X', 'X', '8', '9'},
        {'1', 'X', 'O', 'O', '5', 'X', 'X', 'O', 'X'},
        {'1', 'X', 'O', 'O', '5', 'X', 'X', 'O', '9'},
        {'1', 'X', 'O', 'O', '5', 'X', 'X', '8', 'O'},
        {'1', 'X', 'O', 'O', '5', 'X', 'O', 'X', 'X'},
        {'1', 'X', 'O', 'O', '5', 'X', 'O', '8', 'X'},
        {'1', 'X', 'O', 'O', '5', 'X', '7', 'O', 'X'},
        {'1', 'X', 'O', 'O', '5', 'X', '7', '8', '9'},
        {'1', 'X', 'O', '4', 'O', 'X', 'X', 'O', 'X'},
        {'1', 'X', 'O', '4', 'O', 'X', 'X', 'O', '9'},
        {'1', 'X', 'O', '4', 'O', 'X', 'X', 'X', 'O'},
        {'1', 'X', 'O', '4', 'O', 'X', 'X', '8', 'O'},
        {'1', 'X', 'O', '4', 'O', 'X', 'X', '8', '9'},
        {'1', 'X', 'O', '4', 'O', 'X', '7', 'X', 'O'},
        {'1', 'X', 'O', '4', '5', 'X', 'X', 'O', '9'},
        {'1', 'X', 'O', '4', '5', 'X', '7', 'O', 'X'},
        {'1', 'X', 'O', 'O', '5', '6', 'X', 'X', '9'},
        {'1', 'X', 'O', 'O', '5', '6', 'X', 'O', 'X'},
        {'1', 'X', 'O', '4', 'O', '6', 'X', 'X', 'O'},
        {'1', 'X', 'O', '4', 'O', '6', 'X', 'O', 'X'},
        {'1', 'X', 'O', '4', 'O', '6', 'X', '8', '9'},
        {'1', 'X', 'O', '4', '5', 'O', 'X', 'O', 'X'},
        {'1', 'X', 'O', '4', '5', 'O', 'X', '8', 'X'},
        {'1', 'X', 'O', '4', '5', '6', 'X', 'O', 'X'},
        {'1', 'X', 'O', '4', '5', '6', 'X', 'O', '9'},
        {'1', 'X', 'O', 'O', '5', '6', '7', 'X', 'X'},
        {'1', 'X', 'O', '4', 'O', '6', '7', 'X', '9'},
        {'1', 'X', 'O', '4', '5', 'O', '7', 'X', 'X'},
        {'1', 'X', 'O', '4', '5', '6', '7', 'O', 'X'},
        {'1', 'X', 'X', 'O', 'O', 'X', 'O', 'X', 'O'},
        {'1', 'X', 'X', 'O', 'O', 'X', 'O', 'X', '9'},
        {'1', 'X', 'X', 'O', 'O', 'X', 'O', '8', '9'},
        {'1', 'X', 'X', 'O', 'O', 'X', 'X', 'O', 'O'},
        {'1', 'X', 'X', 'O', 'O', 'X', 'X', 'O', '9'},
        {'1', 'X', 'X', 'O', 'O', 'X', '7', 'O', '9'},
        {'1', 'X', 'X', 'O', 'O', 'X', 'X', '8', 'O'},
        {'1', 'X', 'X', 'O', 'O', 'X', '7', 'X', 'O'},
        {'1', 'X', 'X', 'O', 'O', 'X', '7', '8', 'O'},
        {'1', 'X', 'X', 'O', 'O', 'X', '7', '8', '9'},
        {'1', 'X', 'X', 'O', 'O', '6', 'X', 'O', '9'},
        {'1', 'X', 'X', 'O', 'O', '6', 'X', 'X', 'O'},
        {'1', 'X', 'X', 'O', 'O', '6', 'X', '8', 'O'},
        {'1', 'X', 'X', 'O', 'O', '6', 'O', 'X', 'X'},
        {'1', 'X', 'X', 'O', 'O', '6', 'O', 'X', '9'},
        {'1', 'X', 'X', 'O', 'O', '6', '7', 'X', 'O'},
        {'1', 'X', 'X', 'O', 'O', '6', 'O', '8', 'X'},
        {'1', 'X', 'X', 'O', 'O', '6', '7', 'O', 'X'},
        {'1', 'X', 'X', 'O', 'O', '6', '7', '8', '9'},
        {'1', 'X', 'X', 'O', 'X', 'O', 'O', 'O', 'X'},
        {'1', 'X', 'X', 'O', 'X', 'O', 'O', '8', 'X'},
        {'1', 'X', 'X', 'O', 'X', 'O', 'O', '8', '9'},
        {'1', 'X', 'X', 'O', 'X', 'O', '7', 'O', 'X'},
        {'1', 'X', 'X', 'O', 'X', 'O', '7', 'O', '9'},
        {'1', 'X', 'X', 'O', 'X', 'O', '7', '8', 'O'},
        {'1', 'X', 'X', 'O', 'X', 'O', '7', '8', '9'},
        {'1', 'X', 'X', 'O', '5', 'O', 'X', 'O', '9'},
        {'1', 'X', 'X', 'O', '5', 'O', 'X', '8', 'O'},
        {'1', 'X', 'X', 'O', '5', 'O', 'O', 'X', 'X'},
        {'1', 'X', 'X', 'O', '5', 'O', 'O', 'X', '9'},
        {'1', 'X', 'X', 'O', '5', 'O', '7', 'X', 'O'},
        {'1', 'X', 'X', 'O', '5', 'O', 'O', '8', 'X'},
        {'1', 'X', 'X', 'O', '5', 'O', '7', 'O', 'X'},
        {'1', 'X', 'X', 'O', '5', 'O', '7', '8', '9'},
        {'1', 'X', 'X', 'O', 'X', 'X', 'O', 'O', '9'},
        {'1', 'X', 'X', 'O', 'X', '6', 'O', 'O', 'X'},
        {'1', 'X', 'X', 'O', 'X', '6', 'O', 'O', '9'},
        {'1', 'X', 'X', 'O', 'X', 'X', 'O', '8', 'O'},
        {'1', 'X', 'X', 'O', 'X', '6', 'O', '8', 'O'},
        {'1', 'X', 'X', 'O', 'X', '6', 'O', '8', '9'},
        {'1', 'X', 'X', 'O', '5', 'X', 'O', 'O', '9'},
        {'1', 'X', 'X', 'O', '5', 'X', 'O', 'X', 'O'},
        {'1', 'X', 'X', 'O', '5', 'X', 'O', '8', 'O'},
        {'1', 'X', 'X', 'O', '5', 'X', 'O', '8', '9'},
        {'1', 'X', 'X', 'O', '5', '6', 'O', 'X', 'O'},
        {'1', 'X', 'X', 'O', '5', '6', 'O', 'X', '9'},
        {'1', 'X', 'X', 'O', '5', '6', 'O', 'O', 'X'},
        {'1', 'X', 'X', 'O', '5', '6', 'O', '8', 'X'},
        {'1', 'X', 'X', 'O', '5', '6', 'O', '8', '9'},
        {'1', 'X', 'X', 'O', 'X', '6', '7', 'O', 'O'},
        {'1', 'X', 'X', 'O', 'X', '6', '7', 'O', '9'},
        {'1', 'X', 'X', 'O', '5', 'X', 'X', 'O', 'O'},
        {'1', 'X', 'X', 'O', '5', 'X', '7', 'O', 'O'},
        {'1', 'X', 'X', 'O', '5', 'X', '7', 'O', '9'},
        {'1', 'X', 'X', 'O', '5', '6', 'X', 'O', 'O'},
        {'1', 'X', 'X', 'O', '5', '6', 'X', 'O', '9'},
        {'1', 'X', 'X', 'O', '5', '6', '7', 'O', 'X'},
        {'1', 'X', 'X', 'O', '5', '6', '7', 'O', '9'},
        {'1', 'X', 'X', 'O', 'X', '6', '7', '8', 'O'},
        {'1', 'X', 'X', 'O', '5', 'X', '7', '8', 'O'},
        {'1', 'X', 'X', 'O', '5', '6', 'X', '8', 'O'},
        {'1', 'X', 'X', 'O', '5', '6', '7', 'X', 'O'},
        {'1', 'X', 'X', 'O', '5', '6', '7', '8', 'O'},
        {'1', 'X', 'X', 'O', '5', '6', '7', '8', '9'},
        {'1', 'X', '3', 'O', 'X', 'O', 'X', 'O', 'X'},
        {'1', 'X', '3', 'O', 'X', 'O', 'X', '8', '9'},
        {'1', 'X', '3', 'O', 'X', 'O', 'O', '8', 'X'},
        {'1', 'X', '3', 'O', 'X', 'O', '7', 'O', 'X'},
        {'1', 'X', '3', 'O', 'X', 'O', '7', '8', 'X'},
        {'1', 'X', '3', 'O', 'X', 'X', 'O', 'O', 'X'},
        {'1', 'X', '3', 'O', 'X', 'X', 'O', 'O', '9'},
        {'1', 'X', '3', 'O', 'X', 'X', 'O', '8', '9'},
        {'1', 'X', '3', 'O', 'X', '6', 'O', 'O', 'X'},
        {'1', 'X', '3', 'O', 'X', '6', 'O', '8', 'X'},
        {'1', 'X', '3', 'O', 'X', '6', '7', 'O', 'X'},
        {'1', 'X', '3', 'O', 'X', '6', '7', 'O', '9'},
        {'1', 'X', '3', 'O', 'X', '6', 'X', '8', 'O'},
        {'1', 'X', '3', 'O', 'X', '6', '7', '8', '9'},
        {'1', 'X', '3', 'O', 'O', 'X', 'X', 'X', 'O'},
        {'1', 'X', '3', 'O', 'O', 'X', 'X', '8', 'O'},
        {'1', 'X', '3', 'O', 'O', 'X', 'O', 'X', 'X'},
        {'1', 'X', '3', 'O', 'O', 'X', 'O', 'X', '9'},
        {'1', 'X', '3', 'O', 'O', 'X', '7', 'X', 'O'},
        {'1', 'X', '3', 'O', 'O', 'X', '7', 'X', '9'},
        {'1', 'X', '3', 'O', '5', 'X', 'O', 'X', '9'},
        {'1', 'X', '3', 'O', '5', 'X', 'O', '8', 'X'},
        {'1', 'X', '3', 'O', '5', 'X', 'O', '8', '9'},
        {'1', 'X', '3', 'O', '5', 'X', '7', '8', 'O'},
        {'1', 'X', '3', 'O', 'O', '6', 'X', 'X', 'O'},
        {'1', 'X', '3', 'O', 'O', '6', 'O', 'X', 'X'},
        {'1', 'X', '3', 'O', 'O', '6', '7', 'X', '9'},
        {'1', 'X', '3', 'O', '5', '6', 'O', 'X', 'X'},
        {'1', 'X', '3', 'O', '5', '6', 'O', '8', 'X'},
        {'1', 'X', '3', 'O', '5', '6', '7', 'O', 'X'},
        {'1', 'X', '3', 'O', '5', '6', '7', '8', '9'},
        {'1', 'X', 'X', 'X', 'O', 'O', 'O', 'X', 'O'},
        {'1', 'X', 'X', 'X', 'O', 'O', 'O', 'X', '9'},
        {'1', 'X', 'X', 'X', 'O', 'O', 'O', 'O', 'X'},
        {'1', 'X', 'X', 'X', 'O', 'O', 'O', '8', 'X'},
        {'1', 'X', 'X', 'X', 'O', 'O', 'O', '8', '9'},
        {'1', 'X', 'X', 'X', 'O', 'O', 'X', 'O', 'O'},
        {'1', 'X', 'X', 'X', 'O', 'O', 'X', 'O', '9'},
        {'1', 'X', 'X', 'X', 'O', 'O', '7', 'O', 'X'},
        {'1', 'X', 'X', 'X', 'O', 'O', '7', 'O', '9'},
        {'1', 'X', 'X', 'X', 'O', 'O', 'X', '8', 'O'},
        {'1', 'X', 'X', 'X', 'O', 'O', '7', 'X', 'O'},
        {'1', 'X', 'X', 'X', 'O', 'O', '7', '8', 'O'},
        {'1', 'X', 'X', 'X', 'O', 'O', '7', '8', '9'},
        {'1', 'X', 'X', '4', 'O', 'O', 'X', 'O', '9'},
        {'1', 'X', 'X', '4', 'O', 'O', 'X', 'X', 'O'},
        {'1', 'X', 'X', '4', 'O', 'O', 'X', '8', 'O'},
        {'1', 'X', 'X', '4', 'O', 'O', 'O', 'X', '9'},
        {'1', 'X', 'X', '4', 'O', 'O', '7', 'X', 'O'},
        {'1', 'X', 'X', '4', 'O', 'O', 'O', '8', 'X'},
        {'1', 'X', 'X', '4', 'O', 'O', '7', 'O', 'X'},
        {'1', 'X', 'X', '4', 'O', 'O', '7', '8', '9'},
        {'1', 'X', 'X', 'X', 'O', '6', 'O', 'O', 'X'},
        {'1', 'X', 'X', 'X', 'O', '6', 'O', 'O', '9'},
        {'1', 'X', 'X', 'X', 'O', 'X', 'O', '8', 'O'},
        {'1', 'X', 'X', 'X', 'O', '6', 'O', 'X', 'O'},
        {'1', 'X', 'X', 'X', 'O', '6', 'O', '8', 'O'},
        {'1', 'X', 'X', 'X', 'O', '6', 'O', '8', '9'},
        {'1', 'X', 'X', '4', 'O', 'X', 'O', 'O', '9'},
        {'1', 'X', 'X', '4', 'O', 'X', 'O', 'X', 'O'},
        {'1', 'X', 'X', '4', 'O', 'X', 'O', '8', 'O'},
        {'1', 'X', 'X', '4', 'O', 'X', 'O', '8', '9'},
        {'1', 'X', 'X', '4', 'O', '6', 'O', 'X', 'O'},
        {'1', 'X', 'X', '4', 'O', '6', 'O', 'X', '9'},
        {'1', 'X', 'X', '4', 'O', '6', 'O', 'O', 'X'},
        {'1', 'X', 'X', '4', 'O', '6', 'O', '8', 'X'},
        {'1', 'X', 'X', '4', 'O', '6', 'O', '8', '9'},
        {'1', 'X', 'X', 'X', 'O', 'X', '7', 'O', 'O'},
        {'1', 'X', 'X', 'X', 'O', '6', 'X', 'O', 'O'},
        {'1', 'X', 'X', 'X', 'O', '6', '7', 'O', 'O'},
        {'1', 'X', 'X', 'X', 'O', '6', '7', 'O', '9'},
        {'1', 'X', 'X', '4', 'O', 'X', 'X', 'O', 'O'},
        {'1', 'X', 'X', '4', 'O', 'X', '7', 'O', 'O'},
        {'1', 'X', 'X', '4', 'O', 'X', '7', 'O', '9'},
        {'1', 'X', 'X', '4', 'O', '6', 'X', 'O', 'O'},
        {'1', 'X', 'X', '4', 'O', '6', 'X', 'O', '9'},
        {'1', 'X', 'X', '4', 'O', '6', '7', 'O', 'X'},
        {'1', 'X', 'X', '4', 'O', '6', '7', 'O', '9'},
        {'1', 'X', 'X', 'X', 'O', '6', '7', '8', 'O'},
        {'1', 'X', 'X', '4', 'O', 'X', '7', '8', 'O'},
        {'1', 'X', 'X', '4', 'O', '6', 'X', '8', 'O'},
        {'1', 'X', 'X', '4', 'O', '6', '7', 'X', 'O'},
        {'1', 'X', 'X', '4', 'O', '6', '7', '8', 'O'},
        {'1', 'X', 'X', '4', 'O', '6', '7', '8', '9'},
        {'1', 'X', '3', 'X', 'O', 'O', 'X', 'O', 'X'},
        {'1', 'X', '3', 'X', 'O', 'O', 'X', 'O', '9'},
        {'1', 'X', '3', 'X', 'O', 'O', 'X', 'X', 'O'},
        {'1', 'X', '3', 'X', 'O', 'O', 'X', '8', 'O'},
        {'1', 'X', '3', 'X', 'O', 'O', 'X', '8', '9'},
        {'1', 'X', '3', 'X', 'O', 'O', '7', 'X', 'O'},
        {'1', 'X', '3', 'X', 'O', 'O', '7', 'O', 'X'},
        {'1', 'X', '3', 'X', 'O', 'O', '7', '8', 'X'},
        {'1', 'X', '3', 'X', 'O', 'O', '7', '8', '9'},
        {'1', 'X', '3', 'X', 'O', 'X', 'O', 'O', '9'},
        {'1', 'X', '3', 'X', 'O', 'X', 'O', 'X', 'O'},
        {'1', 'X', '3', 'X', 'O', 'X', 'O', '8', 'O'},
        {'1', 'X', '3', 'X', 'O', '6', 'O', 'X', 'O'},
        {'1', 'X', '3', 'X', 'O', 'X', 'X', 'O', 'O'},
        {'1', 'X', '3', 'X', 'O', 'X', '7', 'O', 'O'},
        {'1', 'X', '3', 'X', 'O', '6', 'X', 'O', 'O'},
        {'1', 'X', '3', 'X', 'O', '6', 'X', 'O', '9'},
        {'1', 'X', '3', 'X', 'O', '6', '7', 'O', 'X'},
        {'1', 'X', '3', 'X', 'O', '6', '7', 'O', '9'},
        {'1', 'X', '3', 'X', 'O', 'X', '7', '8', 'O'},
        {'1', 'X', '3', 'X', 'O', '6', 'X', '8', 'O'},
        {'1', 'X', '3', 'X', 'O', '6', '7', 'X', 'O'},
        {'1', 'X', '3', 'X', 'O', '6', '7', '8', 'O'},
        {'1', 'X', '3', 'X', 'O', '6', '7', '8', '9'},
        {'1', 'X', '3', '4', 'O', 'X', 'O', 'X', 'O'},
        {'1', 'X', '3', '4', 'O', 'X', 'X', 'O', 'O'},
        {'1', 'X', '3', '4', 'O', 'X', 'X', 'O', '9'},
        {'1', 'X', '3', '4', 'O', 'X', 'X', '8', 'O'},
        {'1', 'X', '3', '4', 'O', 'X', '7', 'X', 'O'},
        {'1', 'X', '3', '4', 'O', 'X', '7', '8', 'O'},
        {'1', 'X', '3', '4', 'O', 'X', '7', '8', '9'},
        {'1', 'X', '3', '4', 'O', 'O', 'X', 'X', 'O'},
        {'1', 'X', '3', '4', 'O', '6', 'X', 'O', 'X'},
        {'1', 'X', '3', '4', 'O', '6', 'X', 'O', '9'},
        {'1', 'X', '3', '4', 'O', '6', 'X', 'X', 'O'},
        {'1', 'X', '3', '4', 'O', '6', 'X', '8', 'O'},
        {'1', 'X', '3', '4', 'O', '6', 'X', '8', '9'},
        {'1', 'X', '3', '4', 'O', 'O', 'O', 'X', 'X'},
        {'1', 'X', '3', '4', 'O', 'O', '7', 'X', '9'},
        {'1', 'X', '3', '4', 'O', '6', 'O', 'X', '9'},
        {'1', 'X', '3', '4', 'O', '6', '7', 'X', 'O'},
        {'1', 'X', '3', '4', 'O', '6', '7', 'X', '9'},
        {'1', 'X', '3', '4', 'O', '6', '7', '8', 'X'},
        {'1', 'X', '3', '4', 'O', '6', '7', '8', '9'},
        {'1', 'X', 'X', 'X', '5', 'O', 'O', 'O', 'X'},
        {'1', 'X', 'X', 'X', '5', 'O', 'O', 'O', '9'},
        {'1', 'X', 'X', 'X', '5', 'O', 'O', 'X', 'O'},
        {'1', 'X', 'X', 'X', '5', 'O', 'O', '8', 'O'},
        {'1', 'X', 'X', 'X', '5', 'O', 'O', '8', '9'},
        {'1', 'X', 'X', '4', 'X', 'O', 'O', 'O', 'X'},
        {'1', 'X', 'X', '4', 'X', 'O', 'O', 'O', '9'},
        {'1', 'X', 'X', '4', 'X', 'O', 'O', '8', 'O'},
        {'1', 'X', 'X', '4', 'X', 'O', 'O', '8', '9'},
        {'1', 'X', 'X', '4', '5', 'O', 'O', 'X', 'O'},
        {'1', 'X', 'X', '4', '5', 'O', 'O', 'X', '9'},
        {'1', 'X', 'X', '4', '5', 'O', 'O', 'O', 'X'},
        {'1', 'X', 'X', '4', '5', 'O', 'O', '8', 'X'},
        {'1', 'X', 'X', '4', '5', 'O', 'O', '8', '9'},
        {'1', 'X', 'X', 'X', '5', 'O', 'X', 'O', 'O'},
        {'1', 'X', 'X', 'X', '5', 'O', '7', 'O', 'O'},
        {'1', 'X', 'X', 'X', '5', 'O', '7', 'O', '9'},
        {'1', 'X', 'X', '4', 'X', 'O', '7', 'O', 'O'},
        {'1', 'X', 'X', '4', 'X', 'O', '7', 'O', '9'},
        {'1', 'X', 'X', '4', '5', 'O', 'X', 'O', 'O'},
        {'1', 'X', 'X', '4', '5', 'O', 'X', 'O', '9'},
        {'1', 'X', 'X', '4', '5', 'O', '7', 'O', 'X'},
        {'1', 'X', 'X', '4', '5', 'O', '7', 'O', '9'},
        {'1', 'X', 'X', 'X', '5', 'O', '7', '8', 'O'},
        {'1', 'X', 'X', '4', 'X', 'O', '7', '8', 'O'},
        {'1', 'X', 'X', '4', '5', 'O', 'X', '8', 'O'},
        {'1', 'X', 'X', '4', '5', 'O', '7', 'X', 'O'},
        {'1', 'X', 'X', '4', '5', 'O', '7', '8', 'O'},
        {'1', 'X', 'X', '4', '5', 'O', '7', '8', '9'},
        {'1', 'X', '3', 'X', 'X', 'O', 'O', 'O', 'X'},
        {'1', 'X', '3', 'X', '5', 'O', 'O', 'O', 'X'},
        {'1', 'X', '3', 'X', '5', 'O', 'O', '8', 'X'},
        {'1', 'X', '3', 'X', 'X', 'O', '7', 'O', 'O'},
        {'1', 'X', '3', 'X', '5', 'O', 'X', 'O', 'O'},
        {'1', 'X', '3', 'X', '5', 'O', 'X', 'O', '9'},
        {'1', 'X', '3', 'X', '5', 'O', '7', 'O', 'X'},
        {'1', 'X', '3', 'X', '5', 'O', '7', 'O', '9'},
        {'1', 'X', '3', 'X', '5', 'O', '7', '8', '9'},
        {'1', 'X', '3', '4', 'X', 'O', 'O', 'O', 'X'},
        {'1', 'X', '3', '4', 'X', 'O', 'O', '8', 'X'},
        {'1', 'X', '3', '4', 'X', 'O', '7', 'O', 'X'},
        {'1', 'X', '3', '4', 'X', 'O', '7', 'O', '9'},
        {'1', 'X', '3', '4', 'X', 'O', '7', '8', '9'},
        {'1', 'X', '3', '4', '5', 'O', 'X', 'O', 'X'},
        {'1', 'X', '3', '4', '5', 'O', 'X', 'O', '9'},
        {'1', 'X', '3', '4', '5', 'O', 'O', '8', 'X'},
        {'1', 'X', '3', '4', '5', 'O', '7', 'O', 'X'},
        {'1', 'X', 'X', '4', '5', '6', 'O', 'O', 'X'},
        {'1', 'X', 'X', '4', '5', '6', 'O', 'O', '9'},
        {'1', 'X', 'X', '4', '5', '6', 'O', 'X', 'O'},
        {'1', 'X', 'X', '4', '5', '6', 'O', '8', 'O'},
        {'1', 'X', 'X', '4', '5', '6', 'O', '8', '9'},
        {'1', 'X', '3', 'X', '5', '6', 'O', 'O', 'X'},
        {'1', 'X', '3', 'X', '5', '6', 'O', '8', 'O'},
        {'1', 'X', '3', '4', 'X', '6', 'O', 'O', 'X'},
        {'1', 'X', '3', '4', '5', 'X', 'O', 'O', 'X'},
        {'1', 'X', '3', '4', '5', 'X', 'O', '8', 'O'},
        {'1', 'X', '3', '4', '5', 'X', 'O', '8', '9'},
        {'1', 'X', '3', '4', '5', '6', 'O', 'O', 'X'},
        {'1', 'X', '3', '4', '5', '6', 'O', '8', 'X'},
        {'1', 'X', '3', '4', '5', '6', 'O', '8', '9'},
        {'1', 'X', 'X', '4', '5', '6', 'X', 'O', 'O'},
        {'1', 'X', 'X', '4', '5', '6', '7', 'O', 'O'},
        {'1', 'X', 'X', '4', '5', '6', '7', 'O', '9'},
        {'1', 'X', '3', 'X', '5', '6', 'X', 'O', 'O'},
        {'1', 'X', '3', 'X', '5', '6', '7', 'O', '9'},
        {'1', 'X', '3', '4', 'X', '6', '7', 'O', '9'},
        {'1', 'X', '3', '4', '5', 'X', 'X', 'O', 'O'},
        {'1', 'X', '3', '4', '5', 'X', '7', 'O', '9'},
        {'1', 'X', '3', '4', '5', '6', 'X', 'O', 'O'},
        {'1', 'X', '3', '4', '5', '6', 'X', 'O', '9'},
        {'1', 'X', '3', '4', '5', '6', '7', 'O', 'X'},
        {'1', 'X', '3', '4', '5', '6', '7', 'O', '9'},
        {'1', 'X', 'X', '4', '5', '6', '7', '8', 'O'},
        {'1', 'X', '3', '4', '5', '6', '7', '8', '9'},
        {'1', 'O', 'X', 'X', 'O', 'X', 'O', 'X', 'O'},
        {'1', 'O', 'X', 'X', 'O', 'X', 'X', '8', 'O'},
        {'1', 'O', 'X', 'X', 'O', 'X', '7', 'X', 'O'},
        {'1', 'O', 'X', 'X', 'O', 'X', '7', '8', 'O'},
        {'1', 'O', 'X', 'X', 'O', 'O', 'X', 'X', 'O'},
        {'1', 'O', 'X', 'X', 'O', 'O', 'X', 'X', '9'},
        {'1', 'O', 'X', 'X', 'O', 'O', 'X', '8', '9'},
        {'1', 'O', 'X', 'X', 'O', '6', 'X', 'X', 'O'},
        {'1', 'O', 'X', 'X', 'O', '6', 'X', '8', 'O'},
        {'1', 'O', 'X', 'X', 'O', 'O', 'O', 'X', 'X'},
        {'1', 'O', 'X', 'X', 'O', '6', 'O', 'X', '9'},
        {'1', 'O', 'X', 'X', 'O', '6', '7', 'X', 'O'},
        {'1', 'O', 'X', 'X', 'X', 'O', 'O', 'X', 'O'},
        {'1', 'O', 'X', 'X', 'X', 'O', 'O', 'X', '9'},
        {'1', 'O', 'X', 'X', 'X', 'O', 'O', 'O', 'X'},
        {'1', 'O', 'X', 'X', 'X', 'O', 'O', '8', 'X'},
        {'1', 'O', 'X', 'X', 'X', 'O', 'O', '8', '9'},
        {'1', 'O', 'X', 'X', 'X', 'O', '7', 'O', 'X'},
        {'1', 'O', 'X', 'X', '5', 'O', 'X', 'O', '9'},
        {'1', 'O', 'X', 'X', '5', 'O', 'X', 'X', 'O'},
        {'1', 'O', 'X', 'X', '5', 'O', 'X', '8', 'O'},
        {'1', 'O', 'X', 'X', '5', 'O', 'X', '8', '9'},
        {'1', 'O', 'X', 'X', '5', 'O', 'O', 'X', 'X'},
        {'1', 'O', 'X', 'X', '5', 'O', 'O', 'X', '9'},
        {'1', 'O', 'X', 'X', '5', 'O', 'O', '8', 'X'},
        {'1', 'O', 'X', 'X', 'X', '6', 'O', 'O', 'X'},
        {'1', 'O', 'X', 'X', '5', 'X', 'O', '8', '9'},
        {'1', 'O', 'X', 'X', '5', '6', 'O', 'X', 'O'},
        {'1', 'O', 'X', 'X', 'X', '6', '7', 'O', '9'},
        {'1', 'O', 'X', 'X', '5', '6', 'X', 'O', 'O'},
        {'1', 'O', 'X', 'X', 'X', '6', '7', '8', 'O'},
        {'1', 'O', 'X', 'X', '5', '6', 'X', '8', 'O'},
        {'1', 'O', 'X', 'O', 'X', 'X', 'O', 'X', 'O'},
        {'1', 'O', 'X', 'O', 'X', 'X', 'O', 'X', '9'},
        {'1', 'O', 'X', 'O', 'X', 'X', '7', '8', '9'},
        {'1', 'O', 'X', 'O', 'X', 'O', 'O', 'X', 'X'},
        {'1', 'O', 'X', 'O', 'X', 'O', '7', 'X', 'X'},
        {'1', 'O', 'X', 'O', 'X', '6', 'O', 'X', 'X'},
        {'1', 'O', 'X', 'O', 'X', '6', 'O', 'X', '9'},
        {'1', 'O', 'X', 'O', 'X', 'O', '7', '8', 'X'},
        {'1', 'O', 'X', 'O', 'X', '6', 'O', '8', 'X'},
        {'1', 'O', 'X', 'O', 'X', '6', '7', 'O', 'X'},
        {'1', 'O', 'X', 'O', 'X', '6', '7', '8', 'X'},
        {'1', 'O', 'X', '4', 'X', 'O', 'O', 'X', 'X'},
        {'1', 'O', 'X', '4', 'X', 'O', 'O', 'X', '9'},
        {'1', 'O', 'X', '4', 'X', 'O', 'O', '8', 'X'},
        {'1', 'O', 'X', '4', 'X', 'O', '7', 'O', 'X'},
        {'1', 'O', 'X', '4', 'X', 'O', '7', '8', 'X'},
        {'1', 'O', 'X', '4', 'X', 'X', 'O', '8', '9'},
        {'1', 'O', 'X', '4', 'X', '6', 'O', 'X', 'O'},
        {'1', 'O', 'X', '4', 'X', '6', 'O', 'X', '9'},
        {'1', 'O', 'X', '4', 'X', '6', 'O', 'O', 'X'},
        {'1', 'O', 'X', '4', 'X', '6', 'O', '8', 'X'},
        {'1', 'O', 'X', '4', 'X', 'X', '7', 'O', '9'},
        {'1', 'O', 'X', '4', 'X', '6', '7', 'O', 'X'},
        {'1', 'O', 'X', '4', 'X', 'X', '7', '8', 'O'},
        {'1', 'O', 'X', '4', 'X', '6', '7', '8', '9'},
        {'1', 'O', 'X', 'O', 'O', 'X', 'X', 'X', 'O'},
        {'1', 'O', 'X', 'O', '5', 'X', 'X', '8', '9'},
        {'1', 'O', 'X', 'O', '5', 'X', '7', 'X', 'O'},
        {'1', 'O', 'X', '4', 'O', 'X', 'X', 'X', 'O'},
        {'1', 'O', 'X', '4', 'O', 'X', 'X', '8', 'O'},
        {'1', 'O', 'X', '4', 'O', 'X', '7', 'X', 'O'},
        {'1', 'O', 'X', '4', '5', 'X', 'O', 'X', 'O'},
        {'1', 'O', 'X', '4', '5', 'X', '7', '8', '9'},
        {'1', 'O', 'X', 'O', '5', '6', 'X', 'X', '9'},
        {'1', 'O', 'X', 'O', '5', '6', 'X', '8', 'X'},
        {'1', 'O', 'X', '4', 'O', '6', 'X', 'X', 'O'},
        {'1', 'O', 'X', '4', '5', 'O', 'X', 'X', '9'},
        {'1', 'O', 'X', '4', '5', 'O', 'X', '8', 'X'},
        {'1', 'O', 'X', 'O', '5', '6', '7', 'X', 'X'},
        {'1', 'O', 'X', '4', 'O', '6', '7', 'X', 'X'},
        {'1', 'O', 'X', '4', '5', 'O', 'O', 'X', 'X'},
        {'1', 'O', 'X', '4', '5', '6', 'O', 'X', '9'},
        {'1', 'O', 'X', '4', '5', '6', '7', 'X', 'O'},
        {'1', 'O', 'X', '4', '5', '6', '7', '8', 'X'},
        {'1', '2', 'X', 'O', 'X', 'O', 'O', 'X', 'X'},
        {'1', '2', 'X', 'O', 'X', 'O', '7', 'X', '9'},
        {'1', '2', 'X', 'O', 'X', 'O', 'O', '8', 'X'},
        {'1', '2', 'X', 'O', 'X', 'O', '7', 'O', 'X'},
        {'1', '2', 'X', 'O', 'X', 'O', '7', '8', 'X'},
        {'1', '2', 'X', 'O', 'X', 'X', 'O', 'X', 'O'},
        {'1', '2', 'X', 'O', 'X', 'X', 'O', '8', 'O'},
        {'1', '2', 'X', 'O', 'X', 'X', 'O', '8', '9'},
        {'1', '2', 'X', 'O', 'X', '6', 'O', 'X', '9'},
        {'1', '2', 'X', 'O', 'X', '6', 'O', 'O', 'X'},
        {'1', '2', 'X', 'O', 'X', '6', 'O', '8', 'X'},
        {'1', '2', 'X', 'O', 'X', '6', 'O', '8', '9'},
        {'1', '2', 'X', 'O', 'X', 'X', '7', 'O', '9'},
        {'1', '2', 'X', 'O', 'X', '6', '7', 'O', 'X'},
        {'1', '2', 'X', 'O', 'X', '6', '7', 'X', 'O'},
        {'1', '2', 'X', 'O', 'X', '6', '7', '8', '9'},
        {'1', '2', 'X', 'O', 'O', 'X', 'X', 'X', 'O'},
        {'1', '2', 'X', 'O', 'O', 'X', 'X', '8', 'O'},
        {'1', '2', 'X', 'O', 'O', 'X', '7', 'X', 'O'},
        {'1', '2', 'X', 'O', '5', 'X', 'O', 'X', 'O'},
        {'1', '2', 'X', 'O', '5', 'X', 'O', 'X', '9'},
        {'1', '2', 'X', 'O', '5', 'X', 'X', 'O', '9'},
        {'1', '2', 'X', 'O', '5', 'X', '7', 'X', 'O'},
        {'1', '2', 'X', 'O', '5', 'X', '7', '8', 'O'},
        {'1', '2', 'X', 'O', 'O', '6', 'X', 'X', 'O'},
        {'1', '2', 'X', 'O', '5', '6', 'X', 'O', 'X'},
        {'1', '2', 'X', 'O', '5', 'O', 'O', 'X', 'X'},
        {'1', '2', 'X', 'O', '5', '6', 'O', 'X', 'X'},
        {'1', '2', 'X', 'O', '5', '6', 'O', 'X', '9'},
        {'1', '2', 'X', 'O', '5', '6', '7', '8', 'X'},
        {'1', '2', 'X', 'O', '5', '6', '7', '8', '9'},
        {'1', '2', 'X', 'X', 'O', 'O', 'X', 'O', '9'},
        {'1', '2', 'X', 'X', 'O', 'O', 'X', 'X', 'O'},
        {'1', '2', 'X', 'X', 'O', 'O', 'X', '8', 'O'},
        {'1', '2', 'X', 'X', 'O', 'O', 'X', '8', '9'},
        {'1', '2', 'X', 'X', 'O', 'O', 'O', 'X', 'X'},
        {'1', '2', 'X', 'X', 'O', 'O', 'O', 'X', '9'},
        {'1', '2', 'X', 'X', 'O', 'O', '7', 'X', 'O'},
        {'1', '2', 'X', 'X', 'O', 'O', '7', 'X', '9'},
        {'1', '2', 'X', 'X', 'O', 'O', 'O', '8', 'X'},
        {'1', '2', 'X', 'X', 'O', 'O', '7', '8', 'X'},
        {'1', '2', 'X', 'X', 'O', 'O', '7', '8', '9'},
        {'1', '2', 'X', 'X', 'O', 'X', 'O', 'X', 'O'},
        {'1', '2', 'X', 'X', 'O', 'X', 'O', '8', 'O'},
        {'1', '2', 'X', 'X', 'O', '6', 'O', 'X', 'O'},
        {'1', '2', 'X', 'X', 'O', '6', 'O', 'X', '9'},
        {'1', '2', 'X', 'X', 'O', '6', 'O', '8', '9'},
        {'1', '2', 'X', 'X', 'O', 'X', 'X', 'O', 'O'},
        {'1', '2', 'X', 'X', 'O', 'X', '7', 'O', 'O'},
        {'1', '2', 'X', 'X', 'O', '6', 'X', 'O', 'O'},
        {'1', '2', 'X', 'X', 'O', 'X', '7', '8', 'O'},
        {'1', '2', 'X', 'X', 'O', '6', 'X', '8', 'O'},
        {'1', '2', 'X', 'X', 'O', '6', '7', 'X', 'O'},
        {'1', '2', 'X', 'X', 'O', '6', '7', '8', 'O'},
        {'1', '2', 'X', 'X', 'O', '6', '7', '8', '9'},
        {'1', '2', 'X', '4', 'O', 'X', 'O', 'X', 'O'},
        {'1', '2', 'X', '4', 'O', 'X', 'X', 'O', 'O'},
        {'1', '2', 'X', '4', 'O', 'X', 'X', '8', 'O'},
        {'1', '2', 'X', '4', 'O', 'X', '7', 'X', 'O'},
        {'1', '2', 'X', '4', 'O', 'X', '7', '8', 'O'},
        {'1', '2', 'X', '4', 'O', 'O', 'X', 'X', 'O'},
        {'1', '2', 'X', '4', 'O', 'O', 'X', 'O', 'X'},
        {'1', '2', 'X', '4', 'O', '6', 'X', 'X', 'O'},
        {'1', '2', 'X', '4', 'O', '6', 'X', '8', 'O'},
        {'1', '2', 'X', '4', 'O', '6', 'O', 'X', '9'},
        {'1', '2', 'X', '4', 'O', '6', '7', 'X', 'O'},
        {'1', '2', 'X', '4', 'O', '6', '7', '8', '9'},
        {'1', '2', 'X', 'X', 'X', 'O', 'O', 'O', 'X'},
        {'1', '2', 'X', 'X', 'X', 'O', 'O', '8', '9'},
        {'1', '2', 'X', 'X', '5', 'O', 'O', 'X', '9'},
        {'1', '2', 'X', 'X', '5', 'O', 'O', 'O', 'X'},
        {'1', '2', 'X', 'X', '5', 'O', 'O', '8', 'X'},
        {'1', '2', 'X', 'X', '5', 'O', 'O', '8', '9'},
        {'1', '2', 'X', 'X', '5', 'O', 'X', 'O', 'O'},
        {'1', '2', 'X', 'X', '5', 'O', 'X', 'O', '9'},
        {'1', '2', 'X', 'X', '5', 'O', '7', 'O', 'X'},
        {'1', '2', 'X', 'X', '5', 'O', '7', 'O', '9'},
        {'1', '2', 'X', 'X', '5', 'O', 'X', '8', 'O'},
        {'1', '2', 'X', 'X', '5', 'O', '7', 'X', 'O'},
        {'1', '2', 'X', 'X', '5', 'O', '7', '8', 'O'},
        {'1', '2', 'X', 'X', '5', 'O', '7', '8', '9'},
        {'1', '2', 'X', '4', 'X', 'O', 'O', 'O', 'X'},
        {'1', '2', 'X', '4', 'X', 'O', 'O', '8', 'X'},
        {'1', '2', 'X', '4', 'X', 'O', 'O', '8', '9'},
        {'1', '2', 'X', '4', 'X', 'O', '7', 'O', 'X'},
        {'1', '2', 'X', '4', 'X', 'O', '7', 'X', 'O'},
        {'1', '2', 'X', '4', 'X', 'O', '7', '8', '9'},
        {'1', '2', 'X', '4', '5', 'O', '7', 'X', 'O'},
        {'1', '2', 'X', '4', '5', 'O', 'O', '8', 'X'},
        {'1', '2', 'X', '4', '5', 'O', '7', 'O', 'X'},
        {'1', '2', 'X', '4', '5', 'O', '7', '8', '9'},
        {'1', '2', 'X', 'X', '5', '6', 'O', 'O', 'X'},
        {'1', '2', 'X', '4', 'X', '6', 'O', 'O', 'X'},
        {'1', '2', 'X', '4', 'X', '6', 'O', '8', '9'},
        {'1', '2', 'X', '4', '5', 'X', 'O', 'X', 'O'},
        {'1', '2', 'X', '4', '5', 'X', 'O', '8', 'O'},
        {'1', '2', 'X', '4', '5', '6', 'O', 'X', '9'},
        {'1', '2', 'X', '4', '5', '6', 'O', '8', 'X'},
        {'1', '2', 'X', '4', '5', '6', 'O', '8', '9'},
        {'1', '2', 'X', 'X', '5', '6', 'X', 'O', 'O'},
        {'1', '2', 'X', '4', 'X', '6', '7', 'O', '9'},
        {'1', '2', 'X', '4', '5', 'X', '7', 'O', 'O'},
        {'1', '2', 'X', '4', '5', '6', '7', 'O', 'X'},
        {'1', '2', 'X', '4', '5', '6', '7', 'O', '9'},
        {'1', '2', 'X', '4', '5', '6', 'X', '8', 'O'},
        {'1', '2', 'X', '4', '5', '6', '7', '8', 'O'},
        {'1', 'O', 'O', 'X', 'X', 'O', 'X', 'X', '9'},
        {'1', 'O', 'O', 'X', 'X', 'O', 'X', 'O', 'X'},
        {'1', 'O', 'O', 'X', 'X', 'O', 'X', '8', 'X'},
        {'1', 'O', 'O', 'X', 'X', 'O', 'X', '8', '9'},
        {'1', 'O', 'O', 'X', 'X', '6', 'X', 'O', 'X'},
        {'1', 'O', 'O', 'X', 'X', '6', 'X', 'O', '9'},
        {'1', 'O', 'O', 'X', 'X', '6', 'X', 'X', 'O'},
        {'1', 'O', 'O', 'X', 'X', '6', 'X', '8', 'O'},
        {'1', 'O', 'O', 'X', 'X', '6', 'X', '8', '9'},
        {'1', 'O', 'O', 'X', 'X', 'O', 'O', 'X', 'X'},
        {'1', 'O', 'O', 'X', 'X', 'O', '7', 'X', 'X'},
        {'1', 'O', 'O', 'X', 'X', 'O', '7', 'X', '9'},
        {'1', 'O', 'O', 'X', 'X', '6', 'O', 'X', 'X'},
        {'1', 'O', 'O', 'X', 'X', '6', '7', 'X', '9'},
        {'1', 'O', 'O', 'X', 'X', 'O', '7', '8', 'X'},
        {'1', 'O', 'O', 'X', 'X', '6', 'O', '8', 'X'},
        {'1', 'O', 'O', 'X', 'X', '6', '7', 'O', 'X'},
        {'1', 'O', 'O', 'X', 'X', '6', '7', '8', 'X'},
        {'1', 'O', '3', 'X', 'X', 'O', 'X', 'O', 'X'},
        {'1', 'O', '3', 'X', 'X', 'O', 'X', 'O', '9'},
        {'1', 'O', '3', 'X', 'X', 'O', 'X', '8', 'O'},
        {'1', 'O', '3', 'X', 'X', 'O', 'X', '8', '9'},
        {'1', 'O', '3', 'X', 'X', 'O', 'O', 'X', 'X'},
        {'1', 'O', '3', 'X', 'X', 'O', 'O', 'X', '9'},
        {'1', 'O', '3', 'X', 'X', 'O', 'O', '8', 'X'},
        {'1', 'O', '3', 'X', 'X', 'O', '7', 'O', 'X'},
        {'1', 'O', '3', 'X', 'X', 'O', '7', '8', 'X'},
        {'1', 'O', '3', 'X', 'X', 'O', '7', '8', '9'},
        {'1', 'O', '3', 'X', 'X', '6', 'O', 'O', 'X'},
        {'1', 'O', '3', 'X', 'X', '6', 'O', '8', 'X'},
        {'1', 'O', '3', 'X', 'X', '6', 'X', 'O', 'O'},
        {'1', 'O', '3', 'X', 'X', '6', 'X', 'O', '9'},
        {'1', 'O', '3', 'X', 'X', '6', '7', 'O', 'X'},
        {'1', 'O', '3', 'X', 'X', '6', 'X', '8', 'O'},
        {'1', 'O', '3', 'X', 'X', '6', '7', '8', '9'},
        {'1', 'O', 'O', 'X', 'O', 'X', 'X', 'X', 'O'},
        {'1', 'O', 'O', 'X', 'O', 'X', 'X', 'X', '9'},
        {'1', 'O', 'O', 'X', 'O', 'X', 'X', '8', 'X'},
        {'1', 'O', 'O', 'X', 'O', 'X', 'X', '8', '9'},
        {'1', 'O', 'O', 'X', '5', 'X', 'X', 'O', 'X'},
        {'1', 'O', 'O', 'X', '5', 'X', 'X', 'O', '9'},
        {'1', 'O', 'O', 'X', '5', 'X', 'X', 'X', 'O'},
        {'1', 'O', 'O', 'X', '5', 'X', 'X', '8', 'O'},
        {'1', 'O', 'O', 'X', '5', 'X', 'X', '8', '9'},
        {'1', 'O', 'O', 'X', 'O', 'X', '7', 'X', 'X'},
        {'1', 'O', 'O', 'X', 'O', 'X', '7', 'X', '9'},
        {'1', 'O', 'O', 'X', '5', 'X', 'O', 'X', 'X'},
        {'1', 'O', 'O', 'X', '5', 'X', '7', 'X', '9'},
        {'1', 'O', 'O', 'X', 'O', 'X', '7', '8', 'X'},
        {'1', 'O', 'O', 'X', '5', 'X', '7', '8', 'X'},
        {'1', 'O', '3', 'X', 'O', 'X', 'X', 'X', 'O'},
        {'1', 'O', '3', 'X', 'O', 'X', 'X', '8', 'O'},
        {'1', 'O', '3', 'X', 'O', 'X', '7', 'X', 'O'},
        {'1', 'O', '3', 'X', 'O', 'X', '7', 'X', '9'},
        {'1', 'O', '3', 'X', 'O', 'X', '7', '8', '9'},
        {'1', 'O', '3', 'X', '5', 'X', 'O', '8', 'X'},
        {'1', 'O', '3', 'X', '5', 'X', 'X', 'O', 'O'},
        {'1', 'O', '3', 'X', '5', 'X', 'X', '8', 'O'},
        {'1', 'O', 'O', 'X', 'O', '6', 'X', 'X', '9'},
        {'1', 'O', 'O', 'X', '5', 'O', 'X', 'X', '9'},
        {'1', 'O', 'O', 'X', '5', '6', 'X', 'X', 'O'},
        {'1', 'O', 'O', 'X', '5', '6', 'X', 'X', '9'},
        {'1', 'O', 'O', 'X', 'O', '6', 'X', '8', 'X'},
        {'1', 'O', 'O', 'X', '5', 'O', 'X', '8', 'X'},
        {'1', 'O', 'O', 'X', '5', '6', 'X', 'O', 'X'},
        {'1', 'O', 'O', 'X', '5', '6', 'X', '8', 'X'},
        {'1', 'O', 'O', 'X', '5', '6', 'X', '8', '9'},
        {'1', 'O', '3', 'X', 'O', 'O', 'X', 'X', '9'},
        {'1', 'O', '3', 'X', 'O', '6', 'X', 'X', 'O'},
        {'1', 'O', '3', 'X', 'O', '6', 'X', 'X', '9'},
        {'1', 'O', '3', 'X', 'O', 'O', 'X', '8', 'X'},
        {'1', 'O', '3', 'X', 'O', '6', 'X', '8', '9'},
        {'1', 'O', '3', 'X', '5', 'O', 'X', 'X', 'O'},
        {'1', 'O', '3', 'X', '5', 'O', 'X', 'X', '9'},
        {'1', 'O', '3', 'X', '5', 'O', 'X', 'O', 'X'},
        {'1', 'O', '3', 'X', '5', 'O', 'X', '8', 'X'},
        {'1', 'O', '3', 'X', '5', 'O', 'X', '8', '9'},
        {'1', 'O', '3', 'X', '5', '6', 'X', 'O', '9'},
        {'1', 'O', '3', 'X', '5', '6', 'X', 'X', 'O'},
        {'1', 'O', '3', 'X', '5', '6', 'X', '8', 'O'},
        {'1', 'O', '3', 'X', '5', '6', 'X', '8', '9'},
        {'1', 'O', 'O', 'X', '5', '6', 'O', 'X', 'X'},
        {'1', 'O', 'O', 'X', '5', '6', '7', 'X', 'X'},
        {'1', 'O', 'O', 'X', '5', '6', '7', 'X', '9'},
        {'1', 'O', '3', 'X', '5', 'O', 'O', 'X', 'X'},
        {'1', 'O', '3', 'X', '5', '6', 'O', 'X', '9'},
        {'1', 'O', '3', 'X', '5', '6', '7', 'X', 'O'},
        {'1', 'O', 'O', 'X', '5', '6', '7', '8', 'X'},
        {'1', 'O', '3', 'X', '5', 'O', '7', '8', 'X'},
        {'1', 'O', '3', 'X', '5', '6', '7', '8', '9'},
        {'1', '2', 'O', 'X', 'X', 'O', 'X', 'O', 'X'},
        {'1', '2', 'O', 'X', 'X', 'O', 'X', 'O', '9'},
        {'1', '2', 'O', 'X', 'X', 'O', 'O', 'X', 'X'},
        {'1', '2', 'O', 'X', 'X', 'O', 'O', '8', 'X'},
        {'1', '2', 'O', 'X', 'X', 'O', '7', 'O', 'X'},
        {'1', '2', 'O', 'X', 'X', 'O', '7', '8', 'X'},
        {'1', '2', 'O', 'X', 'X', '6', 'O', 'X', '9'},
        {'1', '2', 'O', 'X', 'X', '6', 'O', 'O', 'X'},
        {'1', '2', 'O', 'X', 'X', '6', 'O', '8', 'X'},
        {'1', '2', 'O', 'X', 'X', '6', 'X', 'O', 'O'},
        {'1', '2', 'O', 'X', 'X', '6', 'X', 'O', '9'},
        {'1', '2', 'O', 'X', 'X', '6', '7', 'O', 'X'},
        {'1', '2', 'O', 'X', 'O', 'X', 'X', 'O', '9'},
        {'1', '2', 'O', 'X', 'O', 'X', 'X', 'X', 'O'},
        {'1', '2', 'O', 'X', 'O', 'X', 'X', '8', 'O'},
        {'1', '2', 'O', 'X', 'O', 'X', 'X', '8', '9'},
        {'1', '2', 'O', 'X', 'O', 'X', '7', 'X', 'O'},
        {'1', '2', 'O', 'X', 'O', 'X', '7', 'O', 'X'},
        {'1', '2', 'O', 'X', 'O', 'X', '7', '8', '9'},
        {'1', '2', 'O', 'X', '5', 'X', 'X', 'O', 'O'},
        {'1', '2', 'O', 'X', '5', 'X', 'X', 'O', '9'},
        {'1', '2', 'O', 'X', '5', 'X', 'X', '8', 'O'},
        {'1', '2', 'O', 'X', 'O', 'O', 'X', 'X', '9'},
        {'1', '2', 'O', 'X', 'O', '6', 'X', 'X', 'O'},
        {'1', '2', 'O', 'X', 'O', '6', 'X', 'X', '9'},
        {'1', '2', 'O', 'X', 'O', 'O', 'X', '8', 'X'},
        {'1', '2', 'O', 'X', 'O', '6', 'X', 'O', 'X'},
        {'1', '2', 'O', 'X', 'O', '6', 'X', '8', 'X'},
        {'1', '2', 'O', 'X', 'O', '6', 'X', '8', '9'},
        {'1', '2', 'O', 'X', '5', 'O', 'X', 'O', 'X'},
        {'1', '2', 'O', 'X', '5', 'O', 'X', '8', 'X'},
        {'1', '2', 'O', 'X', '5', 'O', 'X', '8', '9'},
        {'1', '2', 'O', 'X', '5', '6', 'X', 'O', 'X'},
        {'1', '2', 'O', 'X', '5', '6', 'X', 'O', '9'},
        {'1', '2', 'O', 'X', '5', '6', 'X', '8', 'O'},
        {'1', '2', 'O', 'X', '5', '6', 'X', '8', '9'},
        {'1', '2', 'O', 'X', '5', 'O', '7', 'X', 'X'},
        {'1', '2', 'O', 'X', '5', '6', '7', 'X', 'O'},
        {'1', '2', 'O', 'X', '5', '6', '7', 'X', '9'},
        {'1', '2', 'O', 'X', '5', 'O', '7', '8', 'X'},
        {'1', '2', 'O', 'X', '5', '6', '7', 'O', 'X'},
        {'1', '2', 'O', 'X', '5', '6', '7', '8', 'X'},
        {'1', '2', 'O', 'X', '5', '6', '7', '8', '9'},
        {'1', '2', '3', 'X', 'O', 'X', 'O', 'X', 'O'},
        {'1', '2', '3', 'X', 'O', 'X', 'O', '8', '9'},
        {'1', '2', '3', 'X', 'O', 'X', 'X', 'O', 'O'},
        {'1', '2', '3', 'X', 'O', 'X', '7', 'O', '9'},
        {'1', '2', '3', 'X', 'O', 'X', 'X', '8', 'O'},
        {'1', '2', '3', 'X', 'O', 'X', '7', 'X', 'O'},
        {'1', '2', '3', 'X', 'O', 'X', '7', '8', 'O'},
        {'1', '2', '3', 'X', 'O', 'X', '7', '8', '9'},
        {'1', '2', '3', 'X', 'O', 'O', 'X', 'X', 'O'},
        {'1', '2', '3', 'X', 'O', 'O', 'X', 'X', '9'},
        {'1', '2', '3', 'X', 'O', 'O', 'X', 'O', 'X'},
        {'1', '2', '3', 'X', 'O', 'O', 'X', '8', 'X'},
        {'1', '2', '3', 'X', 'O', 'O', 'X', '8', '9'},
        {'1', '2', '3', 'X', 'O', '6', 'X', 'O', '9'},
        {'1', '2', '3', 'X', 'O', '6', 'X', 'X', 'O'},
        {'1', '2', '3', 'X', 'O', '6', 'X', '8', 'O'},
        {'1', '2', '3', 'X', 'O', '6', 'X', '8', '9'},
        {'1', '2', '3', 'X', 'O', '6', '7', 'X', 'O'},
        {'1', '2', '3', 'X', 'O', '6', '7', 'X', '9'},
        {'1', '2', '3', 'X', 'O', '6', '7', '8', 'X'},
        {'1', '2', '3', 'X', 'O', '6', '7', '8', '9'},
        {'1', '2', '3', 'X', 'X', 'O', 'O', 'O', 'X'},
        {'1', '2', '3', 'X', 'X', 'O', 'O', '8', 'X'},
        {'1', '2', '3', 'X', 'X', 'O', 'X', 'O', 'O'},
        {'1', '2', '3', 'X', 'X', 'O', 'X', 'O', '9'},
        {'1', '2', '3', 'X', 'X', 'O', '7', 'O', 'X'},
        {'1', '2', '3', 'X', 'X', 'O', '7', 'O', '9'},
        {'1', '2', '3', 'X', 'X', 'O', '7', '8', '9'},
        {'1', '2', '3', 'X', '5', 'O', 'X', 'O', 'X'},
        {'1', '2', '3', 'X', '5', 'O', 'X', 'O', '9'},
        {'1', '2', '3', 'X', '5', 'O', 'X', '8', 'O'},
        {'1', '2', '3', 'X', '5', 'O', 'X', '8', '9'},
        {'1', '2', '3', 'X', '5', 'O', 'O', 'X', 'X'},
        {'1', '2', '3', 'X', '5', 'O', '7', 'X', '9'},
        {'1', '2', '3', 'X', '5', 'O', 'O', '8', 'X'},
        {'1', '2', '3', 'X', '5', 'O', '7', 'O', 'X'},
        {'1', '2', '3', 'X', '5', 'O', '7', '8', 'X'},
        {'1', '2', '3', 'X', '5', 'O', '7', '8', '9'},
        {'1', '2', '3', 'X', 'X', '6', 'O', 'O', 'X'},
        {'1', '2', '3', 'X', 'X', '6', 'O', 'X', 'O'},
        {'1', '2', '3', 'X', '5', 'X', 'O', 'O', 'X'},
        {'1', '2', '3', 'X', 'X', '6', 'X', 'O', 'O'},
        {'1', '2', '3', 'X', 'X', '6', '7', 'O', '9'},
        {'1', '2', '3', 'X', '5', 'X', 'X', 'O', 'O'},
        {'1', '2', '3', 'X', '5', '6', 'X', 'O', 'O'},
        {'1', '2', '3', 'X', '5', '6', 'X', 'O', '9'},
        {'1', '2', '3', 'X', '5', '6', 'X', '8', 'O'},
        {'1', '2', '3', 'X', '5', '6', '7', '8', '9'},
        {'1', 'O', 'O', 'O', 'X', 'X', 'X', 'X', 'O'},
        {'1', 'O', 'O', 'O', 'X', 'X', 'X', 'X', '9'},
        {'1', 'O', 'O', 'O', 'X', 'X', 'X', 'O', 'X'},
        {'1', 'O', 'O', 'O', 'X', 'X', 'X', '8', 'X'},
        {'1', 'O', 'O', 'O', 'X', 'X', 'X', '8', '9'},
        {'1', 'O', 'O', '4', 'X', 'X', 'X', 'O', 'X'},
        {'1', 'O', 'O', '4', 'X', 'X', 'X', 'X', 'O'},
        {'1', 'O', 'O', '4', 'X', 'X', 'X', '8', '9'},
        {'1', 'O', 'O', 'O', 'X', 'X', 'O', 'X', 'X'},
        {'1', 'O', 'O', 'O', 'X', 'X', '7', 'X', 'X'},
        {'1', 'O', 'O', 'O', 'X', 'X', '7', 'X', '9'},
        {'1', 'O', 'O', '4', 'X', 'X', 'O', 'X', 'X'},
        {'1', 'O', 'O', '4', 'X', 'X', '7', 'X', '9'},
        {'1', 'O', 'O', 'O', 'X', 'X', '7', '8', 'X'},
        {'1', 'O', 'O', '4', 'X', 'X', 'O', '8', 'X'},
        {'1', 'O', 'O', '4', 'X', 'X', '7', 'O', 'X'},
        {'1', 'O', 'O', '4', 'X', 'X', '7', '8', 'X'},
        {'1', 'O', '3', 'O', 'X', 'X', 'X', 'O', 'X'},
        {'1', 'O', '3', 'O', 'X', 'X', 'O', 'X', 'X'},
        {'1', 'O', '3', 'O', 'X', 'X', 'O', 'X', '9'},
        {'1', 'O', '3', 'O', 'X', 'X', '7', 'X', 'O'},
        {'1', 'O', '3', 'O', 'X', 'X', '7', 'X', '9'},
        {'1', 'O', '3', 'O', 'X', 'X', 'O', '8', 'X'},
        {'1', 'O', '3', 'O', 'X', 'X', '7', 'O', 'X'},
        {'1', 'O', '3', 'O', 'X', 'X', '7', '8', 'X'},
        {'1', 'O', '3', '4', 'X', 'X', 'O', 'O', 'X'},
        {'1', 'O', '3', '4', 'X', 'X', 'O', '8', 'X'},
        {'1', 'O', '3', '4', 'X', 'X', 'X', 'O', '9'},
        {'1', 'O', '3', '4', 'X', 'X', '7', 'O', 'X'},
        {'1', 'O', '3', '4', 'X', 'X', 'X', '8', 'O'},
        {'1', 'O', '3', '4', 'X', 'X', '7', '8', '9'},
        {'1', 'O', 'O', '4', 'X', '6', 'X', 'X', 'O'},
        {'1', 'O', 'O', '4', 'X', '6', 'X', 'X', '9'},
        {'1', 'O', 'O', 'O', 'X', '6', 'X', '8', 'X'},
        {'1', 'O', 'O', '4', 'X', 'O', 'X', '8', 'X'},
        {'1', 'O', 'O', '4', 'X', '6', 'X', 'O', 'X'},
        {'1', 'O', 'O', '4', 'X', '6', 'X', '8', 'X'},
        {'1', 'O', 'O', '4', 'X', '6', 'X', '8', '9'},
        {'1', 'O', '3', 'O', 'X', '6', 'X', 'X', '9'},
        {'1', 'O', '3', 'O', 'X', 'O', 'X', '8', 'X'},
        {'1', 'O', '3', 'O', 'X', '6', 'X', 'O', 'X'},
        {'1', 'O', '3', 'O', 'X', '6', 'X', '8', 'X'},
        {'1', 'O', '3', '4', 'X', 'O', 'X', 'X', '9'},
        {'1', 'O', '3', '4', 'X', 'O', 'X', 'O', 'X'},
        {'1', 'O', '3', '4', 'X', 'O', 'X', '8', 'X'},
        {'1', 'O', '3', '4', 'X', '6', 'X', 'O', 'X'},
        {'1', 'O', '3', '4', 'X', '6', 'X', '8', '9'},
        {'1', 'O', 'O', 'O', 'X', '6', '7', 'X', 'X'},
        {'1', 'O', 'O', '4', 'X', 'O', '7', 'X', 'X'},
        {'1', 'O', 'O', '4', 'X', '6', 'O', 'X', 'X'},
        {'1', 'O', 'O', '4', 'X', '6', '7', 'X', 'X'},
        {'1', 'O', 'O', '4', 'X', '6', '7', 'X', '9'},
        {'1', 'O', '3', 'O', 'X', 'O', '7', 'X', 'X'},
        {'1', 'O', '3', 'O', 'X', '6', 'O', 'X', 'X'},
        {'1', 'O', '3', 'O', 'X', '6', '7', 'X', 'X'},
        {'1', 'O', '3', '4', 'X', 'O', 'O', 'X', 'X'},
        {'1', 'O', '3', '4', 'X', 'O', '7', 'X', 'X'},
        {'1', 'O', '3', '4', 'X', '6', 'O', 'X', 'X'},
        {'1', 'O', '3', '4', 'X', '6', 'O', 'X', '9'},
        {'1', 'O', '3', '4', 'X', '6', '7', 'X', 'O'},
        {'1', 'O', '3', '4', 'X', '6', '7', 'X', '9'},
        {'1', 'O', 'O', '4', 'X', '6', '7', '8', 'X'},
        {'1', 'O', '3', 'O', 'X', '6', '7', '8', 'X'},
        {'1', 'O', '3', '4', 'X', 'O', '7', '8', 'X'},
        {'1', 'O', '3', '4', 'X', '6', 'O', '8', 'X'},
        {'1', 'O', '3', '4', 'X', '6', '7', 'O', 'X'},
        {'1', 'O', '3', '4', 'X', '6', '7', '8', 'X'},
        {'1', 'O', '3', '4', 'X', '6', '7', '8', '9'},
        {'1', '2', 'O', 'O', 'X', 'X', 'X', 'O', 'X'},
        {'1', '2', 'O', 'O', 'X', 'X', 'X', 'O', '9'},
        {'1', '2', 'O', 'O', 'X', 'X', 'X', '8', 'O'},
        {'1', '2', 'O', 'O', 'X', 'X', 'X', '8', '9'},
        {'1', '2', 'O', 'O', 'X', 'X', 'O', 'X', 'X'},
        {'1', '2', 'O', 'O', 'X', 'X', 'O', '8', 'X'},
        {'1', '2', 'O', 'O', 'X', 'X', '7', 'O', 'X'},
        {'1', '2', 'O', 'O', 'X', 'X', '7', '8', 'X'},
        {'1', '2', 'O', 'O', 'X', 'X', '7', '8', '9'},
        {'1', '2', 'O', '4', 'X', 'X', 'O', 'X', '9'},
        {'1', '2', 'O', '4', 'X', 'X', 'O', 'O', 'X'},
        {'1', '2', 'O', '4', 'X', 'X', 'O', '8', 'X'},
        {'1', '2', 'O', '4', 'X', 'X', '7', 'O', 'X'},
        {'1', '2', 'O', '4', 'X', 'X', '7', 'X', 'O'},
        {'1', '2', 'O', 'O', 'X', '6', 'X', 'X', '9'},
        {'1', '2', 'O', 'O', 'X', 'O', 'X', '8', 'X'},
        {'1', '2', 'O', 'O', 'X', '6', 'X', 'O', 'X'},
        {'1', '2', 'O', 'O', 'X', '6', 'X', '8', 'X'},
        {'1', '2', 'O', '4', 'X', 'O', 'X', 'O', 'X'},
        {'1', '2', 'O', '4', 'X', 'O', 'X', '8', 'X'},
        {'1', '2', 'O', '4', 'X', '6', 'X', 'O', 'X'},
        {'1', '2', 'O', '4', 'X', '6', 'X', 'O', '9'},
        {'1', '2', 'O', '4', 'X', '6', 'X', '8', '9'},
        {'1', '2', 'O', 'O', 'X', 'O', '7', 'X', 'X'},
        {'1', '2', 'O', 'O', 'X', '6', 'O', 'X', 'X'},
        {'1', '2', 'O', 'O', 'X', '6', '7', 'X', 'X'},
        {'1', '2', 'O', '4', 'X', 'O', 'O', 'X', 'X'},
        {'1', '2', 'O', '4', 'X', 'O', '7', 'X', 'X'},
        {'1', '2', 'O', '4', 'X', '6', 'O', 'X', 'X'},
        {'1', '2', 'O', 'O', 'X', '6', '7', '8', 'X'},
        {'1', '2', 'O', '4', 'X', 'O', '7', '8', 'X'},
        {'1', '2', 'O', '4', 'X', '6', 'O', '8', 'X'},
        {'1', '2', 'O', '4', 'X', '6', '7', 'O', 'X'},
        {'1', '2', 'O', '4', 'X', '6', '7', '8', 'X'},
        {'1', '2', 'O', '4', 'X', '6', '7', '8', '9'},
        {'1', '2', '3', 'O', 'X', 'X', 'O', 'X', '9'},
        {'1', '2', '3', 'O', 'X', 'X', 'O', 'O', 'X'},
        {'1', '2', '3', 'O', 'X', 'X', 'O', '8', 'X'},
        {'1', '2', '3', 'O', 'X', 'X', 'O', '8', '9'},
        {'1', '2', '3', 'O', 'X', 'X', '7', 'O', 'X'},
        {'1', '2', '3', 'O', 'X', 'X', '7', '8', 'O'},
        {'1', '2', '3', 'O', 'X', 'X', '7', '8', '9'},
        {'1', '2', '3', 'O', 'X', 'O', 'X', 'X', '9'},
        {'1', '2', '3', 'O', 'X', 'O', 'X', 'O', 'X'},
        {'1', '2', '3', 'O', 'X', 'O', 'X', '8', 'X'},
        {'1', '2', '3', 'O', 'X', '6', 'X', 'O', 'X'},
        {'1', '2', '3', 'O', 'X', '6', 'X', 'X', 'O'},
        {'1', '2', '3', 'O', 'X', '6', 'X', '8', '9'},
        {'1', '2', '3', 'O', 'X', 'O', 'O', 'X', 'X'},
        {'1', '2', '3', 'O', 'X', 'O', '7', 'X', 'X'},
        {'1', '2', '3', 'O', 'X', '6', 'O', 'X', 'X'},
        {'1', '2', '3', 'O', 'X', '6', '7', 'X', '9'},
        {'1', '2', '3', 'O', 'X', 'O', '7', '8', 'X'},
        {'1', '2', '3', 'O', 'X', '6', 'O', '8', 'X'},
        {'1', '2', '3', 'O', 'X', '6', '7', 'O', 'X'},
        {'1', '2', '3', 'O', 'X', '6', '7', '8', 'X'},
        {'1', '2', '3', 'O', 'X', '6', '7', '8', '9'},
        {'1', '2', '3', '4', 'X', 'O', 'X', 'O', 'X'},
        {'1', '2', '3', '4', 'X', 'O', 'X', '8', '9'},
        {'1', '2', '3', '4', 'X', 'O', 'O', 'X', 'X'},
        {'1', '2', '3', '4', 'X', 'O', '7', 'X', '9'},
        {'1', '2', '3', '4', 'X', 'O', 'O', '8', 'X'},
        {'1', '2', '3', '4', 'X', 'O', '7', 'O', 'X'},
        {'1', '2', '3', '4', 'X', 'O', '7', '8', 'X'},
        {'1', '2', '3', '4', 'X', 'O', '7', '8', '9'},
        {'1', '2', '3', '4', 'X', 'X', 'O', 'O', 'X'},
        {'1', '2', '3', '4', 'X', 'X', 'O', 'X', 'O'},
        {'1', '2', '3', '4', 'X', '6', 'O', 'O', 'X'},
        {'1', '2', '3', '4', 'X', '6', 'O', '8', 'X'},
        {'1', '2', '3', '4', 'X', '6', 'O', '8', '9'},
        {'1', '2', '3', '4', 'X', 'X', 'X', 'O', 'O'},
        {'1', '2', '3', '4', 'X', 'X', '7', 'O', '9'},
        {'1', '2', '3', '4', 'X', '6', 'X', 'O', '9'},
        {'1', '2', '3', '4', 'X', '6', '7', 'O', 'X'},
        {'1', '2', '3', '4', 'X', '6', '7', 'O', '9'},
        {'1', '2', '3', '4', 'X', '6', '7', '8', 'O'},
        {'1', '2', '3', '4', 'X', '6', '7', '8', '9'},
        {'1', 'O', 'O', '4', '5', 'X', 'X', 'X', 'O'},
        {'1', 'O', 'O', '4', '5', 'X', 'X', 'X', '9'},
        {'1', 'O', 'O', '4', '5', 'X', 'X', 'O', 'X'},
        {'1', 'O', 'O', '4', '5', 'X', 'X', '8', 'X'},
        {'1', 'O', 'O', '4', '5', 'X', 'X', '8', '9'},
        {'1', 'O', '3', 'O', '5', 'X', 'X', 'X', 'O'},
        {'1', 'O', '3', 'O', '5', 'X', 'X', '8', 'X'},
        {'1', 'O', '3', '4', 'O', 'X', 'X', 'X', 'O'},
        {'1', 'O', '3', '4', '5', 'X', 'X', 'X', 'O'},
        {'1', 'O', 'O', '4', '5', 'X', 'O', 'X', 'X'},
        {'1', 'O', 'O', '4', '5', 'X', '7', 'X', 'X'},
        {'1', 'O', 'O', '4', '5', 'X', '7', 'X', '9'},
        {'1', 'O', '3', 'O', '5', 'X', '7', 'X', 'X'},
        {'1', 'O', '3', '4', 'O', 'X', '7', 'X', 'X'},
        {'1', 'O', '3', '4', '5', 'X', 'O', 'X', '9'},
        {'1', 'O', '3', '4', '5', 'X', '7', 'X', 'O'},
        {'1', 'O', 'O', '4', '5', 'X', '7', '8', 'X'},
        {'1', '2', 'O', 'O', '5', 'X', 'X', 'X', 'O'},
        {'1', '2', 'O', 'O', '5', 'X', 'X', 'O', 'X'},
        {'1', '2', 'O', 'O', '5', 'X', 'X', '8', '9'},
        {'1', '2', 'O', '4', 'O', 'X', 'X', 'X', 'O'},
        {'1', '2', 'O', '4', 'O', 'X', 'X', '8', '9'},
        {'1', '2', 'O', '4', '5', 'X', 'X', 'X', 'O'},
        {'1', '2', 'O', '4', '5', 'X', 'X', '8', '9'},
        {'1', '2', 'O', 'O', '5', 'X', 'O', 'X', 'X'},
        {'1', '2', 'O', 'O', '5', 'X', '7', 'X', '9'},
        {'1', '2', 'O', '4', '5', 'X', '7', 'X', '9'},
        {'1', '2', 'O', 'O', '5', 'X', '7', '8', 'X'},
        {'1', '2', 'O', '4', '5', 'X', '7', '8', 'X'},
        {'1', '2', 'O', '4', '5', 'X', '7', '8', '9'},
        {'1', '2', '3', 'O', 'O', 'X', 'X', 'X', 'O'},
        {'1', '2', '3', 'O', 'O', 'X', 'X', '8', 'X'},
        {'1', '2', '3', 'O', '5', 'X', 'X', '8', 'O'},
        {'1', '2', '3', 'O', 'O', 'X', '7', 'X', 'X'},
        {'1', '2', '3', 'O', '5', 'X', 'O', 'X', 'X'},
        {'1', '2', '3', 'O', '5', 'X', 'O', 'X', '9'},
        {'1', '2', '3', 'O', '5', 'X', '7', 'X', 'O'},
        {'1', '2', '3', 'O', '5', 'X', '7', '8', '9'},
        {'1', '2', '3', '4', 'O', 'X', 'X', 'X', 'O'},
        {'1', '2', '3', '4', 'O', 'X', 'X', '8', 'O'},
        {'1', '2', '3', '4', 'O', 'X', '7', 'X', 'O'},
        {'1', '2', '3', '4', 'O', 'X', '7', '8', '9'},
        {'1', '2', '3', '4', '5', 'X', 'O', 'X', '9'},
        {'1', '2', '3', '4', '5', 'X', 'O', '8', 'X'},
        {'1', '2', '3', '4', '5', 'X', 'X', 'O', 'O'},
        {'1', '2', '3', '4', '5', 'X', '7', 'O', 'X'},
        {'1', '2', '3', '4', '5', 'X', '7', '8', 'O'},
        {'1', 'O', '3', '4', '5', '6', 'X', 'X', 'O'},
        {'1', 'O', '3', '4', '5', '6', 'X', '8', 'X'},
        {'1', 'O', '3', '4', '5', '6', 'X', '8', '9'},
        {'1', '2', 'O', '4', '5', '6', 'X', 'X', 'O'},
        {'1', '2', 'O', '4', '5', '6', 'X', 'O', 'X'},
        {'1', '2', 'O', '4', '5', '6', 'X', '8', 'X'},
        {'1', '2', 'O', '4', '5', '6', 'X', '8', '9'},
        {'1', '2', '3', 'O', '5', '6', 'X', 'X', '9'},
        {'1', '2', '3', 'O', '5', '6', 'X', '8', 'X'},
        {'1', '2', '3', '4', 'O', '6', 'X', 'X', 'O'},
        {'1', '2', '3', '4', 'O', '6', 'X', '8', '9'},
        {'1', '2', '3', '4', '5', 'O', 'X', 'X', 'O'},
        {'1', '2', '3', '4', '5', 'O', 'X', 'O', 'X'},
        {'1', '2', '3', '4', '5', 'O', 'X', '8', 'X'},
        {'1', '2', '3', '4', '5', 'O', 'X', '8', '9'},
        {'1', '2', '3', '4', '5', '6', 'X', 'O', '9'},
        {'1', '2', '3', '4', '5', '6', 'X', '8', 'O'},
        {'1', 'O', '3', '4', '5', '6', 'O', 'X', 'X'},
        {'1', 'O', '3', '4', '5', '6', '7', 'X', '9'},
        {'1', '2', 'O', '4', '5', '6', '7', 'X', 'X'},
        {'1', '2', '3', 'O', '5', '6', 'O', 'X', 'X'},
        {'1', '2', '3', '4', 'O', '6', '7', 'X', '9'},
        {'1', '2', '3', '4', '5', 'O', '7', 'X', 'X'},
        {'1', '2', '3', '4', '5', '6', 'O', 'X', 'X'},
        {'1', '2', '3', '4', '5', '6', 'O', 'X', '9'},
        {'1', '2', '3', '4', '5', '6', '7', 'X', 'O'},
        {'1', '2', 'O', '4', '5', '6', '7', '8', 'X'},
        {'1', '2', '3', '4', 'O', '6', '7', '8', 'X'},
        {'1', '2', '3', '4', '5', '6', 'O', '8', 'X'},
        {'1', '2', '3', '4', '5', '6', '7', '8', '9'}
    },
    {
        {'X', '2', 'O', 'X', 'O', 'X', '7', 'O', 'X'},
        {'X', '2', 'O', 'X', '5', 'O', 'O', 'X', '9'},
        {'X', '2', 'O', 'X', '5', 'O', '7', '8', 'X'},
        {'X', '2', 'O', 'X', 'X', '6', 'O', '8', '9'},
        {'X', '2', 'O', 'X', '5', '6', 'O', 'X', 'O'},
        {'X', '2', 'O', 'X', 'X', '6', '7', 'O', '9'},
        {'X', '2', 'O', 'X', '5', 'X', '7', 'O', '9'},
        {'X', '2', 'O', 'X', '5', '6', '7', 'O', 'X'},
        {'X', '2', 'O', 'X', '5', 'X', '7', '8', 'O'},
        {'X', '2', 'O', 'X', '5', '6', '7', '8', '9'},
        {'X', '2', 'O', 'O', 'X', 'X', 'O', 'X', 'O'},
        {'X', '2', 'O', 'O', 'X', 'X', 'O', 'X', '9'},
        {'X', '2', 'O', 'O', 'X', 'X', 'X', 'O', 'O'},
        {'X', '2', 'O', 'O', 'X', 'X', 'X', '8', 'O'},
        {'X', '2', 'O', 'O', 'X', 'X', '7', 'X', 'O'},
        {'X', '2', 'O', 'O', 'X', 'X', '7', '8', 'O'},
        {'X', '2', 'O', 'O', 'X', 'O', '7', 'X', '9'},
        {'X', '2', 'O', 'O', 'X', '6', 'O', 'X', '9'},
        {'X', '2', 'O', 'O', 'X', '6', '7', 'X', 'O'},
        {'X', '2', 'O', 'O', 'X', '6', '7', 'X', '9'},
        {'X', '2', 'O', '4', 'X', 'O', 'O', 'X', '9'},
        {'X', '2', 'O', '4', 'X', 'X', 'O', 'X', 'O'},
        {'X', '2', 'O', '4', 'X', 'X', 'O', '8', '9'},
        {'X', '2', 'O', '4', 'X', '6', 'O', 'X', 'O'},
        {'X', '2', 'O', '4', 'X', '6', 'O', 'X', '9'},
        {'X', '2', 'O', '4', 'X', 'X', '7', 'O', '9'},
        {'X', '2', 'O', '4', 'X', '6', 'X', 'O', '9'},
        {'X', '2', 'O', 'O', 'O', 'X', 'X', 'X', 'O'},
        {'X', '2', 'O', 'O', 'O', 'X', 'X', 'O', 'X'},
        {'X', '2', 'O', 'O', 'O', 'X', 'X', '8', '9'},
        {'X', '2', 'O', 'O', '5', 'X', 'X', 'O', '9'},
        {'X', '2', 'O', 'O', '5', 'X', 'X', 'X', 'O'},
        {'X', '2', 'O', 'O', '5', 'X', 'X', '8', 'O'},
        {'X', '2', 'O', 'O', '5', 'X', '7', 'X', 'O'},
        {'X', '2', 'O', 'O', '5', 'X', '7', '8', '9'},
        {'X', '2', 'O', '4', 'O', 'X', 'X', 'O', 'X'},
        {'X', '2', 'O', '4', 'O', 'X', '7', 'O', 'X'},
        {'X', '2', 'O', 'O', '5', '6', 'X', '8', 'X'},
        {'X', '2', 'O', '4', 'O', '6', 'X', 'X', '9'},
        {'X', '2', 'O', '4', 'O', '6', 'X', '8', 'X'},
        {'X', '2', 'O', '4', '5', 'O', 'X', '8', 'X'},
        {'X', '2', 'O', '4', '5', '6', 'X', 'O', 'X'},
        {'X', '2', 'O', '4', '5', '6', 'X', '8', '9'},
        {'X', '2', 'O', 'O', '5', '6', '7', 'X', 'X'},
        {'X', '2', 'O', '4', '5', 'O', '7', 'X', 'X'},
        {'X', '2', 'O', '4', '5', '6', '7', '8', 'X'},
        {'X', '2', 'X', 'O', 'O', 'X', 'O', 'X', 'O'},
        {'X', '2', 'X', 'O', 'O', 'X', 'O', 'X', '9'},
        {'X', '2', 'X', 'O', 'O', 'X', 'O', '8', '9'},
        {'X', '2', 'X', 'O', 'O', 'X', 'X', 'O', 'O'},
        {'X', '2', 'X', 'O', 'O', 'X', 'X', 'O', '9'},
        {'X', '2', 'X', 'O', 'O', 'X', '7', 'O', '9'},
        {'X', '2', 'X', 'O', 'O', 'X', 'X', '8', 'O'},
        {'X', '2', 'X', 'O', 'O', 'X', '7', 'X', 'O'},
        {'X', '2', 'X', 'O', 'O', 'X', '7', '8', 'O'},
        {'X', '2', 'X', 'O', 'O', 'X', '7', '8', '9'},
        {'X', '2', 'X', 'O', 'O', '6', 'X', 'O', 'X'},
        {'X', '2', 'X', 'O', 'O', '6', 'X', 'O', '9'},
        {'X', '2', 'X', 'O', 'O', '6', 'X', '8', 'O'},
        {'X', '2', 'X', 'O', 'O', '6', 'O', 'X', '9'},
        {'X', '2', 'X', 'O', 'O', '6', '7', 'X', 'O'},
        {'X', '2', 'X', 'O', 'O', '6', 'O', '8', 'X'},
        {'X', '2', 'X', 'O', 'O', '6', '7', 'O', 'X'},
        {'X', '2', 'X', 'O', 'O', '6', '7', '8', '9'},
        {'X', '2', 'X', 'O', 'X', 'O', 'O', 'X', 'O'},
        {'X', '2', 'X', 'O', 'X', 'O', 'O', 'X', '9'},
        {'X', '2', 'X', 'O', 'X', 'O', 'O', '8', '9'},
        {'X', '2', 'X', 'O', 'X', 'O', '7', 'O', '9'},
        {'X', '2', 'X', 'O', 'X', 'O', '7', 'X', 'O'},
        {'X', '2', 'X', 'O', 'X', 'O', '7', '8', 'O'},
        {'X', '2', 'X', 'O', 'X', 'O', '7', '8', '9'},
        {'X', '2', 'X', 'O', '5', 'O', 'X', 'O', '9'},
        {'X', '2', 'X', 'O', '5', 'O', 'X', '8', 'O'},
        {'X', '2', 'X', 'O', '5', 'O', 'O', 'X', '9'},
        {'X', '2', 'X', 'O', '5', 'O', '7', 'X', 'O'},
        {'X', '2', 'X', 'O', '5', 'O', 'O', '8', 'X'},
        {'X', '2', 'X', 'O', '5', 'O', '7', 'O', 'X'},
        {'X', '2', 'X', 'O', '5', 'O', '7', '8', '9'},
        {'X', '2', 'X', 'O', 'X', '6', 'O', 'O', '9'},
        {'X', '2', 'X', 'O', 'X', '6', 'O', 'X', 'O'},
        {'X', '2', 'X', 'O', 'X', '6', 'O', '8', 'O'},
        {'X', '2', 'X', 'O', 'X', '6', 'O', '8', '9'},
        {'X', '2', 'X', 'O', '5', 'X', 'O', 'O', '9'},
        {'X', '2', 'X', 'O', '5', 'X', 'O', 'X', 'O'},
        {'X', '2', 'X', 'O', '5', 'X', 'O', '8', 'O'},
        {'X', '2', 'X', 'O', '5', 'X', 'O', '8', '9'},
        {'X', '2', 'X', 'O', '5', '6', 'O', 'X', 'O'},
        {'X', '2', 'X', 'O', '5', '6', 'O', 'X', '9'},
        {'X', '2', 'X', 'O', '5', '6', 'O', 'O', 'X'},
        {'X', '2', 'X', 'O', '5', '6', 'O', '8', 'X'},
        {'X', '2', 'X', 'O', '5', '6', 'O', '8', '9'},
        {'X', '2', 'X', 'O', 'X', '6', '7', 'O', 'O'},
        {'X', '2', 'X', 'O', 'X', '6', '7', 'O', '9'},
        {'X', '2', 'X', 'O', '5', 'X', 'X', 'O', 'O'},
        {'X', '2', 'X', 'O', '5', 'X', '7', 'O', 'O'},
        {'X', '2', 'X', 'O', '5', 'X', '7', 'O', '9'},
        {'X', '2', 'X', 'O', '5', '6', 'X', 'O', 'O'},
        {'X', '2', 'X', 'O', '5', '6', 'X', 'O', '9'},
        {'X', '2', 'X', 'O', '5', '6', '7', 'O', 'X'},
        {'X', '2', 'X', 'O', '5', '6', '7', 'O', '9'},
        {'X', '2', 'X', 'O', 'X', '6', '7', '8', 'O'},
        {'X', '2', 'X', 'O', '5', 'X', '7', '8', 'O'},
        {'X', '2', 'X', 'O', '5', '6', 'X', '8', 'O'},
        {'X', '2', 'X', 'O', '5', '6', '7', 'X', 'O'},
        {'X', '2', 'X', 'O', '5', '6', '7', '8', 'O'},
        {'X', '2', 'X', 'O', '5', '6', '7', '8', '9'},
        {'X', '2', '3', 'O', 'X', 'O', 'X', '8', '9'},
        {'X', '2', '3', 'O', 'X', 'O', 'O', 'X', '9'},
        {'X', '2', '3', 'O', 'X', 'O', '7', 'X', 'O'},
        {'X', '2', '3', 'O', 'X', 'O', '7', 'X', '9'},
        {'X', '2', '3', 'O', 'X', 'X', 'O', 'X', 'O'},
        {'X', '2', '3', 'O', 'X', '6', 'O', 'X', 'O'},
        {'X', '2', '3', 'O', 'X', '6', 'O', 'X', '9'},
        {'X', '2', '3', 'O', 'X', '6', 'X', 'O', '9'},
        {'X', '2', '3', 'O', 'X', 'X', '7', '8', 'O'},
        {'X', '2', '3', 'O', 'X', '6', '7', 'X', 'O'},
        {'X', '2', '3', 'O', 'X', '6', '7', '8', 'O'},
        {'X', '2', '3', 'O', 'X', '6', '7', '8', '9'},
        {'X', '2', '3', 'O', 'O', 'X', 'X', 'O', 'X'},
        {'X', '2', '3', 'O', 'O', 'X', 'X', 'O', '9'},
        {'X', '2', '3', 'O', 'O', 'X', 'X', 'X', 'O'},
        {'X', '2', '3', 'O', 'O', 'X', 'X', '8', 'O'},
        {'X', '2', '3', 'O', 'O', 'X', 'X', '8', '9'},
        {'X', '2', '3', 'O', 'O', 'X', '7', 'X', 'O'},
        {'X', '2', '3', 'O', '5', 'X', 'O', 'X', 'O'},
        {'X', '2', '3', 'O', '5', 'X', 'O', 'X', '9'},
        {'X', '2', '3', 'O', '5', 'X', 'O', '8', 'X'},
        {'X', '2', '3', 'O', '5', 'X', 'O', '8', '9'},
        {'X', '2', '3', 'O', '5', 'X', '7', 'O', 'X'},
        {'X', '2', '3', 'O', '5', 'X', 'X', '8', 'O'},
        {'X', '2', '3', 'O', '5', 'X', '7', 'X', 'O'},
        {'X', '2', '3', 'O', '5', 'X', '7', '8', 'O'},
        {'X', '2', '3', 'O', 'O', '6', 'X', 'O', 'X'},
        {'X', '2', '3', 'O', '5', 'O', 'X', 'X', 'O'},
        {'X', '2', '3', 'O', 'O', '6', 'O', 'X', 'X'},
        {'X', '2', '3', 'O', '5', '6', 'O', 'X', '9'},
        {'X', '2', '3', 'O', '5', '6', '7', 'X', 'O'},
        {'X', '2', '3', 'O', '5', '6', '7', '8', '9'},
        {'X', '2', 'X', 'X', 'O', 'O', 'O', 'X', 'O'},
        {'X', '2', 'X', 'X', 'O', 'O', 'O', 'X', '9'},
        {'X', '2', 'X', 'X', 'O', 'O', 'O', 'O', 'X'},
        {'X', '2', 'X', 'X', 'O', 'O', 'O', '8', 'X'},
        {'X', '2', 'X', 'X', 'O', 'O', 'O', '8', '9'},
        {'X', '2', 'X', 'X', 'O', 'O', '7', 'O', 'X'},
        {'X', '2', 'X', 'X', 'O', 'O', '7', 'O', '9'},
        {'X', '2', 'X', 'X', 'O', 'O', '7', 'X', 'O'},
        {'X', '2', 'X', 'X', 'O', 'O', '7', '8', 'O'},
        {'X', '2', 'X', 'X', 'O', 'O', '7', '8', '9'},
        {'X', '2', 'X', '4', 'O', 'O', 'X', 'O', 'X'},
        {'X', '2', 'X', '4', 'O', 'O', 'X', 'O', '9'},
        {'X', '2', 'X', '4', 'O', 'O', 'X', '8', 'O'},
        {'X', '2', 'X', '4', 'O', 'O', 'O', 'X', '9'},
        {'X', '2', 'X', '4', 'O', 'O', '7', 'X', 'O'},
        {'X', '2', 'X', '4', 'O', 'O', 'O', '8', 'X'},
        {'X', '2', 'X', '4', 'O', 'O', '7', 'O', 'X'},
        {'X', '2', 'X', '4', 'O', 'O', '7', '8', '9'},
        {'X', '2', 'X', 'X', 'O', 'X', 'O', 'O', '9'},
        {'X', '2', 'X', 'X', 'O', '6', 'O', 'O', 'X'},
        {'X', '2', 'X', 'X', 'O', '6', 'O', 'O', '9'},
        {'X', '2', 'X', 'X', 'O', '6', 'O', 'X', 'O'},
        {'X', '2', 'X', 'X', 'O', '6', 'O', '8', 'O'},
        {'X', '2', 'X', 'X', 'O', '6', 'O', '8', '9'},
        {'X', '2', 'X', '4', 'O', 'X', 'O', 'O', '9'},
        {'X', '2', 'X', '4', 'O', 'X', 'O', 'X', 'O'},
        {'X', '2', 'X', '4', 'O', 'X', 'O', '8', 'O'},
        {'X', '2', 'X', '4', 'O', 'X', 'O', '8', '9'},
        {'X', '2', 'X', '4', 'O', '6', 'O', 'X', 'O'},
        {'X', '2', 'X', '4', 'O', '6', 'O', 'X', '9'},
        {'X', '2', 'X', '4', 'O', '6', 'O', 'O', 'X'},
        {'X', '2', 'X', '4', 'O', '6', 'O', '8', 'X'},
        {'X', '2', 'X', '4', 'O', '6', 'O', '8', '9'},
        {'X', '2', 'X', 'X', 'O', 'X', '7', 'O', 'O'},
        {'X', '2', 'X', 'X', 'O', '6', '7', 'O', 'O'},
        {'X', '2', 'X', 'X', 'O', '6', '7', 'O', '9'},
        {'X', '2', 'X', '4', 'O', 'X', 'X', 'O', 'O'},
        {'X', '2', 'X', '4', 'O', 'X', '7', 'O', 'O'},
        {'X', '2', 'X', '4', 'O', 'X', '7', 'O', '9'},
        {'X', '2', 'X', '4', 'O', '6', 'X', 'O', 'O'},
        {'X', '2', 'X', '4', 'O', '6', 'X', 'O', '9'},
        {'X', '2', 'X', '4', 'O', '6', '7', 'O', 'X'},
        {'X', '2', 'X', '4', 'O', '6', '7', 'O', '9'},
        {'X', '2', 'X', 'X', 'O', '6', '7', '8', 'O'},
        {'X', '2', 'X', '4', 'O', 'X', '7', '8', 'O'},
        {'X', '2', 'X', '4', 'O', '6', 'X', '8', 'O'},
        {'X', '2', 'X', '4', 'O', '6', '7', 'X', 'O'},
        {'X', '2', 'X', '4', 'O', '6', '7', '8', 'O'},
        {'X', '2', 'X', '4', 'O', '6', '7', '8', '9'},
        {'X', '2', '3', 'X', 'O', 'X', 'O', 'O', 'X'},
        {'X', '2', '3', 'X', 'O', 'X', 'O', 'O', '9'},
        {'X', '2', '3', 'X', 'O', 'X', 'O', '8', 'O'},
        {'X', '2', '3', 'X', 'O', '6', 'O', 'O', 'X'},
        {'X', '2', '3', 'X', 'O', 'X', '7', 'O', '9'},
        {'X', '2', '3', 'X', 'O', '6', '7', 'O', 'X'},
        {'X', '2', '3', '4', 'O', 'X', 'X', 'O', '9'},
        {'X', '2', '3', '4', 'O', 'X', '7', 'O', 'X'},
        {'X', '2', '3', '4', 'O', 'X', '7', 'O', '9'},
        {'X', '2', '3', '4', 'O', 'X', '7', 'X', 'O'},
        {'X', '2', '3', '4', 'O', 'X', '7', '8', 'O'},
        {'X', '2', '3', '4', 'O', 'X', '7', '8', '9'},
        {'X', '2', '3', '4', 'O', '6', 'X', 'O', 'X'},
        {'X', '2', '3', '4', 'O', 'O', 'O', 'X', 'X'},
        {'X', '2', '3', '4', 'O', '6', '7', 'O', 'X'},
        {'X', '2', '3', '4', 'O', '6', '7', '8', 'X'},
        {'X', '2', '3', '4', 'O', '6', '7', '8', '9'},
        {'X', '2', 'X', 'X', '5', 'O', 'O', 'O', 'X'},
        {'X', '2', 'X', 'X', '5', 'O', 'O', 'O', '9'},
        {'X', '2', 'X', 'X', '5', 'O', 'O', 'X', 'O'},
        {'X', '2', 'X', 'X', '5', 'O', 'O', '8', 'O'},
        {'X', '2', 'X', 'X', '5', 'O', 'O', '8', '9'},
        {'X', '2', 'X', '4', 'X', 'O', 'O', 'O', '9'},
        {'X', '2', 'X', '4', 'X', 'O', 'O', 'X', 'O'},
        {'X', '2', 'X', '4', 'X', 'O', 'O', '8', 'O'},
        {'X', '2', 'X', '4', 'X', 'O', 'O', '8', '9'},
        {'X', '2', 'X', '4', '5', 'O', 'O', 'X', 'O'},
        {'X', '2', 'X', '4', '5', 'O', 'O', 'X', '9'},
        {'X', '2', 'X', '4', '5', 'O', 'O', 'O', 'X'},
        {'X', '2', 'X', '4', '5', 'O', 'O', '8', 'X'},
        {'X', '2', 'X', '4', '5', 'O', 'O', '8', '9'},
        {'X', '2', 'X', 'X', '5', 'O', '7', 'O', 'O'},
        {'X', '2', 'X', 'X', '5', 'O', '7', 'O', '9'},
        {'X', '2', 'X', '4', 'X', 'O', '7', 'O', 'O'},
        {'X', '2', 'X', '4', 'X', 'O', '7', 'O', '9'},
        {'X', '2', 'X', '4', '5', 'O', 'X', 'O', 'O'},
        {'X', '2', 'X', '4', '5', 'O', 'X', 'O', '9'},
        {'X', '2', 'X', '4', '5', 'O', '7', 'O', 'X'},
        {'X', '2', 'X', '4', '5', 'O', '7', 'O', '9'},
        {'X', '2', 'X', 'X', '5', 'O', '7', '8', 'O'},
        {'X', '2', 'X', '4', 'X', 'O', '7', '8', 'O'},
        {'X', '2', 'X', '4', '5', 'O', 'X', '8', 'O'},
        {'X', '2', 'X', '4', '5', 'O', '7', 'X', 'O'},
        {'X', '2', 'X', '4', '5', 'O', '7', '8', 'O'},
        {'X', '2', 'X', '4', '5', 'O', '7', '8', '9'},
        {'X', '2', '3', 'X', 'X', 'O', 'O', '8', 'O'},
        {'X', '2', '3', 'X', 'X', 'O', '7', 'O', '9'},
        {'X', '2', '3', 'X', '5', 'O', '7', 'O', 'X'},
        {'X', '2', '3', '4', 'X', 'O', 'O', 'X', 'O'},
        {'X', '2', '3', '4', 'X', 'O', 'O', 'X', '9'},
        {'X', '2', '3', '4', 'X', 'O', 'X', 'O', '9'},
        {'X', '2', '3', '4', 'X', 'O', '7', '8', '9'},
        {'X', '2', '3', '4', '5', 'O', 'X', 'O', 'X'},
        {'X', '2', '3', '4', '5', 'O', 'X', '8', '9'},
        {'X', '2', '3', '4', '5', 'O', 'O', 'X', '9'},
        {'X', '2', 'X', '4', '5', '6', 'O', 'O', 'X'},
        {'X', '2', 'X', '4', '5', '6', 'O', 'O', '9'},
        {'X', '2', 'X', '4', '5', '6', 'O', 'X', 'O'},
        {'X', '2', 'X', '4', '5', '6', 'O', '8', 'O'},
        {'X', '2', 'X', '4', '5', '6', 'O', '8', '9'},
        {'X', '2', '3', 'X', '5', '6', 'O', 'O', '9'},
        {'X', '2', '3', 'X', '5', '6', 'O', '8', 'O'},
        {'X', '2', '3', '4', 'X', '6', 'O', 'X', 'O'},
        {'X', '2', '3', '4', '5', 'X', 'O', 'O', 'X'},
        {'X', '2', '3', '4', '5', 'X', 'O', 'X', 'O'},
        {'X', '2', '3', '4', '5', '6', 'O', 'X', 'O'},
        {'X', '2', '3', '4', '5', '6', 'O', 'X', '9'},
        {'X', '2', '3', '4', '5', '6', 'O', '8', 'X'},
        {'X', '2', '3', '4', '5', '6', 'O', '8', '9'},
        {'X', '2', 'X', '4', '5', '6', 'X', 'O', 'O'},
        {'X', '2', 'X', '4', '5', '6', '7', 'O', 'O'},
        {'X', '2', 'X', '4', '5', '6', '7', 'O', '9'},
        {'X', '2', '3', '4', 'X', '6', 'X', 'O', 'O'},
        {'X', '2', '3', '4', 'X', '6', '7', 'O', '9'},
        {'X', '2', '3', '4', '5', 'X', 'X', 'O', 'O'},
        {'X', '2', '3', '4', '5', '6', 'X', 'O', '9'},
        {'X', '2', 'X', '4', '5', '6', '7', '8', 'O'},
        {'X', '2', '3', '4', '5', '6', 'X', '8', 'O'},
        {'X', '2', '3', '4', '5', '6', '7', 'X', 'O'},
        {'O', '2', 'X', 'X', 'O', 'X', 'X', 'O', '9'},
        {'O', '2', 'X', 'X', 'O', 'O', 'X', 'O', 'X'},
        {'O', '2', 'X', 'X', 'O', '6', 'X', 'O', 'X'},
        {'O', '2', 'X', 'X', 'O', '6', 'X', 'O', '9'},
        {'O', '2', 'X', 'X', 'O', 'O', 'O', 'X', 'X'},
        {'O', '2', 'X', 'X', 'O', 'O', '7', '8', 'X'},
        {'O', '2', 'X', 'X', 'X', 'O', 'O', 'X', 'O'},
        {'O', '2', 'X', 'X', 'X', 'O', 'O', 'X', '9'},
        {'O', '2', 'X', 'X', 'X', 'O', 'O', 'O', 'X'},
        {'O', '2', 'X', 'X', 'X', 'O', 'O', '8', 'X'},
        {'O', '2', 'X', 'X', 'X', 'O', 'O', '8', '9'},
        {'O', '2', 'X', 'X', 'X', 'O', '7', 'X', 'O'},
        {'O', '2', 'X', 'X', '5', 'O', 'O', 'X', 'X'},
        {'O', '2', 'X', 'X', '5', 'O', 'O', 'X', '9'},
        {'O', '2', 'X', 'X', '5', 'O', 'O', '8', 'X'},
        {'O', '2', 'X', 'X', '5', 'O', '7', 'O', 'X'},
        {'O', '2', 'X', 'X', '5', 'O', '7', '8', '9'},
        {'O', '2', 'X', 'X', 'X', '6', 'O', 'X', 'O'},
        {'O', '2', 'X', 'X', '5', 'X', 'O', '8', '9'},
        {'O', '2', 'X', 'X', 'X', '6', '7', 'O', '9'},
        {'O', '2', 'X', 'X', '5', 'X', '7', 'O', '9'},
        {'O', '2', 'X', 'X', 'X', '6', '7', '8', 'O'},
        {'O', '2', 'X', 'O', 'X', 'O', '7', 'X', '9'},
        {'O', '2', 'X', 'O', 'X', '6', '7', 'X', 'O'},
        {'O', '2', 'X', '4', 'X', 'O', 'O', 'X', '9'},
        {'O', '2', 'X', '4', 'X', 'O', '7', 'X', 'O'},
        {'O', '2', 'X', '4', 'X', 'O', '7', 'X', '9'},
        {'O', '2', 'X', '4', 'X', '6', 'O', 'X', 'O'},
        {'O', '2', 'X', '4', 'X', 'X', '7', 'O', '9'},
        {'O', '2', 'X', '4', 'X', '6', '7', 'O', 'X'},
        {'O', '2', 'X', '4', 'X', 'X', '7', '8', 'O'},
        {'O', '2', 'X', '4', 'X', '6', '7', 'X', 'O'},
        {'O', '2', 'X', 'O', '5', 'X', 'X', '8', '9'},
        {'O', '2', 'X', 'O', '5', 'X', '7', 'X', 'O'},
        {'O', '2', 'X', '4', '5', 'X', 'O', 'X', 'O'},
        {'O', '2', 'X', '4', '5', 'X', 'X', 'O', '9'},
        {'O', '2', 'X', '4', '5', 'X', '7', '8', '9'},
        {'O', '2', 'X', 'O', '5', '6', 'X', 'X', '9'},
        {'O', '2', 'X', 'O', '5', '6', 'X', '8', 'X'},
        {'O', '2', 'X', '4', 'O', '6', 'X', '8', 'X'},
        {'O', '2', 'X', '4', '5', 'O', 'X', 'X', '9'},
        {'O', '2', 'X', '4', '5', 'O', 'X', '8', 'X'},
        {'O', '2', 'X', '4', '5', '6', 'X', 'O', 'X'},
        {'O', '2', 'X', '4', '5', '6', 'X', '8', '9'},
        {'O', '2', 'X', '4', 'O', '6', '7', 'X', 'X'},
        {'O', '2', 'X', '4', '5', '6', '7', '8', 'X'},
        {'1', '2', 'X', 'O', 'X', 'O', 'O', 'X', '9'},
        {'1', '2', 'X', 'O', 'X', 'O', '7', 'X', 'O'},
        {'1', '2', 'X', 'O', 'X', '6', 'O', 'X', 'O'},
        {'1', '2', 'X', 'O', '5', '6', '7', 'X', 'O'},
        {'1', '2', 'X', 'X', 'O', 'O', 'X', 'O', 'X'},
        {'1', '2', 'X', 'X', 'O', 'O', '7', 'O', 'X'},
        {'1', '2', 'X', 'X', 'O', 'X', '7', 'O', '9'},
        {'1', '2', 'X', 'X', 'O', '6', 'X', 'O', '9'},
        {'1', '2', 'X', 'X', 'O', '6', '7', 'O', 'X'},
        {'1', '2', 'X', 'X', 'O', '6', '7', 'O', '9'},
        {'1', '2', 'X', '4', 'O', 'X', 'X', 'O', '9'},
        {'1', '2', 'X', '4', 'O', '6', 'X', 'O', 'X'},
        {'1', '2', 'X', '4', 'O', '6', 'X', 'O', '9'},
        {'1', '2', 'X', '4', 'O', '6', 'X', '8', '9'},
        {'1', '2', 'X', 'X', 'X', 'O', 'O', 'X', 'O'},
        {'1', '2', 'X', 'X', '5', 'O', 'O', 'X', 'O'},
        {'1', '2', 'X', '4', 'X', 'O', 'O', 'X', 'O'},
        {'1', '2', 'X', '4', 'X', 'O', 'O', 'X', '9'},
        {'1', '2', 'X', '4', '5', 'O', 'O', 'X', '9'},
        {'1', '2', 'X', 'X', '5', '6', 'O', 'X', 'O'},
        {'1', '2', 'X', '4', 'X', '6', 'O', 'X', 'O'},
        {'1', '2', 'X', '4', '5', '6', 'O', 'X', 'O'},
        {'1', '2', 'X', '4', '5', '6', '7', 'X', 'O'},
        {'O', '2', 'O', 'X', 'X', 'O', 'X', 'X', '9'},
        {'O', '2', 'O', 'X', 'X', 'O', 'X', 'O', 'X'},
        {'O', '2', 'O', 'X', 'X', 'O', 'X', '8', 'X'},
        {'O', '2', 'O', 'X', 'X', 'O', 'X', '8', '9'},
        {'O', '2', 'O', 'X', 'X', '6', 'X', 'O', 'X'},
        {'O', '2', 'O', 'X', 'X', '6', 'X', 'X', 'O'},
        {'O', '2', 'O', 'X', 'X', '6', 'X', '8', '9'},
        {'O', '2', 'O', 'X', 'X', 'O', 'O', 'X', 'X'},
        {'O', '2', 'O', 'X', 'X', 'O', '7', 'X', 'X'},
        {'O', '2', 'O', 'X', 'X', 'O', '7', 'X', '9'},
        {'O', '2', 'O', 'X', 'X', '6', 'O', 'X', 'X'},
        {'O', '2', 'O', 'X', 'X', '6', 'O', 'X', '9'},
        {'O', '2', 'O', 'X', 'X', '6', '7', 'X', 'O'},
        {'O', '2', 'O', 'X', 'X', '6', '7', 'X', '9'},
        {'O', '2', 'O', 'X', 'X', 'O', '7', '8', 'X'},
        {'O', '2', 'O', 'X', 'X', '6', '7', '8', 'X'},
        {'O', '2', '3', 'X', 'X', 'O', 'O', 'X', 'X'},
        {'O', '2', '3', 'X', 'X', 'O', 'O', 'X', '9'},
        {'O', '2', '3', 'X', 'X', 'O', '7', 'X', 'O'},
        {'O', '2', '3', 'X', 'X', 'O', '7', 'X', '9'},
        {'O', '2', '3', 'X', 'X', 'O', 'O', '8', 'X'},
        {'O', '2', '3', 'X', 'X', 'O', '7', 'O', 'X'},
        {'O', '2', '3', 'X', 'X', 'O', '7', '8', 'X'},
        {'O', '2', '3', 'X', 'X', 'O', '7', '8', '9'},
        {'O', '2', '3', 'X', 'X', '6', 'O', 'X', 'O'},
        {'O', '2', '3', 'X', 'X', '6', 'O', 'X', '9'},
        {'O', '2', '3', 'X', 'X', '6', 'X', 'O', '9'},
        {'O', '2', '3', 'X', 'X', '6', 'X', '8', 'O'},
        {'O', '2', '3', 'X', 'X', '6', '7', 'X', 'O'},
        {'O', '2', 'O', 'X', 'O', 'X', 'X', 'X', '9'},
        {'O', '2', 'O', 'X', 'O', 'X', 'X', 'O', 'X'},
        {'O', '2', 'O', 'X', 'O', 'X', 'X', '8', 'X'},
        {'O', '2', 'O', 'X', 'O', 'X', 'X', '8', '9'},
        {'O', '2', 'O', 'X', '5', 'X', 'X', 'O', 'X'},
        {'O', '2', 'O', 'X', '5', 'X', 'X', 'X', 'O'},
        {'O', '2', 'O', 'X', '5', 'X', 'X', '8', '9'},
        {'O', '2', 'O', 'X', 'O', 'X', '7', 'X', 'X'},
        {'O', '2', 'O', 'X', 'O', 'X', '7', 'X', '9'},
        {'O', '2', 'O', 'X', '5', 'X', 'O', 'X', 'X'},
        {'O', '2', 'O', 'X', '5', 'X', '7', 'X', '9'},
        {'O', '2', 'O', 'X', 'O', 'X', '7', '8', 'X'},
        {'O', '2', 'O', 'X', '5', 'X', '7', '8', 'X'},
        {'O', '2', '3', 'X', 'O', 'X', 'X', 'O', 'X'},
        {'O', '2', '3', 'X', 'O', 'X', 'X', 'O', '9'},
        {'O', '2', '3', 'X', 'O', 'X', 'O', 'X', '9'},
        {'O', '2', '3', 'X', 'O', 'X', '7', '8', '9'},
        {'O', '2', '3', 'X', '5', 'X', 'O', '8', 'X'},
        {'O', '2', '3', 'X', '5', 'X', '7', 'O', 'X'},
        {'O', '2', 'O', 'X', '5', '6', 'X', 'X', 'O'},
        {'O', '2', 'O', 'X', '5', '6', 'X', 'X', '9'},
        {'O', '2', 'O', 'X', '5', '6', 'X', 'O', 'X'},
        {'O', '2', 'O', 'X', '5', '6', 'X', '8', 'X'},
        {'O', '2', 'O', 'X', '5', '6', 'X', '8', '9'},
        {'O', '2', '3', 'X', 'O', '6', 'X', 'O', 'X'},
        {'O', '2', '3', 'X', '5', 'O', 'X', 'X', 'O'},
        {'O', '2', '3', 'X', '5', 'O', 'X', 'O', 'X'},
        {'O', '2', '3', 'X', '5', '6', 'X', 'O', 'X'},
        {'O', '2', '3', 'X', '5', '6', 'X', '8', '9'},
        {'O', '2', 'O', 'X', '5', '6', 'O', 'X', 'X'},
        {'O', '2', 'O', 'X', '5', '6', '7', 'X', 'X'},
        {'O', '2', 'O', 'X', '5', '6', '7', 'X', '9'},
        {'O', '2', '3', 'X', '5', 'O', 'O', 'X', 'X'},
        {'O', '2', '3', 'X', '5', 'O', '7', 'X', '9'},
        {'O', '2', 'O', 'X', '5', '6', '7', '8', 'X'},
        {'O', '2', '3', 'X', 'O', '6', '7', '8', 'X'},
        {'O', '2', '3', 'X', '5', 'O', '7', '8', 'X'},
        {'O', '2', '3', 'X', '5', '6', '7', '8', '9'},
        {'1', '2', 'O', 'X', 'X', 'O', 'O', 'X', '9'},
        {'1', '2', 'O', 'X', 'X', '6', 'O', 'X', 'O'},
        {'1', '2', 'O', 'X', 'O', 'X', 'X', 'O', 'X'},
        {'1', '2', '3', 'X', 'O', 'X', 'X', 'O', '9'},
        {'1', '2', '3', 'X', 'O', 'X', '7', 'O', 'X'},
        {'1', '2', '3', 'X', 'O', '6', 'X', 'O', 'X'},
        {'1', '2', '3', 'X', 'O', '6', '7', 'O', 'X'},
        {'1', '2', '3', 'X', 'X', 'O', 'O', 'X', 'O'},
        {'1', '2', '3', 'X', 'X', 'O', 'O', 'X', '9'},
        {'1', '2', '3', 'X', 'X', 'O', 'O', '8', '9'},
        {'1', '2', '3', 'X', '5', 'O', 'O', 'X', '9'},
        {'1', '2', '3', 'X', '5', '6', 'O', 'X', '9'},
        {'1', '2', '3', 'X', '5', '6', 'O', 'O', 'X'},
        {'O', '2', 'O', 'O', 'X', 'X', 'X', 'X', 'O'},
        {'O', '2', 'O', 'O', 'X', 'X', 'X', 'X', '9'},
        {'O', '2', 'O', 'O', 'X', 'X', 'X', 'O', 'X'},
        {'O', '2', 'O', 'O', 'X', 'X', 'X', '8', 'X'},
        {'O', '2', 'O', 'O', 'X', 'X', 'X', '8', '9'},
        {'O', '2', 'O', '4', 'X', 'X', 'X', 'O', 'X'},
        {'O', '2', 'O', '4', 'X', 'X', 'X', 'X', 'O'},
        {'O', '2', 'O', '4', 'X', 'X', 'X', '8', '9'},
        {'O', '2', 'O', 'O', 'X', 'X', '7', 'X', 'X'},
        {'O', '2', 'O', 'O', 'X', 'X', '7', 'X', '9'},
        {'O', '2', 'O', '4', 'X', 'X', 'O', 'X', 'X'},
        {'O', '2', 'O', '4', 'X', 'X', 'O', 'X', '9'},
        {'O', '2', 'O', '4', 'X', 'X', '7', 'X', 'O'},
        {'O', '2', 'O', '4', 'X', 'X', '7', 'X', '9'},
        {'O', '2', 'O', 'O', 'X', 'X', '7', '8', 'X'},
        {'O', '2', 'O', '4', 'X', 'X', '7', '8', 'X'},
        {'O', '2', '3', 'O', 'X', 'X', 'X', 'X', 'O'},
        {'O', '2', '3', 'O', 'X', 'X', '7', 'X', 'O'},
        {'O', '2', '3', '4', 'X', 'X', 'O', 'X', 'O'},
        {'O', '2', '3', '4', 'X', 'X', 'X', 'O', '9'},
        {'O', '2', '3', '4', 'X', 'X', '7', 'O', 'X'},
        {'O', '2', '3', '4', 'X', 'X', 'X', '8', 'O'},
        {'O', '2', '3', '4', 'X', 'X', '7', 'X', 'O'},
        {'O', '2', 'O', 'O', 'X', '6', 'X', 'X', '9'},
        {'O', '2', 'O', '4', 'X', 'O', 'X', 'X', '9'},
        {'O', '2', 'O', '4', 'X', '6', 'X', 'X', 'O'},
        {'O', '2', 'O', '4', 'X', '6', 'X', 'X', '9'},
        {'O', '2', 'O', '4', 'X', '6', 'X', 'O', 'X'},
        {'O', '2', 'O', '4', 'X', '6', 'X', '8', 'X'},
        {'O', '2', 'O', '4', 'X', '6', 'X', '8', '9'},
        {'O', '2', '3', 'O', 'X', 'O', 'X', 'X', '9'},
        {'O', '2', '3', 'O', 'X', '6', 'X', 'X', 'O'},
        {'O', '2', '3', 'O', 'X', '6', 'X', 'X', '9'},
        {'O', '2', '3', 'O', 'X', '6', 'X', '8', 'X'},
        {'O', '2', '3', '4', 'X', 'O', 'X', 'X', 'O'},
        {'O', '2', '3', '4', 'X', 'O', 'X', 'X', '9'},
        {'O', '2', '3', '4', 'X', 'O', 'X', '8', 'X'},
        {'O', '2', '3', '4', 'X', '6', 'X', 'X', 'O'},
        {'O', '2', 'O', 'O', 'X', '6', '7', 'X', 'X'},
        {'O', '2', 'O', '4', 'X', 'O', '7', 'X', 'X'},
        {'O', '2', 'O', '4', 'X', '6', 'O', 'X', 'X'},
        {'O', '2', 'O', '4', 'X', '6', '7', 'X', 'X'},
        {'O', '2', 'O', '4', 'X', '6', '7', 'X', '9'},
        {'O', '2', '3', 'O', 'X', 'O', '7', 'X', 'X'},
        {'O', '2', '3', 'O', 'X', '6', '7', 'X', '9'},
        {'O', '2', '3', '4', 'X', 'O', 'O', 'X', 'X'},
        {'O', '2', '3', '4', 'X', 'O', '7', 'X', 'X'},
        {'O', '2', '3', '4', 'X', 'O', '7', 'X', '9'},
        {'O', '2', '3', '4', 'X', '6', 'O', 'X', '9'},
        {'O', '2', '3', '4', 'X', '6', '7', 'X', 'O'},
        {'O', '2', '3', '4', 'X', '6', '7', 'X', '9'},
        {'O', '2', 'O', '4', 'X', '6', '7', '8', 'X'},
        {'O', '2', '3', '4', 'X', '6', '7', '8', '9'},
        {'1', '2', 'O', 'O', 'X', 'X', 'X', 'X', 'O'},
        {'1', '2', 'O', 'O', 'X', 'X', 'O', 'X', '9'},
        {'1', '2', 'O', 'O', 'X', 'X', '7', 'X', 'O'},
        {'1', '2', 'O', 'O', 'X', 'X', '7', 'X', '9'},
        {'1', '2', 'O', '4', 'X', 'X', 'O', 'X', 'O'},
        {'1', '2', 'O', 'O', 'X', 'O', 'X', 'X', '9'},
        {'1', '2', 'O', 'O', 'X', '6', 'X', 'X', 'O'},
        {'1', '2', 'O', 'O', 'X', '6', '7', 'X', '9'},
        {'1', '2', 'O', '4', 'X', 'O', '7', 'X', '9'},
        {'1', '2', 'O', '4', 'X', '6', 'O', 'X', '9'},
        {'1', '2', 'O', '4', 'X', '6', '7', 'X', 'O'},
        {'1', '2', 'O', '4', 'X', '6', '7', 'X', '9'},
        {'1', '2', '3', 'O', 'X', 'X', 'O', 'X', 'O'},
        {'1', '2', '3', 'O', 'X', 'X', '7', 'X', 'O'},
        {'1', '2', '3', 'O', 'X', 'O', 'X', 'X', 'O'},
        {'1', '2', '3', 'O', 'X', 'O', '7', 'X', '9'},
        {'1', '2', '3', 'O', 'X', '6', 'O', 'X', '9'},
        {'1', '2', '3', 'O', 'X', '6', '7', 'X', 'O'},
        {'1', '2', '3', '4', 'X', 'O', 'O', 'X', '9'},
        {'1', '2', '3', '4', 'X', 'O', '7', 'X', 'O'},
        {'1', '2', '3', '4', 'X', '6', 'O', 'X', 'O'},
        {'1', '2', '3', '4', 'X', '6', 'O', 'X', '9'},
        {'1', '2', '3', '4', 'X', '6', '7', 'X', 'O'},
        {'O', '2', 'O', '4', '5', 'X', 'X', 'X', 'O'},
        {'O', '2', 'O', '4', '5', 'X', 'X', 'X', '9'},
        {'O', '2', 'O', '4', '5', 'X', 'X', 'O', 'X'},
        {'O', '2', 'O', '4', '5', 'X', 'X', '8', 'X'},
        {'O', '2', 'O', '4', '5', 'X', 'X', '8', '9'},
        {'O', '2', '3', 'O', '5', 'X', 'X', 'X', '9'},
        {'O', '2', '3', 'O', '5', 'X', 'X', '8', 'X'},
        {'O', '2', '3', 'O', '5', 'X', 'X', '8', '9'},
        {'O', '2', '3', '4', 'O', 'X', 'X', '8', 'X'},
        {'O', '2', 'O', '4', '5', 'X', 'O', 'X', 'X'},
        {'O', '2', 'O', '4', '5', 'X', '7', 'X', 'X'},
        {'O', '2', 'O', '4', '5', 'X', '7', 'X', '9'},
        {'O', '2', '3', '4', 'O', 'X', '7', 'X', 'X'},
        {'O', '2', '3', '4', '5', 'X', 'O', 'X', '9'},
        {'O', '2', 'O', '4', '5', 'X', '7', '8', 'X'},
        {'1', '2', 'O', '4', 'O', 'X', 'X', 'O', 'X'},
        {'1', '2', 'O', '4', '5', 'X', 'X', 'O', 'X'},
        {'1', '2', '3', 'O', '5', 'X', 'X', 'X', 'O'},
        {'1', '2', '3', '4', 'O', 'X', 'X', 'O', 'X'},
        {'1', '2', '3', '4', 'O', 'X', 'X', 'O', '9'},
        {'1', '2', '3', '4', 'O', 'X', 'X', '8', '9'},
        {'1', '2', '3', '4', '5', 'X', '7', 'X', 'O'},
        {'O', '2', '3', '4', '5', '6', 'X', 'X', '9'},
        {'O', '2', '3', '4', '5', '6', 'X', '8', 'X'},
        {'1', '2', '3', '4', 'O', '6', 'X', 'O', 'X'},
        {'O', '2', '3', '4', '5', '6', 'O', 'X', 'X'},
        {'1', '2', '3', '4', '5', '6', '7', 'X', '9'}
    },
    {
        {'X', 'O', '3', 'X', 'O', 'X', 'O', 'X', 'O'},
        {'X', 'O', '3', 'X', 'O', 'X', 'O', 'X', '9'},
        {'X', 'O', '3', 'X', 'O', 'X', 'O', '8', 'X'},
        {'X', 'O', '3', 'X', 'O', 'X', 'O', '8', '9'},
        {'X', 'O', '3', 'X', 'O', 'O', 'O', 'X', 'X'},
        {'X', 'O', '3', 'X', 'O', '6', 'O', 'X', 'X'},
        {'X', 'O', '3', 'X', 'O', '6', 'O', 'X', '9'},
        {'X', 'O', '3', 'X', 'O', '6', 'O', '8', 'X'},
        {'X', 'O', '3', 'X', 'X', 'O', 'O', 'X', 'O'},
        {'X', 'O', '3', 'X', 'X', 'O', '7', 'X', 'O'},
        {'X', 'O', '3', 'X', 'X', 'O', '7', '8', '9'},
        {'X', 'O', '3', 'X', '5', 'O', 'O', 'X', '9'},
        {'X', 'O', '3', 'X', '5', 'O', '7', '8', 'X'},
        {'X', 'O', '3', 'X', 'X', '6', 'O', '8', '9'},
        {'X', 'O', '3', 'X', '5', '6', 'O', 'X', 'O'},
        {'X', 'O', '3', 'X', 'X', '6', '7', 'O', '9'},
        {'X', 'O', '3', 'X', 'X', '6', '7', '8', 'O'},
        {'X', 'O', '3', 'X', '5', 'X', '7', '8', 'O'},
        {'X', 'O', '3', 'X', '5', '6', '7', '8', '9'},
        {'X', 'O', '3', 'O', 'X', 'X', 'O', 'X', 'O'},
        {'X', 'O', '3', 'O', 'X', 'X', 'X', 'O', 'O'},
        {'X', 'O', '3', 'O', 'X', 'X', 'X', 'O', '9'},
        {'X', 'O', '3', 'O', 'X', 'X', 'X', '8', 'O'},
        {'X', 'O', '3', 'O', 'X', 'X', '7', 'X', 'O'},
        {'X', 'O', '3', 'O', 'X', 'X', '7', '8', 'O'},
        {'X', 'O', '3', 'O', 'X', 'O', 'X', 'X', 'O'},
        {'X', 'O', '3', 'O', 'X', 'O', 'X', 'X', '9'},
        {'X', 'O', '3', 'O', 'X', 'O', 'X', '8', '9'},
        {'X', 'O', '3', 'O', 'X', '6', 'X', 'O', '9'},
        {'X', 'O', '3', 'O', 'X', '6', 'X', 'X', 'O'},
        {'X', 'O', '3', 'O', 'X', '6', 'X', '8', 'O'},
        {'X', 'O', '3', 'O', 'X', '6', 'X', '8', '9'},
        {'X', 'O', '3', 'O', 'X', '6', '7', 'X', 'O'},
        {'X', 'O', '3', '4', 'X', 'O', 'X', 'O', '9'},
        {'X', 'O', '3', '4', 'X', 'O', 'X', 'X', 'O'},
        {'X', 'O', '3', '4', 'X', 'O', 'X', '8', 'O'},
        {'X', 'O', '3', '4', 'X', 'O', 'X', '8', '9'},
        {'X', 'O', '3', '4', 'X', 'O', '7', 'X', 'O'},
        {'X', 'O', '3', '4', 'X', 'X', 'O', '8', '9'},
        {'X', 'O', '3', '4', 'X', '6', 'O', 'X', 'O'},
        {'X', 'O', '3', '4', 'X', 'X', 'X', 'O', 'O'},
        {'X', 'O', '3', '4', 'X', 'X', '7', 'O', '9'},
        {'X', 'O', '3', '4', 'X', '6', 'X', 'O', 'O'},
        {'X', 'O', '3', '4', 'X', '6', 'X', 'O', '9'},
        {'X', 'O', '3', '4', 'X', '6', 'X', '8', 'O'},
        {'X', 'O', '3', '4', 'X', '6', '7', 'X', 'O'},
        {'X', 'O', '3', '4', 'X', '6', '7', '8', '9'},
        {'X', 'O', '3', 'O', 'O', 'X', 'X', 'X', 'O'},
        {'X', 'O', '3', 'O', '5', 'X', 'X', 'X', 'O'},
        {'X', 'O', '3', 'O', '5', 'X', 'X', '8', 'O'},
        {'X', 'O', '3', 'O', 'O', 'X', 'O', 'X', 'X'},
        {'X', 'O', '3', 'O', 'O', 'X', '7', 'X', 'X'},
        {'X', 'O', '3', 'O', '5', 'X', 'O', 'X', 'X'},
        {'X', 'O', '3', 'O', '5', 'X', '7', 'X', 'O'},
        {'X', 'O', '3', 'O', 'O', 'X', '7', '8', 'X'},
        {'X', 'O', '3', 'O', '5', 'X', 'O', '8', 'X'},
        {'X', 'O', '3', 'O', '5', 'X', '7', 'O', 'X'},
        {'X', 'O', '3', 'O', '5', 'X', '7', '8', 'X'},
        {'X', 'O', '3', '4', 'O', 'X', 'O', 'X', 'X'},
        {'X', 'O', '3', '4', 'O', 'X', 'O', 'X', '9'},
        {'X', 'O', '3', '4', 'O', 'X', '7', 'X', 'O'},
        {'X', 'O', '3', '4', 'O', 'X', 'O', '8', 'X'},
        {'X', 'O', '3', '4', '5', 'X', 'O', 'X', 'O'},
        {'X', 'O', '3', '4', '5', 'X', 'O', 'O', 'X'},
        {'X', 'O', '3', '4', '5', 'X', 'O', '8', 'X'},
        {'X', 'O', '3', 'O', '5', '6', 'X', 'X', 'O'},
        {'X', 'O', '3', 'O', '5', '6', 'X', '8', 'X'},
        {'X', 'O', '3', '4', 'O', '6', 'X', 'X', '9'},
        {'X', 'O', '3', '4', '5', 'O', 'X', 'X', '9'},
        {'X', 'O', '3', '4', '5', 'O', 'X', '8', 'X'},
        {'X', 'O', '3', '4', '5', '6', 'X', '8', '9'},
        {'X', 'O', '3', 'O', '5', '6', '7', 'X', 'X'},
        {'X', 'O', '3', '4', 'O', '6', 'O', 'X', 'X'},
        {'X', 'O', '3', '4', '5', 'O', '7', 'X', 'X'},
        {'X', 'O', '3', '4', '5', '6', 'O', 'X', '9'},
        {'X', 'O', '3', '4', '5', '6', '7', 'X', 'O'},
        {'X', 'X', '3', 'O', 'O', 'X', 'O', 'X', 'O'},
        {'X', 'X', '3', 'O', 'O', 'X', 'O', 'X', '9'},
        {'X', 'X', '3', 'O', 'O', 'X', 'O', 'O', 'X'},
        {'X', 'X', '3', 'O', 'O', 'X', 'O', '8', 'X'},
        {'X', 'X', '3', 'O', 'O', 'X', 'O', '8', '9'},
        {'X', 'X', '3', 'O', 'O', 'X', 'X', 'O', 'O'},
        {'X', 'X', '3', 'O', 'O', 'X', 'X', 'O', '9'},
        {'X', 'X', '3', 'O', 'O', 'X', '7', 'O', 'X'},
        {'X', 'X', '3', 'O', 'O', 'X', '7', 'O', '9'},
        {'X', 'X', '3', 'O', 'O', 'X', 'X', '8', 'O'},
        {'X', 'X', '3', 'O', 'O', 'X', '7', 'X', 'O'},
        {'X', 'X', '3', 'O', 'O', 'X', '7', '8', 'O'},
        {'X', 'X', '3', 'O', 'O', 'X', '7', '8', '9'},
        {'X', 'X', '3', 'O', 'O', '6', 'X', 'O', '9'},
        {'X', 'X', '3', 'O', 'O', '6', 'X', '8', 'O'},
        {'X', 'X', '3', 'O', 'O', '6', 'O', 'X', 'X'},
        {'X', 'X', '3', 'O', 'O', '6', 'O', 'X', '9'},
        {'X', 'X', '3', 'O', 'O', '6', '7', 'X', 'O'},
        {'X', 'X', '3', 'O', 'O', '6', 'O', '8', 'X'},
        {'X', 'X', '3', 'O', 'O', '6', '7', 'O', 'X'},
        {'X', 'X', '3', 'O', 'O', '6', '7', '8', '9'},
        {'X', 'X', '3', 'O', 'X', 'O', 'O', '8', '9'},
        {'X', 'X', '3', 'O', 'X', 'O', 'X', 'O', 'O'},
        {'X', 'X', '3', 'O', 'X', 'O', 'X', 'O', '9'},
        {'X', 'X', '3', 'O', 'X', 'O', '7', 'O', '9'},
        {'X', 'X', '3', 'O', 'X', 'O', 'X', '8', 'O'},
        {'X', 'X', '3', 'O', 'X', 'O', '7', '8', 'O'},
        {'X', 'X', '3', 'O', 'X', 'O', '7', '8', '9'},
        {'X', 'X', '3', 'O', '5', 'O', 'X', 'O', '9'},
        {'X', 'X', '3', 'O', '5', 'O', 'X', 'X', 'O'},
        {'X', 'X', '3', 'O', '5', 'O', 'X', '8', 'O'},
        {'X', 'X', '3', 'O', '5', 'O', 'O', 'X', '9'},
        {'X', 'X', '3', 'O', '5', 'O', '7', 'X', 'O'},
        {'X', 'X', '3', 'O', '5', 'O', 'O', '8', 'X'},
        {'X', 'X', '3', 'O', '5', 'O', '7', 'O', 'X'},
        {'X', 'X', '3', 'O', '5', 'O', '7', '8', '9'},
        {'X', 'X', '3', 'O', 'X', '6', 'O', 'O', '9'},
        {'X', 'X', '3', 'O', 'X', '6', 'O', '8', 'O'},
        {'X', 'X', '3', 'O', 'X', '6', 'O', '8', '9'},
        {'X', 'X', '3', 'O', '5', 'X', 'O', 'O', 'X'},
        {'X', 'X', '3', 'O', '5', 'X', 'O', 'O', '9'},
        {'X', 'X', '3', 'O', '5', 'X', 'O', 'X', 'O'},
        {'X', 'X', '3', 'O', '5', 'X', 'O', '8', 'O'},
        {'X', 'X', '3', 'O', '5', 'X', 'O', '8', '9'},
        {'X', 'X', '3', 'O', '5', '6', 'O', 'X', 'O'},
        {'X', 'X', '3', 'O', '5', '6', 'O', 'X', '9'},
        {'X', 'X', '3', 'O', '5', '6', 'O', 'O', 'X'},
        {'X', 'X', '3', 'O', '5', '6', 'O', '8', 'X'},
        {'X', 'X', '3', 'O', '5', '6', 'O', '8', '9'},
        {'X', 'X', '3', 'O', 'X', '6', 'X', 'O', 'O'},
        {'X', 'X', '3', 'O', 'X', '6', '7', 'O', 'O'},
        {'X', 'X', '3', 'O', 'X', '6', '7', 'O', '9'},
        {'X', 'X', '3', 'O', '5', 'X', 'X', 'O', 'O'},
        {'X', 'X', '3', 'O', '5', 'X', '7', 'O', 'O'},
        {'X', 'X', '3', 'O', '5', 'X', '7', 'O', '9'},
        {'X', 'X', '3', 'O', '5', '6', 'X', 'O', 'O'},
        {'X', 'X', '3', 'O', '5', '6', 'X', 'O', '9'},
        {'X', 'X', '3', 'O', '5', '6', '7', 'O', 'X'},
        {'X', 'X', '3', 'O', '5', '6', '7', 'O', '9'},
        {'X', 'X', '3', 'O', 'X', '6', '7', '8', 'O'},
        {'X', 'X', '3', 'O', '5', 'X', '7', '8', 'O'},
        {'X', 'X', '3', 'O', '5', '6', 'X', '8', 'O'},
        {'X', 'X', '3', 'O', '5', '6', '7', 'X', 'O'},
        {'X', 'X', '3', 'O', '5', '6', '7', '8', 'O'},
        {'X', 'X', '3', 'O', '5', '6', '7', '8', '9'},
        {'X', '2', '3', 'O', 'X', 'O', 'X', 'O', '9'},
        {'X', '2', '3', 'O', 'X', 'O', 'X', 'X', 'O'},
        {'X', '2', '3', 'O', 'X', 'O', 'X', '8', 'O'},
        {'X', '2', '3', 'O', 'X', 'X', 'X', 'O', 'O'},
        {'X', '2', '3', 'O', 'X', '6', 'X', 'O', 'O'},
        {'X', '2', '3', 'O', 'X', '6', 'X', '8', 'O'},
        {'X', '2', '3', 'O', 'O', 'X', 'O', 'X', 'X'},
        {'X', '2', '3', 'O', 'O', 'X', 'O', 'X', '9'},
        {'X', '2', '3', 'O', 'O', 'X', '7', 'X', '9'},
        {'X', '2', '3', 'O', 'O', 'X', 'O', '8', 'X'},
        {'X', '2', '3', 'O', 'O', 'X', '7', 'O', 'X'},
        {'X', '2', '3', 'O', 'O', 'X', '7', '8', 'X'},
        {'X', '2', '3', 'O', 'O', 'X', '7', '8', '9'},
        {'X', '2', '3', 'O', '5', 'X', 'O', 'O', 'X'},
        {'X', '2', '3', 'O', '5', 'X', 'X', 'O', 'O'},
        {'X', '2', '3', 'O', '5', 'X', 'X', 'O', '9'},
        {'X', '2', '3', 'O', '5', 'X', '7', 'O', '9'},
        {'X', '2', '3', 'O', '5', 'X', '7', '8', '9'},
        {'X', '2', '3', 'O', '5', '6', 'X', 'O', '9'},
        {'X', '2', '3', 'O', '5', '6', 'X', '8', 'O'},
        {'X', 'X', '3', 'X', 'O', 'O', 'O', 'X', 'O'},
        {'X', 'X', '3', 'X', 'O', 'O', 'O', 'X', '9'},
        {'X', 'X', '3', 'X', 'O', 'O', 'O', 'O', 'X'},
        {'X', 'X', '3', 'X', 'O', 'O', 'O', '8', 'X'},
        {'X', 'X', '3', 'X', 'O', 'O', 'O', '8', '9'},
        {'X', 'X', '3', 'X', 'O', 'O', '7', 'O', 'X'},
        {'X', 'X', '3', 'X', 'O', 'O', '7', 'O', '9'},
        {'X', 'X', '3', 'X', 'O', 'O', '7', 'X', 'O'},
        {'X', 'X', '3', 'X', 'O', 'O', '7', '8', 'O'},
        {'X', 'X', '3', 'X', 'O', 'O', '7', '8', '9'},
        {'X', 'X', '3', '4', 'O', 'O', 'X', 'O', '9'},
        {'X', 'X', '3', '4', 'O', 'O', 'X', 'X', 'O'},
        {'X', 'X', '3', '4', 'O', 'O', 'X', '8', 'O'},
        {'X', 'X', '3', '4', 'O', 'O', 'O', 'X', 'X'},
        {'X', 'X', '3', '4', 'O', 'O', 'O', 'X', '9'},
        {'X', 'X', '3', '4', 'O', 'O', '7', 'X', 'O'},
        {'X', 'X', '3', '4', 'O', 'O', 'O', '8', 'X'},
        {'X', 'X', '3', '4', 'O', 'O', '7', 'O', 'X'},
        {'X', 'X', '3', '4', 'O', 'O', '7', '8', '9'},
        {'X', 'X', '3', 'X', 'O', 'X', 'O', 'O', '9'},
        {'X', 'X', '3', 'X', 'O', '6', 'O', 'O', 'X'},
        {'X', 'X', '3', 'X', 'O', '6', 'O', 'O', '9'},
        {'X', 'X', '3', 'X', 'O', 'X', 'O', '8', 'O'},
        {'X', 'X', '3', 'X', 'O', '6', 'O', 'X', 'O'},
        {'X', 'X', '3', 'X', 'O', '6', 'O', '8', 'O'},
        {'X', 'X', '3', 'X', 'O', '6', 'O', '8', '9'},
        {'X', 'X', '3', '4', 'O', 'X', 'O', 'O', 'X'},
        {'X', 'X', '3', '4', 'O', 'X', 'O', 'O', '9'},
        {'X', 'X', '3', '4', 'O', 'X', 'O', 'X', 'O'},
        {'X', 'X', '3', '4', 'O', 'X', 'O', '8', 'O'},
        {'X', 'X', '3', '4', 'O', 'X', 'O', '8', '9'},
        {'X', 'X', '3', '4', 'O', '6', 'O', 'X', 'O'},
        {'X', 'X', '3', '4', 'O', '6', 'O', 'X', '9'},
        {'X', 'X', '3', '4', 'O', '6', 'O', 'O', 'X'},
        {'X', 'X', '3', '4', 'O', '6', 'O', '8', 'X'},
        {'X', 'X', '3', '4', 'O', '6', 'O', '8', '9'},
        {'X', 'X', '3', 'X', 'O', '6', '7', 'O', 'O'},
        {'X', 'X', '3', 'X', 'O', '6', '7', 'O', '9'},
        {'X', 'X', '3', '4', 'O', 'X', 'X', 'O', 'O'},
        {'X', 'X', '3', '4', 'O', 'X', '7', 'O', 'O'},
        {'X', 'X', '3', '4', 'O', 'X', '7', 'O', '9'},
        {'X', 'X', '3', '4', 'O', '6', 'X', 'O', 'O'},
        {'X', 'X', '3', '4', 'O', '6', 'X', 'O', '9'},
        {'X', 'X', '3', '4', 'O', '6', '7', 'O', 'X'},
        {'X', 'X', '3', '4', 'O', '6', '7', 'O', '9'},
        {'X', 'X', '3', 'X', 'O', '6', '7', '8', 'O'},
        {'X', 'X', '3', '4', 'O', 'X', '7', '8', 'O'},
        {'X', 'X', '3', '4', 'O', '6', 'X', '8', 'O'},
        {'X', 'X', '3', '4', 'O', '6', '7', 'X', 'O'},
        {'X', 'X', '3', '4', 'O', '6', '7', '8', 'O'},
        {'X', 'X', '3', '4', 'O', '6', '7', '8', '9'},
        {'X', '2', '3', 'X', 'O', 'O', 'O', 'X', 'X'},
        {'X', '2', '3', 'X', 'O', 'O', 'O', 'X', '9'},
        {'X', '2', '3', 'X', 'O', 'O', 'O', '8', 'X'},
        {'X', '2', '3', 'X', 'O', 'X', 'O', 'X', 'O'},
        {'X', '2', '3', 'X', 'O', 'X', 'O', '8', '9'},
        {'X', '2', '3', 'X', 'O', '6', 'O', 'X', 'O'},
        {'X', '2', '3', 'X', 'O', '6', 'O', 'X', '9'},
        {'X', '2', '3', 'X', 'O', '6', 'O', '8', 'X'},
        {'X', '2', '3', 'X', 'O', '6', 'O', '8', '9'},
        {'X', '2', '3', '4', 'O', 'X', 'O', 'X', 'O'},
        {'X', '2', '3', '4', 'O', 'X', 'O', 'X', '9'},
        {'X', '2', '3', '4', 'O', 'X', 'O', 'O', 'X'},
        {'X', '2', '3', '4', 'O', 'X', 'O', '8', 'X'},
        {'X', '2', '3', '4', 'O', 'X', 'O', '8', '9'},
        {'X', '2', '3', '4', 'O', '6', 'O', 'X', 'X'},
        {'X', '2', '3', '4', 'O', '6', 'O', 'X', '9'},
        {'X', '2', '3', '4', 'O', '6', '7', 'X', 'O'},
        {'X', '2', '3', '4', 'O', '6', 'O', '8', 'X'},
        {'X', 'X', '3', 'X', '5', 'O', 'O', 'O', 'X'},
        {'X', 'X', '3', 'X', '5', 'O', 'O', 'O', '9'},
        {'X', 'X', '3', 'X', 'X', 'O', 'O', '8', 'O'},
        {'X', 'X', '3', 'X', '5', 'O', 'O', 'X', 'O'},
        {'X', 'X', '3', 'X', '5', 'O', 'O', '8', 'O'},
        {'X', 'X', '3', 'X', '5', 'O', 'O', '8', '9'},
        {'X', 'X', '3', '4', 'X', 'O', 'O', 'O', '9'},
        {'X', 'X', '3', '4', 'X', 'O', 'O', '8', 'O'},
        {'X', 'X', '3', '4', 'X', 'O', 'O', '8', '9'},
        {'X', 'X', '3', '4', '5', 'O', 'O', 'X', 'O'},
        {'X', 'X', '3', '4', '5', 'O', 'O', 'X', '9'},
        {'X', 'X', '3', '4', '5', 'O', 'O', 'O', 'X'},
        {'X', 'X', '3', '4', '5', 'O', 'O', '8', 'X'},
        {'X', 'X', '3', '4', '5', 'O', 'O', '8', '9'},
        {'X', 'X', '3', 'X', 'X', 'O', '7', 'O', 'O'},
        {'X', 'X', '3', 'X', '5', 'O', '7', 'O', 'O'},
        {'X', 'X', '3', 'X', '5', 'O', '7', 'O', '9'},
        {'X', 'X', '3', '4', 'X', 'O', 'X', 'O', 'O'},
        {'X', 'X', '3', '4', 'X', 'O', '7', 'O', 'O'},
        {'X', 'X', '3', '4', 'X', 'O', '7', 'O', '9'},
        {'X', 'X', '3', '4', '5', 'O', 'X', 'O', 'O'},
        {'X', 'X', '3', '4', '5', 'O', 'X', 'O', '9'},
        {'X', 'X', '3', '4', '5', 'O', '7', 'O', 'X'},
        {'X', 'X', '3', '4', '5', 'O', '7', 'O', '9'},
        {'X', 'X', '3', 'X', '5', 'O', '7', '8', 'O'},
        {'X', 'X', '3', '4', 'X', 'O', '7', '8', 'O'},
        {'X', 'X', '3', '4', '5', 'O', 'X', '8', 'O'},
        {'X', 'X', '3', '4', '5', 'O', '7', 'X', 'O'},
        {'X', 'X', '3', '4', '5', 'O', '7', '8', 'O'},
        {'X', 'X', '3', '4', '5', 'O', '7', '8', '9'},
        {'X', '2', '3', 'X', 'X', 'O', 'O', 'X', 'O'},
        {'X', '2', '3', 'X', '5', 'O', 'O', 'X', 'O'},
        {'X', '2', '3', 'X', '5', 'O', 'O', 'X', '9'},
        {'X', '2', '3', 'X', '5', 'O', 'O', '8', '9'},
        {'X', '2', '3', 'X', 'X', 'O', '7', '8', 'O'},
        {'X', '2', '3', 'X', '5', 'O', '7', 'X', 'O'},
        {'X', '2', '3', '4', 'X', 'O', 'X', 'O', 'O'},
        {'X', '2', '3', '4', 'X', 'O', 'X', '8', 'O'},
        {'X', '2', '3', '4', 'X', 'O', '7', 'X', 'O'},
        {'X', '2', '3', '4', 'X', 'O', '7', '8', 'O'},
        {'X', '2', '3', '4', '5', 'O', 'X', 'X', 'O'},
        {'X', '2', '3', '4', '5', 'O', '7', 'X', 'O'},
        {'X', '2', '3', '4', '5', 'O', '7', '8', '9'},
        {'X', 'X', '3', '4', '5', '6', 'O', 'O', 'X'},
        {'X', 'X', '3', '4', '5', '6', 'O', 'O', '9'},
        {'X', 'X', '3', '4', '5', '6', 'O', 'X', 'O'},
        {'X', 'X', '3', '4', '5', '6', 'O', '8', 'O'},
        {'X', 'X', '3', '4', '5', '6', 'O', '8', '9'},
        {'X', '2', '3', 'X', '5', '6', 'O', 'X', 'O'},
        {'X', 'X', '3', '4', '5', '6', 'X', 'O', 'O'},
        {'X', 'X', '3', '4', '5', '6', '7', 'O', 'O'},
        {'X', 'X', '3', '4', '5', '6', '7', 'O', '9'},
        {'X', '2', '3', '4', '5', '6', '7', 'O', '9'},
        {'X', 'X', '3', '4', '5', '6', '7', '8', 'O'},
        {'X', '2', '3', '4', 'X', '6', '7', '8', 'O'},
        {'X', '2', '3', '4', '5', '6', '7', '8', 'O'},
        {'O', 'X', '3', 'X', 'O', 'X', 'O', 'X', '9'},
        {'O', 'X', '3', 'X', 'O', 'X', 'O', 'O', 'X'},
        {'O', 'X', '3', 'X', 'O', 'X', 'O', '8', 'X'},
        {'O', 'X', '3', 'X', 'O', 'X', 'O', '8', '9'},
        {'O', 'X', '3', 'X', 'O', 'X', '7', 'O', 'X'},
        {'O', 'X', '3', 'X', 'O', 'O', 'X', 'O', 'X'},
        {'O', 'X', '3', 'X', 'O', '6', 'X', 'O', 'X'},
        {'O', 'X', '3', 'X', 'O', 'O', 'O', 'X', 'X'},
        {'O', 'X', '3', 'X', 'O', '6', 'O', 'X', 'X'},
        {'O', 'X', '3', 'X', 'O', '6', 'O', 'X', '9'},
        {'O', 'X', '3', 'X', 'O', 'O', '7', '8', 'X'},
        {'O', 'X', '3', 'X', 'O', '6', 'O', '8', 'X'},
        {'O', 'X', '3', 'X', 'O', '6', '7', 'O', 'X'},
        {'O', 'X', '3', 'X', 'O', '6', '7', '8', 'X'},
        {'O', 'X', '3', 'X', 'X', 'O', 'O', 'O', 'X'},
        {'O', 'X', '3', 'X', 'X', 'O', 'X', 'O', 'O'},
        {'O', 'X', '3', 'X', 'X', 'O', 'X', 'O', '9'},
        {'O', 'X', '3', 'X', 'X', 'O', '7', 'O', 'X'},
        {'O', 'X', '3', 'X', 'X', 'O', '7', 'O', '9'},
        {'O', 'X', '3', 'X', 'X', 'O', 'X', '8', 'O'},
        {'O', 'X', '3', 'X', '5', 'O', 'X', 'O', 'X'},
        {'O', 'X', '3', 'X', '5', 'O', 'X', 'O', '9'},
        {'O', 'X', '3', 'X', '5', 'O', 'X', 'X', 'O'},
        {'O', 'X', '3', 'X', '5', 'O', 'X', '8', 'O'},
        {'O', 'X', '3', 'X', '5', 'O', 'O', '8', 'X'},
        {'O', 'X', '3', 'X', '5', 'O', '7', 'O', 'X'},
        {'O', 'X', '3', 'X', '5', 'O', '7', '8', '9'},
        {'O', 'X', '3', 'X', 'X', '6', 'O', '8', '9'},
        {'O', 'X', '3', 'X', '5', 'X', 'O', 'O', 'X'},
        {'O', 'X', '3', 'X', 'X', '6', 'X', 'O', 'O'},
        {'O', 'X', '3', 'X', '5', '6', 'X', 'O', '9'},
        {'O', 'X', '3', 'X', '5', '6', '7', 'O', 'X'},
        {'O', 'X', '3', 'X', 'X', '6', '7', '8', 'O'},
        {'O', 'X', '3', 'O', 'X', 'X', 'X', 'O', 'O'},
        {'O', 'X', '3', 'O', 'X', 'X', 'X', 'O', '9'},
        {'O', 'X', '3', 'O', 'X', 'X', 'X', '8', 'O'},
        {'O', 'X', '3', 'O', 'X', 'O', 'X', 'O', 'X'},
        {'O', 'X', '3', 'O', 'X', 'O', 'X', '8', 'X'},
        {'O', 'X', '3', 'O', 'X', 'O', 'X', '8', '9'},
        {'O', 'X', '3', 'O', 'X', '6', 'X', 'O', 'X'},
        {'O', 'X', '3', 'O', 'X', '6', 'X', 'O', '9'},
        {'O', 'X', '3', 'O', 'X', '6', 'X', '8', 'O'},
        {'O', 'X', '3', 'O', 'X', '6', 'X', '8', '9'},
        {'O', 'X', '3', '4', 'X', 'O', 'X', 'O', 'X'},
        {'O', 'X', '3', '4', 'X', 'O', 'X', 'O', '9'},
        {'O', 'X', '3', '4', 'X', 'O', 'X', '8', 'O'},
        {'O', 'X', '3', '4', 'X', 'O', 'X', '8', '9'},
        {'O', 'X', '3', '4', 'X', 'O', '7', 'O', 'X'},
        {'O', 'X', '3', '4', 'X', 'X', 'X', 'O', 'O'},
        {'O', 'X', '3', '4', 'X', '6', 'X', 'O', 'O'},
        {'O', 'X', '3', '4', 'X', '6', 'X', 'O', '9'},
        {'O', 'X', '3', '4', 'X', '6', '7', 'O', 'X'},
        {'O', 'X', '3', '4', 'X', 'X', '7', '8', 'O'},
        {'O', 'X', '3', '4', 'X', '6', 'X', '8', 'O'},
        {'O', 'X', '3', 'O', 'O', 'X', 'X', 'O', 'X'},
        {'O', 'X', '3', 'O', 'O', 'X', 'X', '8', 'X'},
        {'O', 'X', '3', 'O', '5', 'X', 'X', 'O', 'X'},
        {'O', 'X', '3', 'O', '5', 'X', 'X', 'O', '9'},
        {'O', 'X', '3', 'O', '5', 'X', 'X', '8', '9'},
        {'O', 'X', '3', 'O', 'O', 'X', '7', 'X', '9'},
        {'O', 'X', '3', 'O', 'O', 'X', '7', '8', 'X'},
        {'O', 'X', '3', 'O', '5', 'X', '7', 'O', 'X'},
        {'O', 'X', '3', '4', 'O', 'X', 'X', 'O', 'X'},
        {'O', 'X', '3', '4', 'O', 'X', 'O', 'X', 'X'},
        {'O', 'X', '3', '4', 'O', 'X', 'O', 'X', '9'},
        {'O', 'X', '3', '4', 'O', 'X', 'O', '8', 'X'},
        {'O', 'X', '3', '4', 'O', 'X', '7', 'O', 'X'},
        {'O', 'X', '3', '4', 'O', 'X', '7', '8', 'X'},
        {'O', 'X', '3', '4', '5', 'X', 'O', 'O', 'X'},
        {'O', 'X', '3', '4', '5', 'X', 'O', '8', '9'},
        {'O', 'X', '3', '4', '5', 'X', 'X', 'O', '9'},
        {'O', 'X', '3', '4', '5', 'X', '7', 'O', 'X'},
        {'O', 'X', '3', 'O', '5', '6', 'X', 'X', '9'},
        {'O', 'X', '3', 'O', '5', '6', 'X', 'O', 'X'},
        {'O', 'X', '3', 'O', '5', '6', 'X', '8', 'X'},
        {'O', 'X', '3', '4', 'O', '6', 'X', 'O', 'X'},
        {'O', 'X', '3', '4', '5', 'O', 'X', 'X', '9'},
        {'O', 'X', '3', '4', '5', 'O', 'X', 'O', 'X'},
        {'O', 'X', '3', '4', '5', '6', 'X', 'O', 'X'},
        {'O', 'X', '3', '4', '5', '6', 'X', 'O', '9'},
        {'O', 'X', '3', '4', 'O', '6', 'O', 'X', 'X'},
        {'O', 'X', '3', '4', 'O', '6', '7', 'X', '9'},
        {'O', 'X', '3', '4', '5', 'O', '7', 'X', 'X'},
        {'O', 'X', '3', '4', 'O', '6', '7', '8', 'X'},
        {'O', 'X', '3', '4', '5', '6', '7', 'O', 'X'},
        {'1', 'X', '3', 'O', 'X', 'O', 'X', 'O', '9'},
        {'1', 'X', '3', 'O', 'X', 'O', 'X', '8', 'O'},
        {'1', 'X', '3', 'O', 'X', 'X', 'X', 'O', 'O'},
        {'1', 'X', '3', 'O', 'X', '6', 'X', 'O', 'O'},
        {'1', 'X', '3', 'O', 'X', '6', 'X', 'O', '9'},
        {'1', 'X', '3', 'O', 'O', 'X', 'X', 'O', 'X'},
        {'1', 'X', '3', 'O', 'O', 'X', 'X', 'O', '9'},
        {'1', 'X', '3', 'O', 'O', 'X', 'X', '8', '9'},
        {'1', 'X', '3', 'O', 'O', 'X', 'O', '8', 'X'},
        {'1', 'X', '3', 'O', 'O', 'X', '7', 'O', 'X'},
        {'1', 'X', '3', 'O', 'O', 'X', '7', '8', 'X'},
        {'1', 'X', '3', 'O', 'O', 'X', '7', '8', '9'},
        {'1', 'X', '3', 'O', '5', 'X', 'O', 'O', 'X'},
        {'1', 'X', '3', 'O', '5', 'X', 'X', 'O', 'O'},
        {'1', 'X', '3', 'O', '5', 'X', 'X', 'O', '9'},
        {'1', 'X', '3', 'O', '5', 'X', '7', 'O', 'X'},
        {'1', 'X', '3', 'O', '5', 'X', '7', 'O', '9'},
        {'1', 'X', '3', 'O', '5', 'X', 'X', '8', 'O'},
        {'1', 'X', '3', 'O', '5', 'X', '7', '8', '9'},
        {'1', 'X', '3', 'O', '5', '6', 'X', 'O', 'X'},
        {'1', 'X', '3', 'O', '5', '6', 'X', 'O', '9'},
        {'1', 'X', '3', 'O', '5', '6', 'X', '8', 'O'},
        {'1', 'X', '3', 'X', 'O', 'O', 'O', 'X', 'X'},
        {'1', 'X', '3', 'X', 'O', 'O', 'O', 'X', '9'},
        {'1', 'X', '3', 'X', 'O', 'O', '7', 'X', '9'},
        {'1', 'X', '3', 'X', 'O', 'O', 'O', '8', 'X'},
        {'1', 'X', '3', 'X', 'O', 'X', 'O', 'O', 'X'},
        {'1', 'X', '3', 'X', 'O', 'X', 'O', '8', '9'},
        {'1', 'X', '3', 'X', 'O', '6', 'O', 'X', '9'},
        {'1', 'X', '3', 'X', 'O', '6', 'O', 'O', 'X'},
        {'1', 'X', '3', 'X', 'O', '6', 'O', '8', 'X'},
        {'1', 'X', '3', 'X', 'O', '6', 'O', '8', '9'},
        {'1', 'X', '3', '4', 'O', 'X', 'O', 'X', '9'},
        {'1', 'X', '3', '4', 'O', 'X', 'O', 'O', 'X'},
        {'1', 'X', '3', '4', 'O', 'X', 'O', '8', 'X'},
        {'1', 'X', '3', '4', 'O', 'X', 'O', '8', '9'},
        {'1', 'X', '3', '4', 'O', 'X', '7', 'O', 'X'},
        {'1', 'X', '3', '4', 'O', 'X', '7', 'O', '9'},
        {'1', 'X', '3', '4', 'O', '6', 'O', 'X', 'X'},
        {'1', 'X', '3', '4', 'O', '6', 'O', '8', 'X'},
        {'1', 'X', '3', '4', 'O', '6', '7', 'O', 'X'},
        {'1', 'X', '3', 'X', '5', 'O', 'O', '8', '9'},
        {'1', 'X', '3', 'X', 'X', 'O', 'X', 'O', 'O'},
        {'1', 'X', '3', 'X', 'X', 'O', '7', '8', 'O'},
        {'1', 'X', '3', 'X', '5', 'O', 'X', '8', 'O'},
        {'1', 'X', '3', 'X', '5', 'O', '7', 'X', 'O'},
        {'1', 'X', '3', 'X', '5', 'O', '7', '8', 'O'},
        {'1', 'X', '3', '4', 'X', 'O', 'X', 'O', 'O'},
        {'1', 'X', '3', '4', 'X', 'O', 'X', 'O', '9'},
        {'1', 'X', '3', '4', 'X', 'O', 'X', '8', 'O'},
        {'1', 'X', '3', '4', '5', 'O', 'X', 'X', 'O'},
        {'1', 'X', '3', '4', '5', 'O', 'X', '8', 'O'},
        {'1', 'X', '3', '4', '5', 'O', '7', '8', '9'},
        {'1', 'X', '3', '4', 'X', '6', 'X', 'O', 'O'},
        {'1', 'X', '3', 'X', '5', '6', '7', '8', 'O'},
        {'1', 'X', '3', '4', '5', '6', 'X', '8', 'O'},
        {'1', 'X', '3', '4', '5', '6', '7', '8', 'O'},
        {'O', 'O', '3', 'X', 'X', 'O', 'X', 'X', 'O'},
        {'O', 'O', '3', 'X', 'X', 'O', 'X', 'X', '9'},
        {'O', 'O', '3', 'X', 'X', 'O', 'X', 'O', 'X'},
        {'O', 'O', '3', 'X', 'X', 'O', 'X', '8', 'X'},
        {'O', 'O', '3', 'X', 'X', 'O', 'X', '8', '9'},
        {'O', 'O', '3', 'X', 'X', '6', 'X', 'O', 'X'},
        {'O', 'O', '3', 'X', 'X', '6', 'X', 'O', '9'},
        {'O', 'O', '3', 'X', 'X', '6', 'X', 'X', 'O'},
        {'O', 'O', '3', 'X', 'X', '6', 'X', '8', 'O'},
        {'O', 'O', '3', 'X', 'X', '6', 'X', '8', '9'},
        {'O', 'O', '3', 'X', 'X', 'O', 'O', 'X', 'X'},
        {'O', 'O', '3', 'X', 'X', 'O', '7', 'X', 'X'},
        {'O', 'O', '3', 'X', 'X', 'O', '7', 'X', '9'},
        {'O', 'O', '3', 'X', 'X', '6', 'O', 'X', 'X'},
        {'O', 'O', '3', 'X', 'X', '6', '7', 'X', '9'},
        {'O', 'O', '3', 'X', 'X', 'O', '7', '8', 'X'},
        {'O', 'O', '3', 'X', 'X', '6', '7', '8', 'X'},
        {'O', '2', '3', 'X', 'X', 'O', 'X', 'O', 'X'},
        {'O', '2', '3', 'X', 'X', 'O', 'X', 'O', '9'},
        {'O', '2', '3', 'X', 'X', 'O', 'X', 'X', 'O'},
        {'O', '2', '3', 'X', 'X', 'O', 'X', '8', 'O'},
        {'O', '2', '3', 'X', 'X', 'O', 'X', '8', '9'},
        {'O', '2', '3', 'X', 'X', '6', 'X', 'O', 'O'},
        {'O', 'O', '3', 'X', 'O', 'X', 'X', 'X', '9'},
        {'O', 'O', '3', 'X', 'O', 'X', 'X', '8', 'X'},
        {'O', 'O', '3', 'X', 'O', 'X', 'X', '8', '9'},
        {'O', 'O', '3', 'X', '5', 'X', 'X', 'O', 'X'},
        {'O', 'O', '3', 'X', '5', 'X', 'X', 'X', 'O'},
        {'O', 'O', '3', 'X', '5', 'X', 'X', '8', '9'},
        {'O', 'O', '3', 'X', 'O', 'X', 'O', 'X', 'X'},
        {'O', 'O', '3', 'X', 'O', 'X', '7', 'X', 'X'},
        {'O', 'O', '3', 'X', 'O', 'X', '7', 'X', '9'},
        {'O', 'O', '3', 'X', '5', 'X', 'O', 'X', 'X'},
        {'O', 'O', '3', 'X', '5', 'X', '7', 'X', '9'},
        {'O', 'O', '3', 'X', 'O', 'X', '7', '8', 'X'},
        {'O', 'O', '3', 'X', '5', 'X', 'O', '8', 'X'},
        {'O', 'O', '3', 'X', '5', 'X', '7', 'O', 'X'},
        {'O', 'O', '3', 'X', '5', 'X', '7', '8', 'X'},
        {'O', '2', '3', 'X', 'O', 'X', 'O', 'X', 'X'},
        {'O', '2', '3', 'X', 'O', 'X', 'O', '8', 'X'},
        {'O', '2', '3', 'X', 'O', 'X', '7', 'O', 'X'},
        {'O', '2', '3', 'X', 'O', 'X', '7', '8', 'X'},
        {'O', '2', '3', 'X', '5', 'X', 'O', 'O', 'X'},
        {'O', 'O', '3', 'X', '5', '6', 'X', 'X', 'O'},
        {'O', 'O', '3', 'X', '5', '6', 'X', 'X', '9'},
        {'O', 'O', '3', 'X', '5', '6', 'X', 'O', 'X'},
        {'O', 'O', '3', 'X', '5', '6', 'X', '8', 'X'},
        {'O', 'O', '3', 'X', '5', '6', 'X', '8', '9'},
        {'O', '2', '3', 'X', '5', 'O', 'X', '8', '9'},
        {'O', 'O', '3', 'X', '5', '6', 'O', 'X', 'X'},
        {'O', 'O', '3', 'X', '5', '6', '7', 'X', 'X'},
        {'O', 'O', '3', 'X', '5', '6', '7', 'X', '9'},
        {'O', '2', '3', 'X', 'O', '6', 'O', 'X', 'X'},
        {'O', '2', '3', 'X', '5', '6', 'O', 'X', 'X'},
        {'O', '2', '3', 'X', '5', '6', '7', 'X', '9'},
        {'O', 'O', '3', 'X', '5', '6', '7', '8', 'X'},
        {'O', '2', '3', 'X', '5', '6', '7', '8', 'X'},
        {'1', 'O', '3', 'X', 'X', 'O', 'X', 'X', 'O'},
        {'1', 'O', '3', 'X', 'X', 'O', '7', 'X', 'O'},
        {'1', 'O', '3', 'X', 'X', 'O', '7', 'X', '9'},
        {'1', 'O', '3', 'X', 'O', 'X', 'O', 'X', 'X'},
        {'1', 'O', '3', 'X', 'O', 'X', 'O', 'X', '9'},
        {'1', 'O', '3', 'X', 'O', 'X', 'O', '8', 'X'},
        {'1', 'O', '3', 'X', '5', 'X', 'O', 'O', 'X'},
        {'1', 'O', '3', 'X', 'O', '6', 'O', 'X', 'X'},
        {'1', 'O', '3', 'X', '5', '6', 'O', 'X', 'X'},
        {'1', '2', '3', 'X', 'O', 'X', 'O', 'X', '9'},
        {'1', '2', '3', 'X', 'O', 'X', 'O', 'O', 'X'},
        {'1', '2', '3', 'X', 'O', 'X', 'O', '8', 'X'},
        {'1', '2', '3', 'X', 'O', 'O', 'O', 'X', 'X'},
        {'1', '2', '3', 'X', 'O', '6', 'O', 'X', 'X'},
        {'1', '2', '3', 'X', 'O', '6', 'O', 'X', '9'},
        {'1', '2', '3', 'X', 'O', '6', 'O', '8', 'X'},
        {'1', '2', '3', 'X', 'X', 'O', 'X', '8', 'O'},
        {'1', '2', '3', 'X', 'X', 'O', '7', 'X', 'O'},
        {'1', '2', '3', 'X', 'X', 'O', '7', '8', 'O'},
        {'1', '2', '3', 'X', '5', 'O', 'X', 'X', 'O'},
        {'1', '2', '3', 'X', '5', 'O', '7', 'X', 'O'},
        {'1', '2', '3', 'X', '5', '6', 'O', '8', '9'},
        {'1', '2', '3', 'X', '5', '6', '7', 'X', 'O'},
        {'O', 'O', '3', 'O', 'X', 'X', 'X', 'X', 'O'},
        {'O', 'O', '3', 'O', 'X', 'X', 'X', 'X', '9'},
        {'O', 'O', '3', 'O', 'X', 'X', 'X', 'O', 'X'},
        {'O', 'O', '3', 'O', 'X', 'X', 'X', '8', 'X'},
        {'O', 'O', '3', 'O', 'X', 'X', 'X', '8', '9'},
        {'O', 'O', '3', '4', 'X', 'X', 'X', 'O', 'X'},
        {'O', 'O', '3', '4', 'X', 'X', 'X', 'O', '9'},
        {'O', 'O', '3', '4', 'X', 'X', 'X', 'X', 'O'},
        {'O', 'O', '3', '4', 'X', 'X', 'X', '8', 'O'},
        {'O', 'O', '3', '4', 'X', 'X', 'X', '8', '9'},
        {'O', 'O', '3', 'O', 'X', 'X', '7', 'X', 'X'},
        {'O', 'O', '3', 'O', 'X', 'X', '7', 'X', '9'},
        {'O', 'O', '3', '4', 'X', 'X', 'O', 'X', 'X'},
        {'O', 'O', '3', '4', 'X', 'X', '7', 'X', '9'},
        {'O', 'O', '3', 'O', 'X', 'X', '7', '8', 'X'},
        {'O', 'O', '3', '4', 'X', 'X', 'O', '8', 'X'},
        {'O', 'O', '3', '4', 'X', 'X', '7', 'O', 'X'},
        {'O', 'O', '3', '4', 'X', 'X', '7', '8', 'X'},
        {'O', '2', '3', 'O', 'X', 'X', 'X', 'O', 'X'},
        {'O', '2', '3', 'O', 'X', 'X', 'X', 'O', '9'},
        {'O', '2', '3', 'O', 'X', 'X', 'X', '8', 'O'},
        {'O', '2', '3', 'O', 'X', 'X', 'X', '8', '9'},
        {'O', '2', '3', 'O', 'X', 'X', '7', 'O', 'X'},
        {'O', '2', '3', '4', 'X', 'X', 'O', 'O', 'X'},
        {'O', '2', '3', '4', 'X', 'X', 'X', 'O', 'O'},
        {'O', 'O', '3', 'O', 'X', '6', 'X', 'X', '9'},
        {'O', 'O', '3', '4', 'X', 'O', 'X', 'X', '9'},
        {'O', 'O', '3', '4', 'X', '6', 'X', 'X', 'O'},
        {'O', 'O', '3', '4', 'X', '6', 'X', 'X', '9'},
        {'O', 'O', '3', 'O', 'X', '6', 'X', '8', 'X'},
        {'O', 'O', '3', '4', 'X', 'O', 'X', '8', 'X'},
        {'O', 'O', '3', '4', 'X', '6', 'X', 'O', 'X'},
        {'O', 'O', '3', '4', 'X', '6', 'X', '8', 'X'},
        {'O', 'O', '3', '4', 'X', '6', 'X', '8', '9'},
        {'O', '2', '3', 'O', 'X', 'O', 'X', '8', 'X'},
        {'O', '2', '3', 'O', 'X', '6', 'X', 'O', 'X'},
        {'O', '2', '3', 'O', 'X', '6', 'X', '8', '9'},
        {'O', '2', '3', '4', 'X', 'O', 'X', 'O', 'X'},
        {'O', '2', '3', '4', 'X', 'O', 'X', '8', '9'},
        {'O', '2', '3', '4', 'X', '6', 'X', 'O', 'X'},
        {'O', '2', '3', '4', 'X', '6', 'X', 'O', '9'},
        {'O', '2', '3', '4', 'X', '6', 'X', '8', 'O'},
        {'O', '2', '3', '4', 'X', '6', 'X', '8', '9'},
        {'O', 'O', '3', '4', 'X', '6', 'O', 'X', 'X'},
        {'O', 'O', '3', '4', 'X', '6', '7', 'X', 'X'},
        {'O', 'O', '3', '4', 'X', '6', '7', 'X', '9'},
        {'O', 'O', '3', '4', 'X', '6', '7', '8', 'X'},
        {'O', '2', '3', '4', 'X', '6', '7', 'O', 'X'},
        {'O', '2', '3', '4', 'X', '6', '7', '8', 'X'},
        {'1', 'O', '3', 'O', 'X', 'X', 'X', 'O', '9'},
        {'1', 'O', '3', 'O', 'X', 'X', 'X', 'X', 'O'},
        {'1', 'O', '3', 'O', 'X', 'X', 'X', '8', 'O'},
        {'1', 'O', '3', 'O', 'X', 'X', 'X', '8', '9'},
        {'1', 'O', '3', 'O', 'X', 'X', '7', '8', '9'},
        {'1', 'O', '3', '4', 'X', 'X', 'X', 'O', 'O'},
        {'1', 'O', '3', 'O', 'X', 'O', 'X', 'X', '9'},
        {'1', 'O', '3', 'O', 'X', '6', 'X', 'X', 'O'},
        {'1', 'O', '3', 'O', 'X', '6', 'X', '8', '9'},
        {'1', 'O', '3', '4', 'X', 'O', 'X', 'X', 'O'},
        {'1', 'O', '3', '4', 'X', 'O', 'X', '8', '9'},
        {'1', 'O', '3', '4', 'X', '6', 'X', 'O', '9'},
        {'1', 'O', '3', '4', 'X', '6', 'X', 'X', 'O'},
        {'1', 'O', '3', '4', 'X', '6', 'X', '8', 'O'},
        {'1', '2', '3', 'O', 'X', 'X', 'X', 'O', 'O'},
        {'1', '2', '3', 'O', 'X', 'X', 'X', 'O', '9'},
        {'1', '2', '3', 'O', 'X', 'X', '7', 'O', '9'},
        {'1', '2', '3', 'O', 'X', 'X', 'X', '8', 'O'},
        {'1', '2', '3', 'O', 'X', 'O', 'X', '8', '9'},
        {'1', '2', '3', 'O', 'X', '6', 'X', 'O', '9'},
        {'1', '2', '3', 'O', 'X', '6', 'X', '8', 'O'},
        {'1', '2', '3', '4', 'X', 'O', 'X', 'O', '9'},
        {'1', '2', '3', '4', 'X', 'O', 'X', 'X', 'O'},
        {'1', '2', '3', '4', 'X', 'O', 'X', '8', 'O'},
        {'1', '2', '3', '4', 'X', '6', 'X', 'O', 'O'},
        {'1', '2', '3', '4', 'X', '6', 'X', '8', 'O'},
        {'O', 'O', '3', '4', '5', 'X', 'X', 'X', 'O'},
        {'O', 'O', '3', '4', '5', 'X', 'X', 'X', '9'},
        {'O', 'O', '3', 'O', '5', 'X', 'X', '8', 'X'},
        {'O', 'O', '3', '4', 'O', 'X', 'X', '8', 'X'},
        {'O', 'O', '3', '4', '5', 'X', 'X', 'O', 'X'},
        {'O', 'O', '3', '4', '5', 'X', 'X', '8', 'X'},
        {'O', 'O', '3', '4', '5', 'X', 'X', '8', '9'},
        {'O', '2', '3', 'O', 'O', 'X', 'X', '8', 'X'},
        {'O', '2', '3', 'O', '5', 'X', 'X', 'O', 'X'},
        {'O', '2', '3', '4', 'O', 'X', 'X', 'O', 'X'},
        {'O', '2', '3', '4', '5', 'X', 'X', 'O', 'X'},
        {'O', '2', '3', '4', '5', 'X', 'X', 'O', '9'},
        {'O', '2', '3', '4', '5', 'X', 'X', '8', '9'},
        {'O', 'O', '3', 'O', '5', 'X', '7', 'X', 'X'},
        {'O', 'O', '3', '4', 'O', 'X', '7', 'X', 'X'},
        {'O', 'O', '3', '4', '5', 'X', 'O', 'X', 'X'},
        {'O', 'O', '3', '4', '5', 'X', '7', 'X', 'X'},
        {'O', 'O', '3', '4', '5', 'X', '7', 'X', '9'},
        {'O', '2', '3', 'O', 'O', 'X', '7', 'X', 'X'},
        {'O', '2', '3', '4', 'O', 'X', 'O', 'X', 'X'},
        {'O', '2', '3', '4', '5', 'X', '7', 'X', '9'},
        {'O', 'O', '3', '4', '5', 'X', '7', '8', 'X'},
        {'O', '2', '3', 'O', '5', 'X', '7', '8', 'X'},
        {'O', '2', '3', '4', 'O', 'X', '7', '8', 'X'},
        {'O', '2', '3', '4', '5', 'X', 'O', '8', 'X'},
        {'O', '2', '3', '4', '5', 'X', '7', 'O', 'X'},
        {'O', '2', '3', '4', '5', 'X', '7', '8', 'X'},
        {'O', '2', '3', '4', '5', 'X', '7', '8', '9'},
        {'1', 'O', '3', 'O', 'O', 'X', 'X', '8', 'X'},
        {'1', 'O', '3', 'O', '5', 'X', 'X', 'O', 'X'},
        {'1', 'O', '3', 'O', '5', 'X', 'X', '8', '9'},
        {'1', 'O', '3', 'O', 'O', 'X', '7', 'X', 'X'},
        {'1', 'O', '3', 'O', '5', 'X', 'O', 'X', 'X'},
        {'1', 'O', '3', '4', 'O', 'X', 'O', 'X', 'X'},
        {'1', 'O', '3', '4', '5', 'X', 'O', 'X', 'X'},
        {'1', 'O', '3', 'O', '5', 'X', '7', '8', 'X'},
        {'1', 'O', '3', '4', 'O', 'X', '7', '8', 'X'},
        {'1', 'O', '3', '4', '5', 'X', 'O', '8', 'X'},
        {'1', 'O', '3', '4', '5', 'X', '7', 'O', 'X'},
        {'1', 'O', '3', '4', '5', 'X', '7', '8', 'X'},
        {'1', 'O', '3', '4', '5', 'X', '7', '8', '9'},
        {'1', '2', '3', 'O', 'O', 'X', 'X', 'O', 'X'},
        {'1', '2', '3', 'O', '5', 'X', 'X', 'O', 'X'},
        {'1', '2', '3', 'O', '5', 'X', 'X', 'O', '9'},
        {'1', '2', '3', 'O', '5', 'X', 'X', '8', '9'},
        {'1', '2', '3', 'O', 'O', 'X', 'O', 'X', 'X'},
        {'1', '2', '3', 'O', '5', 'X', '7', 'X', '9'},
        {'1', '2', '3', 'O', 'O', 'X', '7', '8', 'X'},
        {'1', '2', '3', 'O', '5', 'X', 'O', '8', 'X'},
        {'1', '2', '3', 'O', '5', 'X', '7', 'O', 'X'},
        {'1', '2', '3', 'O', '5', 'X', '7', '8', 'X'},
        {'1', '2', '3', '4', 'O', 'X', 'O', 'X', 'X'},
        {'1', '2', '3', '4', 'O', 'X', 'O', 'X', '9'},
        {'1', '2', '3', '4', 'O', 'X', '7', 'X', '9'},
        {'1', '2', '3', '4', 'O', 'X', 'O', '8', 'X'},
        {'1', '2', '3', '4', 'O', 'X', '7', 'O', 'X'},
        {'1', '2', '3', '4', 'O', 'X', '7', '8', 'X'},
        {'1', '2', '3', '4', '5', 'X', 'O', 'O', 'X'},
        {'1', '2', '3', '4', '5', 'X', '7', '8', '9'},
        {'O', '2', '3', '4', '5', '6', 'X', 'O', 'X'},
        {'O', '2', '3', '4', '5', '6', 'X', '8', '9'},
        {'1', '2', '3', 'O', '5', '6', 'X', 'O', 'X'},
        {'1', '2', '3', '4', '5', '6', 'X', 'X', 'O'},
        {'1', '2', '3', '4', 'O', '6', 'O', 'X', 'X'},
        {'O', '2', '3', '4', '5', '6', '7', '8', 'X'},
        {'1', 'O', '3', '4', '5', '6', '7', '8', 'X'},
        {'1', '2', '3', 'O', '5', '6', '7', '8', 'X'},
        {'1', '2', '3', '4', '5', '6', '7', 'O', 'X'}
    },
    {
        {'X', 'O', 'X', '4', 'O', 'X', 'O', 'X', 'O'},
        {'X', 'O', 'X', '4', 'O', 'X', '7', 'X', 'O'},
        {'X', 'O', 'X', '4', 'O', 'O', 'X', 'X', 'O'},
        {'X', 'O', 'X', '4', 'O', 'O', 'X', 'X', '9'},
        {'X', 'O', 'X', '4', 'O', 'O', 'X', '8', 'X'},
        {'X', 'O', 'X', '4', 'O', 'O', 'X', '8', '9'},
        {'X', 'O', 'X', '4', 'O', '6', 'X', 'X', 'O'},
        {'X', 'O', 'X', '4', 'O', '6', 'X', '8', 'O'},
        {'X', 'O', 'X', '4', 'O', 'O', 'O', 'X', 'X'},
        {'X', 'O', 'X', '4', 'O', 'O', '7', 'X', 'X'},
        {'X', 'O', 'X', '4', 'O', 'O', '7', 'X', '9'},
        {'X', 'O', 'X', '4', 'O', '6', 'O', 'X', '9'},
        {'X', 'O', 'X', '4', 'O', '6', '7', 'X', 'O'},
        {'X', 'O', 'X', '4', 'O', '6', '7', 'X', '9'},
        {'X', 'O', 'X', '4', 'O', 'O', '7', '8', 'X'},
        {'X', 'O', 'X', '4', 'X', 'O', 'O', 'X', 'O'},
        {'X', 'O', 'X', '4', 'X', 'O', '7', '8', '9'},
        {'X', 'O', 'X', '4', '5', 'O', 'X', 'O', '9'},
        {'X', 'O', 'X', '4', '5', 'O', 'X', 'X', 'O'},
        {'X', 'O', 'X', '4', '5', 'O', 'X', '8', 'O'},
        {'X', 'O', 'X', '4', '5', 'O', 'X', '8', '9'},
        {'X', 'O', 'X', '4', '5', 'O', 'O', 'X', '9'},
        {'X', 'O', 'X', '4', 'X', '6', 'O', 'X', 'O'},
        {'X', 'O', 'X', '4', '5', 'X', 'O', 'X', 'O'},
        {'X', 'O', 'X', '4', '5', '6', 'O', 'X', 'O'},
        {'X', 'O', 'X', '4', '5', '6', 'O', '8', 'X'},
        {'X', 'O', 'X', '4', 'X', '6', '7', 'O', '9'},
        {'X', 'O', 'X', '4', '5', 'X', '7', 'O', 'O'},
        {'X', 'O', 'X', '4', '5', '6', 'X', 'O', 'O'},
        {'X', 'O', 'X', '4', '5', '6', 'X', '8', 'O'},
        {'X', 'O', 'X', '4', '5', '6', '7', 'X', 'O'},
        {'X', 'O', 'O', '4', 'X', 'X', 'O', 'X', 'O'},
        {'X', 'O', 'O', '4', 'X', 'X', 'O', 'X', '9'},
        {'X', 'O', 'O', '4', 'X', 'X', 'O', '8', '9'},
        {'X', 'O', 'O', '4', 'X', 'X', 'X', 'O', 'O'},
        {'X', 'O', 'O', '4', 'X', 'X', 'X', 'O', '9'},
        {'X', 'O', 'O', '4', 'X', 'X', '7', 'O', '9'},
        {'X', 'O', 'O', '4', 'X', 'X', 'X', '8', 'O'},
        {'X', 'O', 'O', '4', 'X', 'X', '7', 'X', 'O'},
        {'X', 'O', 'O', '4', 'X', 'X', '7', '8', 'O'},
        {'X', 'O', 'O', '4', 'X', 'X', '7', '8', '9'},
        {'X', 'O', 'O', '4', 'X', 'O', 'X', '8', '9'},
        {'X', 'O', 'O', '4', 'X', '6', 'X', 'O', '9'},
        {'X', 'O', 'O', '4', 'X', '6', 'X', '8', 'O'},
        {'X', 'O', 'O', '4', 'X', '6', 'X', '8', '9'},
        {'X', 'O', '3', '4', 'X', 'X', 'O', 'O', '9'},
        {'X', 'O', '3', '4', 'X', 'X', 'O', 'X', 'O'},
        {'X', 'O', '3', '4', 'X', 'X', 'O', '8', 'O'},
        {'X', 'O', '3', '4', 'X', 'X', '7', 'O', 'O'},
        {'X', 'O', '3', '4', 'X', 'X', '7', '8', 'O'},
        {'X', 'O', '3', '4', 'X', '6', '7', '8', 'O'},
        {'X', 'O', 'O', '4', 'O', 'X', 'X', 'X', 'O'},
        {'X', 'O', 'O', '4', 'O', 'X', 'X', 'X', '9'},
        {'X', 'O', 'O', '4', 'O', 'X', 'X', '8', '9'},
        {'X', 'O', 'O', '4', '5', 'X', 'X', 'O', '9'},
        {'X', 'O', 'O', '4', '5', 'X', 'X', 'X', 'O'},
        {'X', 'O', 'O', '4', '5', 'X', 'X', '8', 'O'},
        {'X', 'O', 'O', '4', '5', 'X', 'X', '8', '9'},
        {'X', 'O', 'O', '4', '5', 'X', '7', 'X', 'O'},
        {'X', 'O', 'O', '4', '5', 'X', '7', 'X', '9'},
        {'X', 'O', 'O', '4', 'O', 'X', '7', '8', 'X'},
        {'X', 'O', 'O', '4', '5', 'X', '7', '8', '9'},
        {'X', 'O', '3', '4', 'O', 'X', 'X', 'X', 'O'},
        {'X', 'O', '3', '4', 'O', 'X', 'X', '8', 'O'},
        {'X', 'O', '3', '4', '5', 'X', 'X', 'O', 'O'},
        {'X', 'O', '3', '4', '5', 'X', 'X', '8', 'O'},
        {'X', 'O', '3', '4', '5', 'X', '7', 'X', 'O'},
        {'X', 'O', '3', '4', '5', 'X', '7', '8', 'O'},
        {'X', 'O', 'O', '4', 'O', '6', 'X', 'X', '9'},
        {'X', 'O', 'O', '4', '5', 'O', 'X', 'X', '9'},
        {'X', 'O', 'O', '4', '5', '6', 'X', 'X', 'O'},
        {'X', 'O', 'O', '4', '5', '6', 'X', 'X', '9'},
        {'X', 'O', 'O', '4', 'O', '6', 'X', '8', 'X'},
        {'X', 'O', 'O', '4', '5', 'O', 'X', '8', 'X'},
        {'X', 'O', 'O', '4', '5', '6', 'X', 'O', 'X'},
        {'X', 'O', 'O', '4', '5', '6', 'X', '8', 'X'},
        {'X', 'O', 'O', '4', '5', '6', 'X', '8', '9'},
        {'X', 'O', '3', '4', 'O', 'O', 'X', 'X', '9'},
        {'X', 'O', '3', '4', 'O', '6', 'X', 'X', 'O'},
        {'X', 'O', '3', '4', 'O', 'O', 'X', '8', 'X'},
        {'X', 'O', '3', '4', 'O', '6', 'X', '8', '9'},
        {'X', 'O', '3', '4', '5', 'O', 'X', 'X', 'O'},
        {'X', 'O', '3', '4', '5', 'O', 'X', 'O', 'X'},
        {'X', 'O', '3', '4', '5', 'O', 'X', '8', '9'},
        {'X', 'O', '3', '4', '5', '6', 'X', 'O', '9'},
        {'X', 'O', '3', '4', '5', '6', 'X', 'X', 'O'},
        {'X', 'O', '3', '4', '5', '6', 'X', '8', 'O'},
        {'X', 'O', 'O', '4', '5', '6', '7', 'X', 'X'},
        {'X', 'O', 'O', '4', '5', '6', '7', 'X', '9'},
        {'X', 'O', '3', '4', '5', '6', '7', '8', '9'},
        {'X', 'X', 'O', '4', 'O', 'X', 'X', 'O', 'O'},
        {'X', 'X', 'O', '4', 'O', 'X', 'X', 'O', '9'},
        {'X', 'X', 'O', '4', 'O', 'X', 'X', '8', 'O'},
        {'X', 'X', 'O', '4', 'O', 'O', 'X', 'X', '9'},
        {'X', 'X', 'O', '4', 'O', 'O', 'X', 'O', 'X'},
        {'X', 'X', 'O', '4', 'O', 'O', 'X', '8', 'X'},
        {'X', 'X', 'O', '4', 'O', 'O', 'X', '8', '9'},
        {'X', 'X', 'O', '4', 'O', '6', 'X', 'O', 'X'},
        {'X', 'X', 'O', '4', 'O', '6', 'X', 'O', '9'},
        {'X', 'X', 'O', '4', 'O', '6', 'X', '8', 'O'},
        {'X', 'X', 'O', '4', 'O', '6', 'X', '8', '9'},
        {'X', 'X', 'O', '4', 'O', 'O', '7', 'X', 'X'},
        {'X', 'X', 'O', '4', 'O', 'O', '7', 'X', '9'},
        {'X', 'X', 'O', '4', 'O', '6', '7', 'X', 'O'},
        {'X', 'X', 'O', '4', 'O', 'O', '7', '8', 'X'},
        {'X', 'X', 'O', '4', '5', 'O', 'X', 'O', 'X'},
        {'X', 'X', 'O', '4', '5', 'O', 'X', 'O', '9'},
        {'X', 'X', 'O', '4', '5', 'O', '7', '8', '9'},
        {'X', 'X', 'O', '4', 'X', '6', 'O', '8', '9'},
        {'X', 'X', 'O', '4', '5', 'X', 'O', 'O', '9'},
        {'X', 'X', 'O', '4', '5', 'X', 'O', '8', 'O'},
        {'X', 'X', 'O', '4', 'X', '6', '7', 'O', 'O'},
        {'X', 'X', 'O', '4', '5', 'X', 'X', 'O', 'O'},
        {'X', 'X', 'O', '4', '5', '6', 'X', 'O', 'O'},
        {'X', 'X', 'O', '4', '5', '6', 'X', 'O', '9'},
        {'X', 'X', 'O', '4', '5', '6', '7', '8', 'O'},
        {'X', '2', 'O', '4', 'X', 'O', 'X', 'O', '9'},
        {'X', '2', 'O', '4', 'X', 'X', 'O', 'O', '9'},
        {'X', '2', 'O', '4', 'X', 'X', 'O', '8', 'O'},
        {'X', '2', 'O', '4', 'X', 'X', 'X', 'O', 'O'},
        {'X', '2', 'O', '4', 'X', 'X', '7', 'O', 'O'},
        {'X', '2', 'O', '4', 'X', '6', 'X', 'O', 'O'},
        {'X', '2', 'O', '4', 'X', 'X', '7', '8', 'O'},
        {'X', '2', 'O', '4', 'O', 'X', 'X', 'O', '9'},
        {'X', '2', 'O', '4', 'O', 'X', 'X', 'X', 'O'},
        {'X', '2', 'O', '4', 'O', 'X', 'X', '8', 'O'},
        {'X', '2', 'O', '4', 'O', 'X', 'X', '8', '9'},
        {'X', '2', 'O', '4', '5', 'X', 'X', 'O', 'O'},
        {'X', '2', 'O', '4', '5', 'X', 'X', 'O', '9'},
        {'X', '2', 'O', '4', '5', 'X', '7', 'O', '9'},
        {'X', '2', 'O', '4', '5', 'X', 'X', '8', 'O'},
        {'X', '2', 'O', '4', '5', 'X', '7', 'X', 'O'},
        {'X', '2', 'O', '4', '5', 'X', '7', '8', 'O'},
        {'X', '2', 'O', '4', '5', 'X', '7', '8', '9'},
        {'X', '2', 'O', '4', 'O', 'O', 'X', 'X', '9'},
        {'X', '2', 'O', '4', 'O', '6', 'X', 'X', 'O'},
        {'X', '2', 'O', '4', 'O', 'O', 'X', '8', 'X'},
        {'X', '2', 'O', '4', 'O', '6', 'X', 'O', 'X'},
        {'X', '2', 'O', '4', 'O', '6', 'X', '8', '9'},
        {'X', '2', 'O', '4', '5', 'O', 'X', 'O', 'X'},
        {'X', '2', 'O', '4', '5', 'O', 'X', '8', '9'},
        {'X', '2', 'O', '4', '5', '6', 'X', 'O', '9'},
        {'X', '2', 'O', '4', '5', '6', 'X', '8', 'O'},
        {'X', '2', 'O', '4', '5', '6', '7', '8', '9'},
        {'X', 'X', '3', '4', 'O', 'O', 'X', 'O', 'X'},
        {'X', 'X', '3', '4', 'O', 'O', 'X', '8', '9'},
        {'X', 'X', '3', '4', 'O', 'O', '7', 'X', '9'},
        {'X', 'X', '3', '4', 'O', 'O', '7', '8', 'X'},
        {'X', '2', 'X', '4', 'O', 'O', 'X', 'X', 'O'},
        {'X', '2', 'X', '4', 'O', 'O', 'X', '8', '9'},
        {'X', '2', 'X', '4', 'O', 'O', 'O', 'X', 'X'},
        {'X', '2', 'X', '4', 'O', 'O', '7', 'X', '9'},
        {'X', '2', 'X', '4', 'O', 'O', '7', '8', 'X'},
        {'X', '2', '3', '4', 'O', 'X', 'X', 'O', 'O'},
        {'X', '2', '3', '4', 'O', 'X', 'X', '8', 'O'},
        {'X', '2', '3', '4', 'O', 'O', 'X', 'X', 'O'},
        {'X', '2', '3', '4', 'O', 'O', 'X', 'X', '9'},
        {'X', '2', '3', '4', 'O', 'O', 'X', 'O', 'X'},
        {'X', '2', '3', '4', 'O', 'O', 'X', '8', 'X'},
        {'X', '2', '3', '4', 'O', 'O', 'X', '8', '9'},
        {'X', '2', '3', '4', 'O', '6', 'X', 'O', '9'},
        {'X', '2', '3', '4', 'O', '6', 'X', 'X', 'O'},
        {'X', '2', '3', '4', 'O', '6', 'X', '8', 'O'},
        {'X', '2', '3', '4', 'O', '6', 'X', '8', '9'},
        {'X', '2', '3', '4', 'O', 'O', '7', 'X', 'X'},
        {'X', '2', '3', '4', 'O', 'O', '7', 'X', '9'},
        {'X', '2', '3', '4', 'O', '6', '7', 'X', '9'},
        {'X', '2', '3', '4', 'O', 'O', '7', '8', 'X'},
        {'X', '2', '3', '4', '5', 'O', 'X', 'O', '9'},
        {'X', '2', '3', '4', '5', 'O', 'X', '8', 'O'},
        {'X', '2', '3', '4', '5', '6', 'X', 'O', 'O'},
        {'X', '2', '3', '4', '5', 'X', '7', '8', 'O'},
        {'O', 'X', 'X', '4', 'O', 'X', 'O', 'X', '9'},
        {'O', 'X', 'X', '4', 'O', 'O', 'X', 'X', '9'},
        {'O', 'X', 'X', '4', 'O', 'O', 'X', 'O', 'X'},
        {'O', 'X', 'X', '4', 'O', 'O', 'X', '8', 'X'},
        {'O', 'X', 'X', '4', 'O', 'O', 'X', '8', '9'},
        {'O', 'X', 'X', '4', 'O', 'O', 'O', 'X', 'X'},
        {'O', 'X', 'X', '4', 'O', 'O', '7', 'X', 'X'},
        {'O', 'X', 'X', '4', 'O', 'O', '7', 'X', '9'},
        {'O', 'X', 'X', '4', 'O', '6', 'O', 'X', 'X'},
        {'O', 'X', 'X', '4', 'O', '6', 'O', 'X', '9'},
        {'O', 'X', 'X', '4', 'O', 'O', '7', '8', 'X'},
        {'O', 'X', 'X', '4', 'X', 'O', 'O', 'O', 'X'},
        {'O', 'X', 'X', '4', 'X', 'O', 'O', '8', 'X'},
        {'O', 'X', 'X', '4', 'X', 'O', '7', '8', '9'},
        {'O', 'X', 'X', '4', '5', 'O', 'O', 'X', 'X'},
        {'O', 'X', 'X', '4', '5', 'O', 'O', '8', 'X'},
        {'O', 'X', 'X', '4', '5', 'O', '7', 'O', 'X'},
        {'O', 'X', 'X', '4', '5', 'O', '7', '8', 'X'},
        {'O', 'X', 'X', '4', 'X', 'X', 'O', 'O', '9'},
        {'O', 'X', 'X', '4', 'X', '6', 'O', 'O', 'X'},
        {'O', 'X', 'X', '4', 'X', '6', 'O', 'O', '9'},
        {'O', 'X', 'X', '4', 'X', 'X', 'O', '8', 'O'},
        {'O', 'X', 'X', '4', 'X', '6', 'O', '8', '9'},
        {'O', 'X', 'X', '4', '5', 'X', 'O', 'X', 'O'},
        {'O', 'X', 'X', '4', '5', 'X', 'O', '8', 'O'},
        {'O', 'X', 'X', '4', '5', 'X', 'O', '8', '9'},
        {'O', 'X', 'X', '4', '5', '6', 'O', 'X', '9'},
        {'O', 'X', 'X', '4', '5', '6', 'O', '8', 'X'},
        {'O', 'X', 'X', '4', '5', '6', 'O', '8', '9'},
        {'O', 'X', 'X', '4', '5', 'X', '7', 'O', 'O'},
        {'O', 'X', 'X', '4', 'X', '6', '7', '8', 'O'},
        {'O', 'X', 'X', '4', '5', '6', '7', '8', '9'},
        {'O', 'X', 'O', '4', 'X', 'X', 'O', 'O', 'X'},
        {'O', 'X', 'O', '4', 'X', 'X', 'O', '8', 'X'},
        {'O', 'X', 'O', '4', 'X', 'X', 'O', '8', '9'},
        {'O', 'X', 'O', '4', 'X', 'X', 'X', 'O', 'O'},
        {'O', 'X', 'O', '4', 'X', 'X', 'X', 'O', '9'},
        {'O', 'X', 'O', '4', 'X', 'X', '7', 'O', 'X'},
        {'O', 'X', 'O', '4', 'X', 'X', '7', 'O', '9'},
        {'O', 'X', 'O', '4', 'X', 'X', 'X', '8', 'O'},
        {'O', 'X', 'O', '4', 'X', 'X', '7', '8', 'O'},
        {'O', 'X', 'O', '4', 'X', 'X', '7', '8', '9'},
        {'O', 'X', 'O', '4', 'X', 'O', 'X', 'O', 'X'},
        {'O', 'X', 'O', '4', 'X', '6', 'X', 'O', 'X'},
        {'O', 'X', 'O', '4', 'X', '6', 'X', 'O', '9'},
        {'O', 'X', 'O', '4', 'X', '6', '7', 'O', 'X'},
        {'O', 'X', '3', '4', 'X', 'X', 'O', 'O', 'X'},
        {'O', 'X', '3', '4', 'X', 'X', 'O', 'O', '9'},
        {'O', 'X', '3', '4', 'X', 'X', 'O', '8', 'O'},
        {'O', 'X', '3', '4', 'X', 'X', 'O', '8', '9'},
        {'O', 'X', '3', '4', 'X', '6', 'O', 'O', 'X'},
        {'O', 'X', '3', '4', 'X', '6', 'O', '8', 'X'},
        {'O', 'X', '3', '4', 'X', 'X', '7', 'O', 'O'},
        {'O', 'X', '3', '4', 'X', 'X', '7', 'O', '9'},
        {'O', 'X', '3', '4', 'X', '6', '7', 'O', '9'},
        {'O', 'X', 'O', '4', 'O', 'X', 'X', 'O', 'X'},
        {'O', 'X', 'O', '4', '5', 'X', 'X', 'O', 'X'},
        {'O', 'X', 'O', '4', '5', 'X', 'X', 'O', '9'},
        {'O', 'X', 'O', '4', 'O', 'X', '7', 'X', '9'},
        {'O', 'X', 'O', '4', '5', 'X', 'O', 'X', 'X'},
        {'O', 'X', 'O', '4', '5', 'X', 'O', '8', 'X'},
        {'O', 'X', 'O', '4', '5', 'X', '7', 'O', 'X'},
        {'O', 'X', '3', '4', '5', 'X', 'O', 'X', '9'},
        {'O', 'X', '3', '4', '5', 'X', 'O', '8', 'X'},
        {'O', 'X', '3', '4', '5', 'X', '7', 'O', '9'},
        {'O', 'X', 'O', '4', '5', '6', 'X', 'X', '9'},
        {'O', 'X', 'O', '4', '5', '6', 'X', 'O', 'X'},
        {'O', 'X', 'O', '4', '5', '6', '7', 'X', 'X'},
        {'O', 'X', '3', '4', '5', '6', 'O', 'X', 'X'},
        {'O', 'X', '3', '4', '5', '6', 'O', '8', 'X'},
        {'O', 'X', '3', '4', '5', '6', '7', '8', '9'},
        {'1', 'X', 'O', '4', 'X', 'X', 'O', 'O', '9'},
        {'1', 'X', 'O', '4', 'X', 'X', 'O', '8', 'O'},
        {'1', 'X', 'O', '4', 'X', 'X', 'X', 'O', 'O'},
        {'1', 'X', 'O', '4', 'X', 'X', '7', 'O', 'O'},
        {'1', 'X', 'O', '4', 'X', 'X', '7', 'O', '9'},
        {'1', 'X', 'O', '4', 'X', '6', '7', 'O', '9'},
        {'1', 'X', 'O', '4', '5', 'X', 'X', 'O', 'O'},
        {'1', 'X', 'O', '4', '5', 'X', '7', 'O', '9'},
        {'1', 'X', 'O', '4', '5', 'X', 'X', '8', 'O'},
        {'1', 'X', 'O', '4', '5', 'X', '7', '8', '9'},
        {'1', 'X', 'O', '4', '5', 'O', '7', '8', 'X'},
        {'1', 'X', 'X', '4', 'O', 'O', 'X', 'O', 'X'},
        {'1', 'X', 'X', '4', 'O', 'O', 'X', '8', '9'},
        {'1', 'X', 'X', '4', 'O', 'O', 'O', 'X', 'X'},
        {'1', 'X', 'X', '4', 'O', 'O', '7', 'X', '9'},
        {'1', 'X', 'X', '4', 'O', 'O', '7', '8', 'X'},
        {'1', 'X', '3', '4', 'O', 'O', 'X', 'X', '9'},
        {'1', 'X', '3', '4', 'O', 'O', 'X', 'O', 'X'},
        {'1', 'X', '3', '4', 'O', 'O', 'X', '8', 'X'},
        {'1', 'X', '3', '4', 'O', 'O', 'X', '8', '9'},
        {'1', 'X', '3', '4', 'O', 'O', '7', 'X', 'X'},
        {'1', 'X', '3', '4', 'O', 'O', '7', '8', 'X'},
        {'O', 'O', 'X', '4', 'X', 'X', 'O', 'X', 'O'},
        {'O', 'O', 'X', '4', 'X', 'X', 'O', 'X', '9'},
        {'O', 'O', 'X', '4', 'X', 'X', 'O', '8', '9'},
        {'O', 'O', 'X', '4', 'X', 'X', '7', 'O', '9'},
        {'O', 'O', 'X', '4', 'X', 'X', '7', 'X', 'O'},
        {'O', 'O', 'X', '4', 'X', 'X', '7', '8', 'O'},
        {'O', 'O', 'X', '4', 'X', 'X', '7', '8', '9'},
        {'O', 'O', 'X', '4', 'X', 'O', 'O', 'X', 'X'},
        {'O', 'O', 'X', '4', 'X', '6', 'O', 'X', 'X'},
        {'O', 'O', 'X', '4', 'X', '6', 'O', 'X', '9'},
        {'O', 'O', 'X', '4', 'X', '6', '7', '8', 'X'},
        {'O', '2', 'X', '4', 'X', 'O', 'O', 'X', 'X'},
        {'O', '2', 'X', '4', 'X', 'O', 'O', '8', 'X'},
        {'O', '2', 'X', '4', 'X', 'X', 'O', 'O', '9'},
        {'O', '2', 'X', '4', 'X', 'X', 'O', 'X', 'O'},
        {'O', '2', 'X', '4', 'X', 'X', 'O', '8', 'O'},
        {'O', '2', 'X', '4', 'X', 'X', 'O', '8', '9'},
        {'O', '2', 'X', '4', 'X', '6', 'O', 'X', '9'},
        {'O', '2', 'X', '4', 'X', '6', 'O', '8', 'X'},
        {'O', '2', 'X', '4', 'X', '6', 'O', '8', '9'},
        {'O', '2', 'X', '4', 'X', 'X', '7', 'O', 'O'},
        {'O', 'O', 'X', '4', '5', 'X', 'X', '8', '9'},
        {'O', 'O', 'X', '4', '5', 'X', '7', 'X', '9'},
        {'O', '2', 'X', '4', '5', 'X', 'O', 'X', '9'},
        {'O', 'O', 'X', '4', '5', '6', 'X', 'X', '9'},
        {'O', 'O', 'X', '4', '5', '6', 'X', '8', 'X'},
        {'O', 'O', 'X', '4', '5', '6', '7', 'X', 'X'},
        {'O', 'O', 'X', '4', '5', '6', '7', 'X', '9'},
        {'O', '2', 'X', '4', '5', 'O', 'O', 'X', 'X'},
        {'O', '2', 'X', '4', '5', '6', 'O', 'X', 'X'},
        {'O', '2', 'X', '4', '5', '6', 'O', 'X', '9'},
        {'1', 'O', 'X', '4', 'X', 'X', 'O', 'O', '9'},
        {'1', 'O', 'X', '4', 'X', 'X', 'O', 'X', 'O'},
        {'1', 'O', 'X', '4', 'X', 'X', 'O', '8', 'O'},
        {'1', 'O', 'X', '4', 'X', 'X', '7', 'O', 'O'},
        {'1', 'O', 'X', '4', '5', 'X', '7', 'X', 'O'},
        {'1', '2', 'X', '4', 'O', 'O', 'X', 'X', '9'},
        {'1', '2', 'X', '4', 'O', 'O', 'X', '8', 'X'},
        {'1', '2', 'X', '4', 'O', 'O', 'X', '8', '9'},
        {'1', '2', 'X', '4', 'O', 'O', 'O', 'X', 'X'},
        {'1', '2', 'X', '4', 'O', 'O', '7', 'X', 'X'},
        {'1', '2', 'X', '4', 'O', 'O', '7', 'X', '9'},
        {'1', '2', 'X', '4', 'O', '6', '7', 'X', '9'},
        {'1', '2', 'X', '4', 'O', 'O', '7', '8', 'X'},
        {'1', '2', 'X', '4', '5', 'O', 'O', 'X', 'X'},
        {'O', 'O', '3', '4', 'X', 'X', 'O', 'X', '9'},
        {'O', 'O', '3', '4', 'X', 'X', '7', 'X', 'O'},
        {'O', 'O', '3', '4', 'X', 'X', '7', '8', '9'},
        {'O', '2', 'O', '4', 'X', 'X', 'X', 'O', '9'},
        {'O', '2', 'O', '4', 'X', 'X', 'X', '8', 'O'},
        {'O', '2', 'O', '4', 'X', 'X', 'O', '8', 'X'},
        {'O', '2', 'O', '4', 'X', 'X', '7', 'O', 'X'},
        {'O', '2', 'O', '4', 'X', 'X', '7', '8', '9'},
        {'O', '2', '3', '4', 'X', 'X', 'O', 'X', '9'},
        {'O', '2', '3', '4', 'X', 'X', 'O', '8', 'X'},
        {'O', '2', '3', '4', 'X', 'X', 'O', '8', '9'},
        {'O', '2', '3', '4', 'X', 'X', '7', 'O', '9'},
        {'O', '2', '3', '4', 'X', 'X', '7', '8', 'O'},
        {'O', '2', '3', '4', 'X', 'X', '7', '8', '9'},
        {'O', '2', '3', '4', 'X', '6', 'O', 'X', 'X'},
        {'O', '2', '3', '4', 'X', '6', 'O', '8', 'X'},
        {'1', 'O', 'O', '4', 'X', 'X', 'X', 'O', '9'},
        {'1', 'O', 'O', '4', 'X', 'X', 'X', '8', 'O'},
        {'1', 'O', 'O', '4', 'X', 'X', 'O', 'X', '9'},
        {'1', 'O', 'O', '4', 'X', 'X', '7', 'X', 'O'},
        {'1', 'O', 'O', '4', 'X', 'X', '7', '8', '9'},
        {'1', 'O', '3', '4', 'X', 'X', 'O', 'X', 'O'},
        {'1', 'O', '3', '4', 'X', 'X', 'O', 'X', '9'},
        {'1', 'O', '3', '4', 'X', 'X', 'O', '8', '9'},
        {'1', 'O', '3', '4', 'X', 'X', '7', 'O', '9'},
        {'1', 'O', '3', '4', 'X', 'X', '7', 'X', 'O'},
        {'1', 'O', '3', '4', 'X', 'X', '7', '8', 'O'},
        {'1', '2', 'O', '4', 'X', 'X', 'O', '8', '9'},
        {'1', '2', 'O', '4', 'X', 'X', 'X', 'O', 'O'},
        {'1', '2', 'O', '4', 'X', 'X', 'X', 'O', '9'},
        {'1', '2', 'O', '4', 'X', 'X', '7', 'O', '9'},
        {'1', '2', 'O', '4', 'X', 'X', 'X', '8', 'O'},
        {'1', '2', 'O', '4', 'X', 'X', '7', '8', 'O'},
        {'1', '2', 'O', '4', 'X', 'X', '7', '8', '9'},
        {'1', '2', '3', '4', 'X', 'X', 'O', 'O', '9'},
        {'1', '2', '3', '4', 'X', 'X', 'O', '8', 'O'},
        {'1', '2', '3', '4', 'X', 'X', 'O', '8', '9'},
        {'1', '2', '3', '4', 'X', 'X', '7', 'O', 'O'},
        {'1', '2', '3', '4', 'X', 'X', '7', '8', 'O'},
        {'O', '2', '3', '4', '5', 'X', 'O', 'X', 'X'},
        {'1', 'O', '3', '4', '5', 'X', 'X', '8', 'O'},
        {'1', '2', 'O', '4', '5', 'X', 'X', 'O', '9'},
        {'1', '2', 'O', '4', '5', 'X', 'X', '8', 'O'},
        {'1', '2', '3', '4', '5', 'X', 'X', '8', 'O'}
    },
    {
        {'X', 'O', 'X', 'O', '5', 'X', 'O', 'X', 'O'},
        {'X', 'O', 'X', 'O', '5', 'X', 'X', 'O', 'O'},
        {'X', 'O', 'X', 'O', '5', 'X', 'X', 'O', '9'},
        {'X', 'O', 'X', 'O', '5', 'X', 'X', '8', 'O'},
        {'X', 'O', 'X', 'O', '5', 'X', '7', 'X', 'O'},
        {'X', 'O', 'X', 'O', '5', 'X', '7', '8', 'O'},
        {'X', 'O', 'X', 'O', '5', 'O', 'X', 'X', 'O'},
        {'X', 'O', 'X', 'O', '5', 'O', 'X', 'X', '9'},
        {'X', 'O', 'X', 'O', '5', 'O', 'X', 'O', 'X'},
        {'X', 'O', 'X', 'O', '5', 'O', 'X', '8', 'X'},
        {'X', 'O', 'X', 'O', '5', 'O', 'X', '8', '9'},
        {'X', 'O', 'X', 'O', '5', '6', 'X', 'O', 'X'},
        {'X', 'O', 'X', 'O', '5', '6', 'X', 'O', '9'},
        {'X', 'O', 'X', 'O', '5', '6', 'X', 'X', 'O'},
        {'X', 'O', 'X', 'O', '5', '6', 'X', '8', 'O'},
        {'X', 'O', 'X', 'O', '5', '6', 'X', '8', '9'},
        {'X', 'O', 'X', 'O', '5', 'O', 'O', 'X', 'X'},
        {'X', 'O', 'X', 'O', '5', 'O', '7', 'X', 'X'},
        {'X', 'O', 'X', 'O', '5', 'O', '7', 'X', '9'},
        {'X', 'O', 'X', 'O', '5', '6', 'O', 'X', 'X'},
        {'X', 'O', 'X', 'O', '5', '6', '7', 'X', 'O'},
        {'X', 'O', 'X', 'O', '5', '6', '7', 'X', '9'},
        {'X', 'O', 'X', 'O', '5', 'O', '7', '8', 'X'},
        {'X', 'O', 'X', 'O', '5', '6', 'O', '8', 'X'},
        {'X', 'O', 'X', 'O', '5', '6', '7', 'O', 'X'},
        {'X', 'O', 'X', 'O', '5', '6', '7', '8', 'X'},
        {'X', 'O', 'X', 'O', '5', '6', '7', '8', '9'},
        {'X', 'O', 'X', 'X', '5', 'O', 'O', 'X', 'O'},
        {'X', 'O', 'X', 'X', '5', 'O', 'O', 'X', '9'},
        {'X', 'O', 'X', 'X', '5', 'O', 'O', 'O', 'X'},
        {'X', 'O', 'X', 'X', '5', 'O', 'O', '8', 'X'},
        {'X', 'O', 'X', 'X', '5', 'O', 'O', '8', '9'},
        {'X', 'O', 'X', 'X', '5', 'O', '7', 'O', 'X'},
        {'X', 'O', 'X', '4', '5', 'O', 'X', 'O', 'X'},
        {'X', 'O', 'X', '4', '5', 'O', 'O', 'X', 'X'},
        {'X', 'O', 'X', '4', '5', 'O', '7', 'X', '9'},
        {'X', 'O', 'X', '4', '5', 'O', 'O', '8', 'X'},
        {'X', 'O', 'X', '4', '5', 'O', '7', 'O', 'X'},
        {'X', 'O', 'X', '4', '5', 'O', '7', '8', 'X'},
        {'X', 'O', 'X', '4', '5', 'O', '7', '8', '9'},
        {'X', 'O', 'X', 'X', '5', 'X', 'O', 'O', '9'},
        {'X', 'O', 'X', 'X', '5', '6', 'O', 'O', 'X'},
        {'X', 'O', 'X', 'X', '5', '6', 'O', 'O', '9'},
        {'X', 'O', 'X', 'X', '5', '6', 'O', 'X', 'O'},
        {'X', 'O', 'X', '4', '5', '6', 'O', 'X', '9'},
        {'X', 'O', 'X', '4', '5', '6', 'O', 'O', 'X'},
        {'X', 'O', 'X', 'X', '5', 'X', '7', 'O', 'O'},
        {'X', 'O', 'X', 'X', '5', '6', '7', 'O', '9'},
        {'X', 'O', 'X', '4', '5', 'X', 'X', 'O', 'O'},
        {'X', 'O', 'X', '4', '5', 'X', '7', 'O', '9'},
        {'X', 'O', 'X', '4', '5', '6', 'X', 'O', '9'},
        {'X', 'O', 'X', '4', '5', '6', '7', 'O', 'X'},
        {'X', 'O', 'X', '4', '5', '6', '7', 'O', '9'},
        {'X', 'O', 'X', '4', '5', '6', '7', '8', '9'},
        {'X', 'O', 'O', 'X', '5', 'X', 'O', 'X', 'O'},
        {'X', 'O', 'O', 'X', '5', 'X', 'O', 'X', '9'},
        {'X', 'O', 'O', 'X', '5', 'X', 'O', 'O', 'X'},
        {'X', 'O', 'O', 'X', '5', 'X', 'O', '8', 'X'},
        {'X', 'O', 'O', 'X', '5', 'X', 'O', '8', '9'},
        {'X', 'O', 'O', 'X', '5', 'X', '7', 'O', 'X'},
        {'X', 'O', 'O', 'X', '5', 'X', '7', 'O', '9'},
        {'X', 'O', 'O', 'X', '5', 'X', '7', 'X', 'O'},
        {'X', 'O', 'O', 'X', '5', 'X', '7', '8', 'O'},
        {'X', 'O', 'O', 'X', '5', 'X', '7', '8', '9'},
        {'X', 'O', 'O', 'X', '5', 'O', 'O', 'X', 'X'},
        {'X', 'O', 'O', 'X', '5', 'O', '7', 'X', 'X'},
        {'X', 'O', 'O', 'X', '5', '6', 'O', 'X', 'X'},
        {'X', 'O', 'O', 'X', '5', '6', 'O', 'X', '9'},
        {'X', 'O', 'O', 'X', '5', '6', '7', 'X', '9'},
        {'X', 'O', 'O', 'X', '5', 'O', '7', '8', 'X'},
        {'X', 'O', 'O', 'X', '5', '6', 'O', '8', 'X'},
        {'X', 'O', 'O', 'X', '5', '6', '7', 'O', 'X'},
        {'X', 'O', 'O', 'X', '5', '6', '7', '8', 'X'},
        {'X', 'O', '3', 'X', '5', 'O', 'O', 'X', 'X'},
        {'X', 'O', '3', 'X', '5', 'O', 'O', '8', 'X'},
        {'X', 'O', '3', 'X', '5', 'O', '7', 'O', 'X'},
        {'X', 'O', '3', 'X', '5', 'X', 'O', 'O', 'X'},
        {'X', 'O', '3', 'X', '5', 'X', 'O', 'O', '9'},
        {'X', 'O', '3', 'X', '5', 'X', 'O', 'X', 'O'},
        {'X', 'O', '3', 'X', '5', 'X', 'O', '8', 'O'},
        {'X', 'O', '3', 'X', '5', 'X', 'O', '8', '9'},
        {'X', 'O', '3', 'X', '5', '6', 'O', 'X', '9'},
        {'X', 'O', '3', 'X', '5', '6', 'O', 'O', 'X'},
        {'X', 'O', '3', 'X', '5', '6', 'O', '8', 'X'},
        {'X', 'O', '3', 'X', '5', '6', 'O', '8', '9'},
        {'X', 'O', '3', 'X', '5', 'X', '7', 'O', 'O'},
        {'X', 'O', '3', 'X', '5', 'X', '7', 'O', '9'},
        {'X', 'O', '3', 'X', '5', '6', '7', 'O', 'X'},
        {'X', 'O', 'O', 'O', '5', 'X', 'X', 'X', 'O'},
        {'X', 'O', 'O', 'O', '5', 'X', 'X', 'O', 'X'},
        {'X', 'O', 'O', 'O', '5', 'X', 'X', '8', 'X'},
        {'X', 'O', 'O', '4', '5', 'X', 'X', 'O', 'X'},
        {'X', 'O', 'O', 'O', '5', 'X', 'O', 'X', 'X'},
        {'X', 'O', 'O', 'O', '5', 'X', '7', 'X', 'X'},
        {'X', 'O', 'O', '4', '5', 'X', 'O', 'X', 'X'},
        {'X', 'O', 'O', '4', '5', 'X', 'O', 'X', '9'},
        {'X', 'O', 'O', 'O', '5', 'X', '7', '8', 'X'},
        {'X', 'O', 'O', '4', '5', 'X', 'O', '8', 'X'},
        {'X', 'O', 'O', '4', '5', 'X', '7', 'O', 'X'},
        {'X', 'O', 'O', '4', '5', 'X', '7', '8', 'X'},
        {'X', 'O', '3', 'O', '5', 'X', 'X', 'O', 'X'},
        {'X', 'O', '3', 'O', '5', 'X', 'X', 'O', '9'},
        {'X', 'O', '3', 'O', '5', 'X', 'X', '8', '9'},
        {'X', 'O', '3', '4', '5', 'X', 'O', 'X', '9'},
        {'X', 'O', '3', '4', '5', 'X', 'O', '8', '9'},
        {'X', 'O', '3', '4', '5', 'X', 'X', 'O', '9'},
        {'X', 'O', '3', '4', '5', 'X', '7', 'O', 'X'},
        {'X', 'O', '3', '4', '5', 'X', '7', 'O', '9'},
        {'X', 'O', '3', '4', '5', 'X', '7', '8', '9'},
        {'X', 'O', 'O', 'O', '5', '6', 'X', '8', 'X'},
        {'X', 'O', '3', 'O', '5', 'O', 'X', '8', 'X'},
        {'X', 'O', '3', 'O', '5', '6', 'X', 'O', 'X'},
        {'X', 'O', '3', 'O', '5', '6', 'X', '8', '9'},
        {'X', 'O', '3', '4', '5', '6', 'X', 'O', 'X'},
        {'X', 'O', 'O', 'O', '5', '6', '7', 'X', 'X'},
        {'X', 'O', 'O', '4', '5', 'O', '7', 'X', 'X'},
        {'X', 'O', 'O', '4', '5', '6', 'O', 'X', 'X'},
        {'X', 'O', '3', 'O', '5', 'O', '7', 'X', 'X'},
        {'X', 'O', '3', 'O', '5', '6', 'O', 'X', 'X'},
        {'X', 'O', '3', '4', '5', 'O', 'O', 'X', 'X'},
        {'X', 'O', '3', '4', '5', '6', 'O', 'X', 'X'},
        {'X', 'O', 'O', '4', '5', '6', '7', '8', 'X'},
        {'X', 'O', '3', 'O', '5', '6', '7', '8', 'X'},
        {'X', 'O', '3', '4', '5', 'O', '7', '8', 'X'},
        {'X', 'O', '3', '4', '5', '6', 'O', '8', 'X'},
        {'X', 'O', '3', '4', '5', '6', '7', 'O', 'X'},
        {'X', 'O', '3', '4', '5', '6', '7', '8', 'X'},
        {'X', 'X', 'O', 'O', '5', 'X', 'O', 'X', 'O'},
        {'X', 'X', 'O', 'O', '5', 'X', 'O', 'X', '9'},
        {'X', 'X', 'O', 'O', '5', 'X', 'O', 'O', 'X'},
        {'X', 'X', 'O', 'O', '5', 'X', 'O', '8', 'X'},
        {'X', 'X', 'O', 'O', '5', 'X', 'O', '8', '9'},
        {'X', 'X', 'O', 'O', '5', 'X', 'X', 'O', 'O'},
        {'X', 'X', 'O', 'O', '5', 'X', 'X', 'O', '9'},
        {'X', 'X', 'O', 'O', '5', 'X', '7', 'O', 'X'},
        {'X', 'X', 'O', 'O', '5', 'X', '7', 'O', '9'},
        {'X', 'X', 'O', 'O', '5', 'X', 'X', '8', 'O'},
        {'X', 'X', 'O', 'O', '5', 'X', '7', 'X', 'O'},
        {'X', 'X', 'O', 'O', '5', 'X', '7', '8', 'O'},
        {'X', 'X', 'O', 'O', '5', 'X', '7', '8', '9'},
        {'X', 'X', 'O', 'O', '5', 'O', 'X', 'X', '9'},
        {'X', 'X', 'O', 'O', '5', 'O', 'X', 'O', 'X'},
        {'X', 'X', 'O', 'O', '5', 'O', 'X', '8', 'X'},
        {'X', 'X', 'O', 'O', '5', 'O', 'X', '8', '9'},
        {'X', 'X', 'O', 'O', '5', '6', 'X', 'O', 'X'},
        {'X', 'X', 'O', 'O', '5', '6', 'X', 'O', '9'},
        {'X', 'X', 'O', 'O', '5', 'O', 'O', 'X', 'X'},
        {'X', 'X', 'O', 'O', '5', 'O', '7', 'X', 'X'},
        {'X', 'X', 'O', 'O', '5', 'O', '7', 'X', '9'},
        {'X', 'X', 'O', 'O', '5', '6', 'O', 'X', 'X'},
        {'X', 'X', 'O', 'O', '5', '6', 'O', 'X', '9'},
        {'X', 'X', 'O', 'O', '5', '6', '7', 'X', 'O'},
        {'X', 'X', 'O', 'O', '5', '6', '7', 'X', '9'},
        {'X', 'X', 'O', 'O', '5', 'O', '7', '8', 'X'},
        {'X', 'X', 'O', 'O', '5', '6', 'O', '8', 'X'},
        {'X', 'X', 'O', 'O', '5', '6', '7', 'O', 'X'},
        {'X', 'X', 'O', 'O', '5', '6', '7', '8', 'X'},
        {'X', 'X', 'O', 'O', '5', '6', '7', '8', '9'},
        {'X', 'X', 'O', 'X', '5', 'O', 'O', 'X', '9'},
        {'X', 'X', 'O', 'X', '5', 'O', 'O', 'O', 'X'},
        {'X', 'X', 'O', 'X', '5', 'O', 'O', '8', 'X'},
        {'X', 'X', 'O', 'X', '5', 'O', 'O', '8', '9'},
        {'X', 'X', 'O', 'X', '5', 'O', '7', 'O', 'X'},
        {'X', 'X', 'O', '4', '5', 'O', 'O', 'X', 'X'},
        {'X', 'X', 'O', '4', '5', 'O', 'O', 'X', '9'},
        {'X', 'X', 'O', '4', '5', 'O', 'O', '8', 'X'},
        {'X', 'X', 'O', '4', '5', 'O', '7', 'O', 'X'},
        {'X', 'X', 'O', '4', '5', 'O', '7', '8', 'X'},
        {'X', 'X', 'O', 'X', '5', 'X', 'O', 'O', '9'},
        {'X', 'X', 'O', 'X', '5', '6', 'O', 'O', 'X'},
        {'X', 'X', 'O', 'X', '5', '6', 'O', 'O', '9'},
        {'X', 'X', 'O', 'X', '5', 'X', 'O', '8', 'O'},
        {'X', 'X', 'O', 'X', '5', '6', 'O', 'X', 'O'},
        {'X', 'X', 'O', 'X', '5', '6', 'O', '8', 'O'},
        {'X', 'X', 'O', 'X', '5', '6', 'O', '8', '9'},
        {'X', 'X', 'O', '4', '5', 'X', 'O', 'O', 'X'},
        {'X', 'X', 'O', '4', '5', 'X', 'O', 'X', 'O'},
        {'X', 'X', 'O', '4', '5', 'X', 'O', '8', '9'},
        {'X', 'X', 'O', '4', '5', '6', 'O', 'X', 'O'},
        {'X', 'X', 'O', '4', '5', '6', 'O', 'X', '9'},
        {'X', 'X', 'O', '4', '5', '6', 'O', 'O', 'X'},
        {'X', 'X', 'O', '4', '5', '6', 'O', '8', 'X'},
        {'X', 'X', 'O', '4', '5', '6', 'O', '8', '9'},
        {'X', 'X', 'O', '4', '5', '6', '7', 'O', 'X'},
        {'X', '2', 'O', 'X', '5', 'O', 'O', 'X', 'X'},
        {'X', '2', 'O', 'X', '5', 'O', 'O', '8', 'X'},
        {'X', '2', 'O', 'X', '5', 'O', '7', 'O', 'X'},
        {'X', '2', 'O', 'X', '5', 'X', 'O', 'O', 'X'},
        {'X', '2', 'O', 'X', '5', 'X', 'O', 'O', '9'},
        {'X', '2', 'O', 'X', '5', 'X', 'O', 'X', 'O'},
        {'X', '2', 'O', 'X', '5', 'X', 'O', '8', 'O'},
        {'X', '2', 'O', 'X', '5', 'X', 'O', '8', '9'},
        {'X', '2', 'O', 'X', '5', '6', 'O', 'X', '9'},
        {'X', '2', 'O', 'X', '5', '6', 'O', 'O', 'X'},
        {'X', '2', 'O', 'X', '5', '6', 'O', '8', 'X'},
        {'X', '2', 'O', 'X', '5', '6', 'O', '8', '9'},
        {'X', '2', 'O', 'X', '5', 'X', '7', 'O', 'O'},
        {'X', '2', 'O', 'O', '5', 'X', 'X', 'O', 'X'},
        {'X', '2', 'O', 'O', '5', 'X', 'X', '8', '9'},
        {'X', '2', 'O', 'O', '5', 'X', 'O', 'X', 'X'},
        {'X', '2', 'O', 'O', '5', 'X', 'O', 'X', '9'},
        {'X', '2', 'O', 'O', '5', 'X', '7', 'X', '9'},
        {'X', '2', 'O', 'O', '5', 'X', 'O', '8', 'X'},
        {'X', '2', 'O', 'O', '5', 'X', '7', 'O', 'X'},
        {'X', '2', 'O', 'O', '5', 'X', '7', '8', 'X'},
        {'X', '2', 'O', '4', '5', 'X', 'O', 'X', 'O'},
        {'X', '2', 'O', '4', '5', 'X', 'O', 'X', '9'},
        {'X', '2', 'O', '4', '5', 'X', 'O', 'O', 'X'},
        {'X', '2', 'O', '4', '5', 'X', 'O', '8', 'X'},
        {'X', '2', 'O', '4', '5', 'X', 'O', '8', '9'},
        {'X', '2', 'O', '4', '5', 'X', '7', 'O', 'X'},
        {'X', '2', 'O', 'O', '5', 'O', 'X', '8', 'X'},
        {'X', '2', 'O', 'O', '5', '6', 'X', 'O', 'X'},
        {'X', '2', 'O', 'O', '5', 'O', '7', 'X', 'X'},
        {'X', '2', 'O', 'O', '5', '6', 'O', 'X', 'X'},
        {'X', '2', 'O', 'O', '5', '6', '7', 'X', '9'},
        {'X', '2', 'O', '4', '5', 'O', 'O', 'X', 'X'},
        {'X', '2', 'O', '4', '5', '6', 'O', 'X', 'X'},
        {'X', '2', 'O', '4', '5', '6', 'O', 'X', '9'},
        {'X', '2', 'O', 'O', '5', '6', '7', '8', 'X'},
        {'X', '2', 'O', '4', '5', 'O', '7', '8', 'X'},
        {'X', '2', 'O', '4', '5', '6', 'O', '8', 'X'},
        {'X', '2', 'O', '4', '5', '6', '7', 'O', 'X'},
        {'X', 'X', '3', 'O', '5', 'O', 'X', 'O', 'X'},
        {'X', 'X', '3', 'O', '5', 'O', 'X', '8', '9'},
        {'X', 'X', '3', 'O', '5', 'O', 'O', 'X', 'X'},
        {'X', 'X', '3', 'O', '5', 'O', '7', 'X', '9'},
        {'X', 'X', '3', 'O', '5', 'O', '7', '8', 'X'},
        {'X', '2', 'X', 'O', '5', 'O', 'X', 'O', 'X'},
        {'X', '2', 'X', 'O', '5', 'O', 'X', 'X', 'O'},
        {'X', '2', 'X', 'O', '5', 'O', 'X', '8', '9'},
        {'X', '2', 'X', 'O', '5', 'O', 'O', 'X', 'X'},
        {'X', '2', 'X', 'O', '5', 'O', '7', 'X', '9'},
        {'X', '2', 'X', 'O', '5', 'O', '7', '8', 'X'},
        {'X', '2', '3', 'O', '5', 'O', 'X', 'X', '9'},
        {'X', '2', '3', 'O', '5', 'O', 'X', 'O', 'X'},
        {'X', '2', '3', 'O', '5', 'O', 'X', '8', 'X'},
        {'X', '2', '3', 'O', '5', 'O', 'X', '8', '9'},
        {'X', '2', '3', 'O', '5', '6', 'X', 'O', 'X'},
        {'X', '2', '3', 'O', '5', '6', 'X', '8', '9'},
        {'X', '2', '3', 'O', '5', 'O', 'O', 'X', 'X'},
        {'X', '2', '3', 'O', '5', 'O', '7', 'X', 'X'},
        {'X', '2', '3', 'O', '5', 'O', '7', 'X', '9'},
        {'X', '2', '3', 'O', '5', '6', 'O', 'X', 'X'},
        {'X', '2', '3', 'O', '5', '6', '7', 'X', '9'},
        {'X', '2', '3', 'O', '5', 'O', '7', '8', 'X'},
        {'X', '2', '3', 'O', '5', '6', 'O', '8', 'X'},
        {'X', '2', '3', 'O', '5', '6', '7', 'O', 'X'},
        {'X', '2', '3', 'O', '5', '6', '7', '8', 'X'},
        {'X', '2', '3', 'X', '5', 'O', 'O', 'O', 'X'},
        {'X', '2', '3', 'X', '5', 'O', 'O', '8', 'X'},
        {'X', '2', '3', '4', '5', 'O', 'O', 'X', 'X'},
        {'X', '2', '3', '4', '5', 'O', '7', 'X', '9'},
        {'X', '2', '3', '4', '5', 'O', 'O', '8', 'X'},
        {'X', '2', '3', '4', '5', 'O', '7', 'O', 'X'},
        {'X', '2', '3', '4', '5', 'O', '7', '8', 'X'},
        {'X', '2', '3', 'X', '5', '6', 'O', 'O', 'X'},
        {'X', '2', '3', '4', '5', '6', 'O', 'O', 'X'},
        {'X', '2', '3', '4', '5', 'X', '7', 'O', '9'},
        {'X', '2', '3', '4', '5', '6', '7', 'O', 'X'},
        {'X', '2', '3', '4', '5', '6', '7', '8', '9'},
        {'O', 'X', 'X', 'O', '5', 'X', 'X', 'O', 'O'},
        {'O', 'X', 'X', 'O', '5', 'X', 'X', 'O', '9'},
        {'O', 'X', 'X', 'O', '5', 'X', 'X', '8', 'O'},
        {'O', 'X', 'X', 'O', '5', 'X', '7', 'X', 'O'},
        {'O', 'X', 'X', 'O', '5', 'X', '7', '8', 'O'},
        {'O', 'X', 'X', 'O', '5', 'O', 'X', 'X', 'O'},
        {'O', 'X', 'X', 'O', '5', 'O', 'X', 'X', '9'},
        {'O', 'X', 'X', 'O', '5', 'O', 'X', 'O', 'X'},
        {'O', 'X', 'X', 'O', '5', 'O', 'X', '8', 'X'},
        {'O', 'X', 'X', 'O', '5', 'O', 'X', '8', '9'},
        {'O', 'X', 'X', 'O', '5', '6', 'X', 'O', 'X'},
        {'O', 'X', 'X', 'O', '5', '6', 'X', 'O', '9'},
        {'O', 'X', 'X', 'O', '5', '6', 'X', 'X', 'O'},
        {'O', 'X', 'X', 'O', '5', '6', 'X', '8', 'O'},
        {'O', 'X', 'X', 'O', '5', '6', 'X', '8', '9'},
        {'O', 'X', 'X', 'O', '5', 'O', '7', 'X', 'X'},
        {'O', 'X', 'X', 'O', '5', 'O', '7', 'X', '9'},
        {'O', 'X', 'X', 'O', '5', '6', '7', 'X', 'O'},
        {'O', 'X', 'X', 'O', '5', 'O', '7', '8', 'X'},
        {'O', 'X', 'X', 'O', '5', '6', '7', '8', '9'},
        {'O', 'X', 'X', 'X', '5', 'O', 'O', 'X', 'O'},
        {'O', 'X', 'X', 'X', '5', 'O', 'O', 'X', '9'},
        {'O', 'X', 'X', 'X', '5', 'O', 'O', 'O', 'X'},
        {'O', 'X', 'X', 'X', '5', 'O', 'O', '8', 'X'},
        {'O', 'X', 'X', 'X', '5', 'O', 'O', '8', '9'},
        {'O', 'X', 'X', 'X', '5', 'O', 'X', 'O', 'O'},
        {'O', 'X', 'X', 'X', '5', 'O', 'X', 'O', '9'},
        {'O', 'X', 'X', 'X', '5', 'O', '7', 'O', 'X'},
        {'O', 'X', 'X', 'X', '5', 'O', '7', 'O', '9'},
        {'O', 'X', 'X', 'X', '5', 'O', 'X', '8', 'O'},
        {'O', 'X', 'X', 'X', '5', 'O', '7', 'X', 'O'},
        {'O', 'X', 'X', 'X', '5', 'O', '7', '8', 'O'},
        {'O', 'X', 'X', 'X', '5', 'O', '7', '8', '9'},
        {'O', 'X', 'X', '4', '5', 'O', 'X', 'O', 'X'},
        {'O', 'X', 'X', '4', '5', 'O', 'X', 'O', '9'},
        {'O', 'X', 'X', '4', '5', 'O', 'X', 'X', 'O'},
        {'O', 'X', 'X', '4', '5', 'O', 'X', '8', 'O'},
        {'O', 'X', 'X', '4', '5', 'O', 'X', '8', '9'},
        {'O', 'X', 'X', '4', '5', 'O', 'O', 'X', '9'},
        {'O', 'X', 'X', '4', '5', 'O', '7', 'X', 'O'},
        {'O', 'X', 'X', '4', '5', 'O', '7', 'X', '9'},
        {'O', 'X', 'X', '4', '5', 'O', '7', '8', '9'},
        {'O', 'X', 'X', 'X', '5', 'X', 'O', '8', 'O'},
        {'O', 'X', 'X', 'X', '5', '6', 'O', 'X', 'O'},
        {'O', 'X', 'X', 'X', '5', '6', 'O', '8', 'O'},
        {'O', 'X', 'X', '4', '5', '6', 'O', 'X', 'O'},
        {'O', 'X', 'X', 'X', '5', 'X', '7', 'O', 'O'},
        {'O', 'X', 'X', 'X', '5', '6', 'X', 'O', 'O'},
        {'O', 'X', 'X', 'X', '5', '6', '7', 'O', 'O'},
        {'O', 'X', 'X', '4', '5', 'X', 'X', 'O', 'O'},
        {'O', 'X', 'X', '4', '5', '6', 'X', 'O', 'O'},
        {'O', 'X', 'X', '4', '5', '6', 'X', 'O', '9'},
        {'O', 'X', 'X', 'X', '5', '6', '7', '8', 'O'},
        {'O', 'X', 'X', '4', '5', 'X', '7', '8', 'O'},
        {'O', 'X', 'X', '4', '5', '6', 'X', '8', 'O'},
        {'O', 'X', 'X', '4', '5', '6', '7', 'X', 'O'},
        {'O', 'X', 'X', '4', '5', '6', '7', '8', 'O'},
        {'O', 'X', 'O', 'X', '5', 'X', 'O', 'X', 'O'},
        {'O', 'X', 'O', 'X', '5', 'X', 'O', 'X', '9'},
        {'O', 'X', 'O', 'X', '5', 'X', 'O', 'O', 'X'},
        {'O', 'X', 'O', 'X', '5', 'X', 'O', '8', 'X'},
        {'O', 'X', 'O', 'X', '5', 'X', 'O', '8', '9'},
        {'O', 'X', 'O', 'X', '5', 'X', 'X', 'O', 'O'},
        {'O', 'X', 'O', 'X', '5', 'X', 'X', 'O', '9'},
        {'O', 'X', 'O', 'X', '5', 'X', '7', 'O', 'X'},
        {'O', 'X', 'O', 'X', '5', 'X', '7', 'O', '9'},
        {'O', 'X', 'O', 'X', '5', 'X', 'X', '8', 'O'},
        {'O', 'X', 'O', 'X', '5', 'X', '7', 'X', 'O'},
        {'O', 'X', 'O', 'X', '5', 'X', '7', '8', 'O'},
        {'O', 'X', 'O', 'X', '5', 'X', '7', '8', '9'},
        {'O', 'X', 'O', 'X', '5', 'O', 'X', 'O', 'X'},
        {'O', 'X', 'O', 'X', '5', '6', 'X', 'O', 'X'},
        {'O', 'X', 'O', 'X', '5', '6', 'X', 'O', '9'},
        {'O', 'X', 'O', 'X', '5', '6', 'X', 'X', 'O'},
        {'O', 'X', 'O', 'X', '5', '6', 'X', '8', 'O'},
        {'O', 'X', 'O', 'X', '5', 'O', 'O', 'X', 'X'},
        {'O', 'X', 'O', 'X', '5', 'O', '7', 'X', 'X'},
        {'O', 'X', 'O', 'X', '5', 'O', '7', 'X', '9'},
        {'O', 'X', 'O', 'X', '5', '6', 'O', 'X', 'X'},
        {'O', 'X', 'O', 'X', '5', '6', 'O', 'X', '9'},
        {'O', 'X', 'O', 'X', '5', '6', '7', 'X', 'O'},
        {'O', 'X', 'O', 'X', '5', '6', '7', 'X', '9'},
        {'O', 'X', 'O', 'X', '5', '6', 'O', '8', 'X'},
        {'O', 'X', 'O', 'X', '5', '6', '7', 'O', 'X'},
        {'O', 'X', 'O', 'X', '5', '6', '7', '8', 'X'},
        {'O', 'X', 'O', 'X', '5', '6', '7', '8', '9'},
        {'O', 'X', '3', 'X', '5', 'O', 'O', 'X', 'X'},
        {'O', 'X', '3', 'X', '5', 'O', 'O', 'X', '9'},
        {'O', 'X', '3', 'X', '5', 'O', '7', 'X', 'O'},
        {'O', 'X', '3', 'X', '5', 'O', '7', 'X', '9'},
        {'O', 'X', '3', 'X', '5', 'O', '7', '8', 'X'},
        {'O', 'X', '3', 'X', '5', 'X', 'O', 'O', '9'},
        {'O', 'X', '3', 'X', '5', 'X', 'O', 'X', 'O'},
        {'O', 'X', '3', 'X', '5', 'X', 'O', '8', 'O'},
        {'O', 'X', '3', 'X', '5', 'X', 'O', '8', '9'},
        {'O', 'X', '3', 'X', '5', '6', 'O', 'X', 'O'},
        {'O', 'X', '3', 'X', '5', '6', 'O', 'X', '9'},
        {'O', 'X', '3', 'X', '5', '6', 'O', '8', 'X'},
        {'O', 'X', '3', 'X', '5', '6', 'O', '8', '9'},
        {'O', 'X', '3', 'X', '5', 'X', 'X', 'O', 'O'},
        {'O', 'X', '3', 'X', '5', 'X', '7', 'O', 'O'},
        {'O', 'X', '3', 'X', '5', 'X', '7', 'O', '9'},
        {'O', 'X', '3', 'X', '5', '6', 'X', 'O', 'O'},
        {'O', 'X', '3', 'X', '5', '6', '7', 'O', '9'},
        {'O', 'X', '3', 'X', '5', 'X', '7', '8', 'O'},
        {'O', 'X', '3', 'X', '5', '6', 'X', '8', 'O'},
        {'O', 'X', '3', 'X', '5', '6', '7', 'X', 'O'},
        {'O', 'X', '3', 'X', '5', '6', '7', '8', 'O'},
        {'O', 'X', '3', 'X', '5', '6', '7', '8', '9'},
        {'O', 'X', 'O', 'O', '5', 'X', 'X', 'X', 'O'},
        {'O', 'X', 'O', 'O', '5', 'X', 'X', 'X', '9'},
        {'O', 'X', 'O', 'O', '5', 'X', 'X', 'O', 'X'},
        {'O', 'X', 'O', '4', '5', 'X', 'X', 'X', 'O'},
        {'O', 'X', 'O', '4', '5', 'X', 'X', '8', 'O'},
        {'O', 'X', 'O', '4', '5', 'X', 'X', '8', '9'},
        {'O', 'X', 'O', 'O', '5', 'X', '7', 'X', '9'},
        {'O', 'X', 'O', '4', '5', 'X', 'O', 'X', '9'},
        {'O', 'X', 'O', '4', '5', 'X', '7', 'X', 'O'},
        {'O', 'X', 'O', '4', '5', 'X', '7', 'X', '9'},
        {'O', 'X', 'O', '4', '5', 'X', '7', '8', '9'},
        {'O', 'X', '3', 'O', '5', 'X', 'X', 'X', 'O'},
        {'O', 'X', '3', 'O', '5', 'X', 'X', '8', 'O'},
        {'O', 'X', '3', 'O', '5', 'X', '7', 'X', 'O'},
        {'O', 'X', '3', '4', '5', 'X', 'O', 'X', 'O'},
        {'O', 'X', '3', '4', '5', 'X', 'X', 'O', 'O'},
        {'O', 'X', '3', '4', '5', 'X', 'X', '8', 'O'},
        {'O', 'X', '3', '4', '5', 'X', '7', 'X', 'O'},
        {'O', 'X', '3', '4', '5', 'X', '7', '8', 'O'},
        {'O', 'X', 'O', 'O', '5', '6', 'X', 'X', '9'},
        {'O', 'X', 'O', '4', '5', 'O', 'X', 'X', '9'},
        {'O', 'X', 'O', '4', '5', '6', 'X', 'X', 'O'},
        {'O', 'X', '3', 'O', '5', 'O', 'X', 'X', '9'},
        {'O', 'X', '3', 'O', '5', '6', 'X', 'X', 'O'},
        {'O', 'X', '3', 'O', '5', '6', 'X', '8', '9'},
        {'O', 'X', '3', '4', '5', 'O', 'X', 'X', 'O'},
        {'O', 'X', '3', '4', '5', 'O', 'X', '8', '9'},
        {'O', 'X', '3', '4', '5', '6', 'X', 'X', 'O'},
        {'O', 'X', '3', '4', '5', '6', 'X', '8', 'O'},
        {'O', 'X', '3', '4', '5', '6', 'X', '8', '9'},
        {'O', 'X', 'O', 'O', '5', '6', '7', 'X', 'X'},
        {'O', 'X', 'O', '4', '5', 'O', '7', 'X', 'X'},
        {'O', 'X', 'O', '4', '5', '6', 'O', 'X', 'X'},
        {'O', 'X', 'O', '4', '5', '6', '7', 'X', '9'},
        {'O', 'X', '3', 'O', '5', 'O', '7', 'X', 'X'},
        {'O', 'X', '3', 'O', '5', '6', '7', 'X', '9'},
        {'O', 'X', '3', '4', '5', 'O', 'O', 'X', 'X'},
        {'O', 'X', '3', '4', '5', 'O', '7', 'X', '9'},
        {'O', 'X', '3', '4', '5', '6', 'O', 'X', '9'},
        {'O', 'X', '3', '4', '5', '6', '7', 'X', 'O'},
        {'O', 'X', '3', '4', '5', '6', '7', 'X', '9'},
        {'O', 'X', '3', '4', '5', '6', '7', '8', 'X'},
        {'1', 'X', 'O', 'X', '5', 'O', 'O', 'X', 'X'},
        {'1', 'X', 'O', 'X', '5', 'O', 'O', 'X', '9'},
        {'1', 'X', 'O', 'X', '5', 'O', 'O', '8', 'X'},
        {'1', 'X', 'O', 'X', '5', 'X', 'O', 'O', 'X'},
        {'1', 'X', 'O', 'X', '5', 'X', 'O', 'O', '9'},
        {'1', 'X', 'O', 'X', '5', 'X', 'O', 'X', 'O'},
        {'1', 'X', 'O', 'X', '5', 'X', 'O', '8', 'O'},
        {'1', 'X', 'O', 'X', '5', 'X', 'O', '8', '9'},
        {'1', 'X', 'O', 'X', '5', '6', 'O', 'X', 'O'},
        {'1', 'X', 'O', 'X', '5', '6', 'O', 'X', '9'},
        {'1', 'X', 'O', 'X', '5', '6', 'O', 'O', 'X'},
        {'1', 'X', 'O', 'X', '5', '6', 'O', '8', 'X'},
        {'1', 'X', 'O', 'X', '5', '6', 'O', '8', '9'},
        {'1', 'X', 'O', 'X', '5', 'X', '7', 'O', 'O'},
        {'1', 'X', 'O', 'X', '5', 'X', '7', 'O', '9'},
        {'1', 'X', 'O', 'X', '5', '6', '7', 'O', '9'},
        {'1', 'X', 'O', 'X', '5', 'X', '7', '8', 'O'},
        {'1', 'X', 'O', 'O', '5', 'X', 'X', 'X', 'O'},
        {'1', 'X', 'O', 'O', '5', 'X', 'X', '8', '9'},
        {'1', 'X', 'O', 'O', '5', 'X', 'O', 'X', '9'},
        {'1', 'X', 'O', 'O', '5', 'X', '7', 'X', 'O'},
        {'1', 'X', 'O', 'O', '5', 'X', '7', 'X', '9'},
        {'1', 'X', 'O', '4', '5', 'X', 'O', 'X', 'O'},
        {'1', 'X', 'O', '4', '5', 'X', 'O', 'X', '9'},
        {'1', 'X', 'O', '4', '5', 'X', 'O', 'O', 'X'},
        {'1', 'X', 'O', '4', '5', 'X', 'O', '8', 'X'},
        {'1', 'X', 'O', '4', '5', 'X', 'O', '8', '9'},
        {'1', 'X', 'O', '4', '5', 'X', '7', 'X', 'O'},
        {'1', 'X', 'O', '4', '5', 'X', '7', '8', 'O'},
        {'1', 'X', 'O', 'O', '5', 'O', 'X', 'X', '9'},
        {'1', 'X', 'O', 'O', '5', '6', 'X', 'X', 'O'},
        {'1', 'X', 'O', '4', '5', '6', 'X', '8', '9'},
        {'1', 'X', 'O', 'O', '5', 'O', '7', 'X', 'X'},
        {'1', 'X', 'O', 'O', '5', '6', 'O', 'X', 'X'},
        {'1', 'X', 'O', 'O', '5', '6', '7', 'X', '9'},
        {'1', 'X', 'O', '4', '5', 'O', 'O', 'X', 'X'},
        {'1', 'X', 'O', '4', '5', 'O', '7', 'X', '9'},
        {'1', 'X', 'O', '4', '5', '6', 'O', 'X', 'X'},
        {'1', 'X', 'O', '4', '5', '6', 'O', 'X', '9'},
        {'1', 'X', 'O', '4', '5', '6', '7', 'X', 'O'},
        {'1', 'X', 'O', '4', '5', '6', '7', 'X', '9'},
        {'1', 'X', 'O', 'O', '5', '6', '7', '8', 'X'},
        {'1', 'X', 'O', '4', '5', '6', 'O', '8', 'X'},
        {'1', 'X', 'O', '4', '5', '6', '7', '8', 'X'},
        {'1', 'X', 'O', '4', '5', '6', '7', '8', '9'},
        {'1', 'X', 'X', 'O', '5', 'O', 'X', 'O', 'X'},
        {'1', 'X', 'X', 'O', '5', 'O', 'X', 'X', 'O'},
        {'1', 'X', 'X', 'O', '5', 'O', 'X', '8', '9'},
        {'1', 'X', 'X', 'O', '5', 'O', '7', 'X', '9'},
        {'1', 'X', 'X', 'O', '5', 'O', '7', '8', 'X'},
        {'1', 'X', '3', 'O', '5', 'X', 'O', 'X', 'O'},
        {'1', 'X', '3', 'O', '5', 'X', '7', 'X', 'O'},
        {'1', 'X', '3', 'O', '5', 'O', 'X', 'X', 'O'},
        {'1', 'X', '3', 'O', '5', 'O', 'X', 'X', '9'},
        {'1', 'X', '3', 'O', '5', 'O', 'X', 'O', 'X'},
        {'1', 'X', '3', 'O', '5', 'O', 'X', '8', 'X'},
        {'1', 'X', '3', 'O', '5', 'O', 'X', '8', '9'},
        {'1', 'X', '3', 'O', '5', '6', 'X', 'X', 'O'},
        {'1', 'X', '3', 'O', '5', '6', 'X', '8', '9'},
        {'1', 'X', '3', 'O', '5', 'O', 'O', 'X', 'X'},
        {'1', 'X', '3', 'O', '5', 'O', '7', 'X', 'X'},
        {'1', 'X', '3', 'O', '5', 'O', '7', 'X', '9'},
        {'1', 'X', '3', 'O', '5', '6', 'O', 'X', '9'},
        {'1', 'X', '3', 'O', '5', '6', '7', 'X', 'O'},
        {'1', 'X', '3', 'O', '5', '6', '7', 'X', '9'},
        {'1', 'X', '3', 'O', '5', 'O', '7', '8', 'X'},
        {'1', 'X', '3', 'O', '5', '6', '7', '8', 'X'},
        {'1', 'X', '3', 'X', '5', 'O', 'O', 'X', 'O'},
        {'1', 'X', '3', 'X', '5', 'O', 'O', 'X', '9'},
        {'1', 'X', '3', '4', '5', 'O', 'X', '8', '9'},
        {'1', 'X', '3', '4', '5', 'O', 'O', 'X', 'X'},
        {'1', 'X', '3', '4', '5', 'O', 'O', 'X', '9'},
        {'1', 'X', '3', '4', '5', 'O', '7', 'X', 'O'},
        {'1', 'X', '3', '4', '5', 'O', '7', 'X', '9'},
        {'1', 'X', '3', '4', '5', 'O', '7', '8', 'X'},
        {'1', 'X', '3', 'X', '5', '6', 'O', 'X', 'O'},
        {'1', 'X', '3', '4', '5', 'X', 'O', 'X', 'O'},
        {'1', 'X', '3', '4', '5', '6', 'O', 'X', 'O'},
        {'1', 'X', '3', '4', '5', '6', 'O', 'X', '9'},
        {'1', 'X', '3', '4', '5', '6', '7', 'X', 'O'},
        {'O', 'O', 'X', 'X', '5', 'X', 'O', 'X', 'O'},
        {'O', 'O', 'X', 'X', '5', 'X', 'O', 'X', '9'},
        {'O', 'O', 'X', 'X', '5', 'X', 'O', '8', '9'},
        {'O', 'O', 'X', 'X', '5', 'X', 'X', 'O', 'O'},
        {'O', 'O', 'X', 'X', '5', 'X', 'X', 'O', '9'},
        {'O', 'O', 'X', 'X', '5', 'X', '7', 'O', '9'},
        {'O', 'O', 'X', 'X', '5', 'X', 'X', '8', 'O'},
        {'O', 'O', 'X', 'X', '5', 'X', '7', 'X', 'O'},
        {'O', 'O', 'X', 'X', '5', 'X', '7', '8', 'O'},
        {'O', 'O', 'X', 'X', '5', 'X', '7', '8', '9'},
        {'O', 'O', 'X', 'X', '5', 'O', 'X', 'X', 'O'},
        {'O', 'O', 'X', 'X', '5', 'O', 'X', 'X', '9'},
        {'O', 'O', 'X', 'X', '5', 'O', 'X', 'O', 'X'},
        {'O', 'O', 'X', 'X', '5', 'O', 'X', '8', 'X'},
        {'O', 'O', 'X', 'X', '5', 'O', 'X', '8', '9'},
        {'O', 'O', 'X', 'X', '5', '6', 'X', 'O', 'X'},
        {'O', 'O', 'X', 'X', '5', '6', 'X', 'O', '9'},
        {'O', 'O', 'X', 'X', '5', '6', 'X', 'X', 'O'},
        {'O', 'O', 'X', 'X', '5', '6', 'X', '8', 'O'},
        {'O', 'O', 'X', 'X', '5', '6', 'X', '8', '9'},
        {'O', 'O', 'X', 'X', '5', 'O', 'O', 'X', 'X'},
        {'O', 'O', 'X', 'X', '5', '6', '7', 'X', 'O'},
        {'O', 'O', 'X', 'X', '5', '6', '7', 'X', '9'},
        {'O', 'O', 'X', 'X', '5', '6', '7', '8', 'X'},
        {'O', 'O', 'X', 'X', '5', '6', '7', '8', '9'},
        {'O', '2', 'X', 'X', '5', 'O', 'X', 'O', 'X'},
        {'O', '2', 'X', 'X', '5', 'O', 'X', 'O', '9'},
        {'O', '2', 'X', 'X', '5', 'O', 'X', 'X', 'O'},
        {'O', '2', 'X', 'X', '5', 'O', 'X', '8', 'O'},
        {'O', '2', 'X', 'X', '5', 'O', 'X', '8', '9'},
        {'O', '2', 'X', 'X', '5', 'O', '7', 'X', 'O'},
        {'O', '2', 'X', 'X', '5', 'O', '7', 'X', '9'},
        {'O', '2', 'X', 'X', '5', 'O', '7', '8', 'X'},
        {'O', '2', 'X', 'X', '5', 'X', 'O', 'O', '9'},
        {'O', '2', 'X', 'X', '5', 'X', 'O', 'X', 'O'},
        {'O', '2', 'X', 'X', '5', 'X', 'O', '8', 'O'},
        {'O', '2', 'X', 'X', '5', '6', 'O', 'X', 'O'},
        {'O', '2', 'X', 'X', '5', '6', 'O', 'X', '9'},
        {'O', '2', 'X', 'X', '5', 'X', 'X', 'O', 'O'},
        {'O', '2', 'X', 'X', '5', 'X', '7', 'O', 'O'},
        {'O', '2', 'X', 'X', '5', '6', 'X', 'O', 'O'},
        {'O', '2', 'X', 'X', '5', '6', 'X', 'O', '9'},
        {'O', '2', 'X', 'X', '5', '6', '7', 'O', '9'},
        {'O', '2', 'X', 'X', '5', 'X', '7', '8', 'O'},
        {'O', '2', 'X', 'X', '5', '6', 'X', '8', 'O'},
        {'O', '2', 'X', 'X', '5', '6', '7', 'X', 'O'},
        {'O', '2', 'X', 'X', '5', '6', '7', '8', 'O'},
        {'O', '2', 'X', 'X', '5', '6', '7', '8', '9'},
        {'O', 'O', 'X', 'O', '5', 'X', 'X', 'X', 'O'},
        {'O', 'O', 'X', 'O', '5', 'X', 'X', 'X', '9'},
        {'O', 'O', 'X', 'O', '5', 'X', 'X', '8', '9'},
        {'O', 'O', 'X', '4', '5', 'X', 'X', 'O', '9'},
        {'O', 'O', 'X', '4', '5', 'X', 'X', 'X', 'O'},
        {'O', 'O', 'X', '4', '5', 'X', 'X', '8', 'O'},
        {'O', 'O', 'X', '4', '5', 'X', '7', 'X', 'O'},
        {'O', '2', 'X', 'O', '5', 'X', 'X', 'O', '9'},
        {'O', '2', 'X', 'O', '5', 'X', 'X', 'X', 'O'},
        {'O', '2', 'X', 'O', '5', 'X', 'X', '8', 'O'},
        {'O', '2', 'X', '4', '5', 'X', 'X', 'O', 'O'},
        {'O', '2', 'X', '4', '5', 'X', 'X', '8', 'O'},
        {'O', '2', 'X', '4', '5', 'X', '7', 'X', 'O'},
        {'O', '2', 'X', '4', '5', 'X', '7', '8', 'O'},
        {'O', 'O', 'X', 'O', '5', '6', 'X', 'X', '9'},
        {'O', 'O', 'X', '4', '5', 'O', 'X', 'X', '9'},
        {'O', 'O', 'X', '4', '5', '6', 'X', 'X', 'O'},
        {'O', 'O', 'X', 'O', '5', '6', 'X', '8', 'X'},
        {'O', 'O', 'X', '4', '5', 'O', 'X', '8', 'X'},
        {'O', 'O', 'X', '4', '5', '6', 'X', 'O', 'X'},
        {'O', 'O', 'X', '4', '5', '6', 'X', '8', '9'},
        {'O', '2', 'X', 'O', '5', 'O', 'X', 'X', '9'},
        {'O', '2', 'X', 'O', '5', '6', 'X', 'X', 'O'},
        {'O', '2', 'X', 'O', '5', 'O', 'X', '8', 'X'},
        {'O', '2', 'X', 'O', '5', '6', 'X', 'O', 'X'},
        {'O', '2', 'X', 'O', '5', '6', 'X', '8', '9'},
        {'O', '2', 'X', '4', '5', 'O', 'X', 'X', 'O'},
        {'O', '2', 'X', '4', '5', 'O', 'X', 'O', 'X'},
        {'O', '2', 'X', '4', '5', 'O', 'X', '8', '9'},
        {'O', '2', 'X', '4', '5', '6', 'X', 'O', '9'},
        {'O', '2', 'X', '4', '5', '6', 'X', 'X', 'O'},
        {'O', '2', 'X', '4', '5', '6', 'X', '8', 'O'},
        {'O', '2', 'X', '4', '5', 'O', '7', 'X', '9'},
        {'O', '2', 'X', '4', '5', '6', '7', 'X', 'O'},
        {'1', 'O', 'X', 'X', '5', 'O', 'X', 'O', 'X'},
        {'1', 'O', 'X', 'X', '5', 'O', '7', 'O', 'X'},
        {'1', 'O', 'X', 'X', '5', 'O', '7', '8', 'X'},
        {'1', 'O', 'X', 'X', '5', 'X', 'O', 'O', '9'},
        {'1', 'O', 'X', 'X', '5', 'X', 'O', 'X', 'O'},
        {'1', 'O', 'X', 'X', '5', 'X', 'O', '8', 'O'},
        {'1', 'O', 'X', 'X', '5', '6', 'O', 'X', '9'},
        {'1', 'O', 'X', 'X', '5', 'X', 'X', 'O', 'O'},
        {'1', 'O', 'X', 'X', '5', 'X', '7', 'O', 'O'},
        {'1', 'O', 'X', 'X', '5', 'X', '7', 'O', '9'},
        {'1', 'O', 'X', 'X', '5', '6', 'X', 'O', '9'},
        {'1', 'O', 'X', 'X', '5', '6', '7', 'O', 'X'},
        {'1', 'O', 'X', 'X', '5', '6', '7', 'O', '9'},
        {'1', 'O', 'X', 'X', '5', 'X', '7', '8', 'O'},
        {'1', 'O', 'X', 'X', '5', '6', '7', 'X', 'O'},
        {'1', 'O', 'X', 'X', '5', '6', '7', '8', 'O'},
        {'1', 'O', 'X', 'X', '5', '6', '7', '8', '9'},
        {'1', 'O', 'X', 'O', '5', 'X', 'X', 'O', '9'},
        {'1', 'O', 'X', 'O', '5', 'X', 'X', 'X', 'O'},
        {'1', 'O', 'X', 'O', '5', 'X', 'X', '8', 'O'},
        {'1', 'O', 'X', '4', '5', 'X', 'X', 'O', 'O'},
        {'1', 'O', 'X', '4', '5', 'X', 'X', 'O', '9'},
        {'1', 'O', 'X', '4', '5', 'X', 'X', '8', 'O'},
        {'1', 'O', 'X', '4', '5', 'X', '7', '8', 'O'},
        {'1', 'O', 'X', 'O', '5', 'O', 'X', 'X', '9'},
        {'1', 'O', 'X', 'O', '5', '6', 'X', 'X', 'O'},
        {'1', 'O', 'X', 'O', '5', 'O', 'X', '8', 'X'},
        {'1', 'O', 'X', 'O', '5', '6', 'X', 'O', 'X'},
        {'1', 'O', 'X', 'O', '5', '6', 'X', '8', '9'},
        {'1', 'O', 'X', '4', '5', 'O', 'X', 'X', 'O'},
        {'1', 'O', 'X', '4', '5', 'O', 'X', 'O', 'X'},
        {'1', 'O', 'X', '4', '5', 'O', 'X', '8', '9'},
        {'1', 'O', 'X', '4', '5', '6', 'X', 'O', 'X'},
        {'1', 'O', 'X', '4', '5', '6', 'X', 'O', '9'},
        {'1', 'O', 'X', '4', '5', '6', 'X', 'X', 'O'},
        {'1', 'O', 'X', '4', '5', '6', 'X', '8', 'O'},
        {'1', 'O', 'X', '4', '5', '6', 'X', '8', '9'},
        {'1', 'O', 'X', '4', '5', 'O', '7', '8', 'X'},
        {'1', 'O', 'X', '4', '5', '6', '7', '8', '9'},
        {'1', '2', 'X', 'O', '5', 'X', 'X', 'O', 'O'},
        {'1', '2', 'X', 'O', '5', 'X', 'X', '8', 'O'},
        {'1', '2', 'X', 'O', '5', 'O', 'X', 'X', 'O'},
        {'1', '2', 'X', 'O', '5', 'O', 'X', 'X', '9'},
        {'1', '2', 'X', 'O', '5', 'O', 'X', 'O', 'X'},
        {'1', '2', 'X', 'O', '5', 'O', 'X', '8', 'X'},
        {'1', '2', 'X', 'O', '5', 'O', 'X', '8', '9'},
        {'1', '2', 'X', 'O', '5', '6', 'X', 'O', '9'},
        {'1', '2', 'X', 'O', '5', '6', 'X', 'X', 'O'},
        {'1', '2', 'X', 'O', '5', '6', 'X', '8', 'O'},
        {'1', '2', 'X', 'O', '5', '6', 'X', '8', '9'},
        {'1', '2', 'X', 'O', '5', 'O', '7', 'X', 'X'},
        {'1', '2', 'X', 'O', '5', 'O', '7', 'X', '9'},
        {'1', '2', 'X', 'O', '5', '6', '7', 'X', '9'},
        {'1', '2', 'X', 'O', '5', 'O', '7', '8', 'X'},
        {'1', '2', 'X', '4', '5', 'O', 'X', 'O', 'X'},
        {'1', '2', 'X', '4', '5', 'O', 'X', 'O', '9'},
        {'1', '2', 'X', '4', '5', 'O', 'X', 'X', 'O'},
        {'1', '2', 'X', '4', '5', 'O', 'X', '8', 'O'},
        {'1', '2', 'X', '4', '5', 'O', 'X', '8', '9'},
        {'1', '2', 'X', '4', '5', 'O', '7', 'X', '9'},
        {'1', '2', 'X', '4', '5', 'O', '7', '8', 'X'},
        {'1', '2', 'X', 'X', '5', '6', 'O', '8', '9'},
        {'1', '2', 'X', 'X', '5', '6', '7', 'O', '9'},
        {'1', '2', 'X', '4', '5', 'X', 'X', 'O', 'O'},
        {'1', '2', 'X', '4', '5', '6', 'X', 'O', 'O'},
        {'1', '2', 'X', '4', '5', '6', 'X', 'O', '9'},
        {'1', '2', 'X', '4', '5', '6', '7', '8', '9'},
        {'O', 'O', '3', 'X', '5', 'X', 'X', 'O', '9'},
        {'O', 'O', '3', 'X', '5', 'X', 'X', '8', 'O'},
        {'O', 'O', '3', 'X', '5', 'X', 'O', 'X', '9'},
        {'O', 'O', '3', 'X', '5', 'X', '7', 'X', 'O'},
        {'O', 'O', '3', 'X', '5', 'X', '7', '8', '9'},
        {'O', '2', 'O', 'X', '5', 'X', 'X', 'O', '9'},
        {'O', '2', 'O', 'X', '5', 'X', 'X', '8', 'O'},
        {'O', '2', 'O', 'X', '5', 'X', 'O', 'X', '9'},
        {'O', '2', 'O', 'X', '5', 'X', '7', 'X', 'O'},
        {'O', '2', 'O', 'X', '5', 'X', 'O', '8', 'X'},
        {'O', '2', 'O', 'X', '5', 'X', '7', 'O', 'X'},
        {'O', '2', 'O', 'X', '5', 'X', '7', '8', '9'},
        {'O', '2', '3', 'X', '5', 'X', 'O', 'X', 'O'},
        {'O', '2', '3', 'X', '5', 'X', 'O', 'X', '9'},
        {'O', '2', '3', 'X', '5', 'X', 'O', '8', '9'},
        {'O', '2', '3', 'X', '5', 'X', 'X', 'O', 'O'},
        {'O', '2', '3', 'X', '5', 'X', 'X', 'O', '9'},
        {'O', '2', '3', 'X', '5', 'X', '7', 'O', '9'},
        {'O', '2', '3', 'X', '5', 'X', 'X', '8', 'O'},
        {'O', '2', '3', 'X', '5', 'X', '7', 'X', 'O'},
        {'O', '2', '3', 'X', '5', 'X', '7', '8', 'O'},
        {'O', '2', '3', 'X', '5', 'X', '7', '8', '9'},
        {'O', '2', '3', 'X', '5', '6', 'X', 'O', '9'},
        {'O', '2', '3', 'X', '5', '6', 'X', 'X', 'O'},
        {'O', '2', '3', 'X', '5', '6', 'X', '8', 'O'},
        {'O', '2', '3', 'X', '5', '6', 'O', 'X', '9'},
        {'O', '2', '3', 'X', '5', '6', '7', 'X', 'O'},
        {'1', 'O', 'O', 'X', '5', 'X', 'O', 'X', '9'},
        {'1', 'O', 'O', 'X', '5', 'X', '7', 'X', 'O'},
        {'1', 'O', 'O', 'X', '5', 'X', 'O', '8', 'X'},
        {'1', 'O', 'O', 'X', '5', 'X', '7', 'O', 'X'},
        {'1', 'O', 'O', 'X', '5', 'X', '7', '8', '9'},
        {'1', 'O', '3', 'X', '5', 'X', 'O', 'X', 'O'},
        {'1', 'O', '3', 'X', '5', 'X', 'O', 'X', '9'},
        {'1', 'O', '3', 'X', '5', 'X', 'O', '8', '9'},
        {'1', 'O', '3', 'X', '5', 'X', 'X', 'O', '9'},
        {'1', 'O', '3', 'X', '5', 'X', '7', 'O', 'X'},
        {'1', 'O', '3', 'X', '5', 'X', '7', 'O', '9'},
        {'1', 'O', '3', 'X', '5', 'X', '7', 'X', 'O'},
        {'1', 'O', '3', 'X', '5', 'X', '7', '8', 'O'},
        {'1', 'O', '3', 'X', '5', 'X', '7', '8', '9'},
        {'1', 'O', '3', 'X', '5', '6', 'X', 'O', 'X'},
        {'1', 'O', '3', 'X', '5', '6', 'O', '8', 'X'},
        {'1', 'O', '3', 'X', '5', '6', '7', 'O', 'X'},
        {'1', 'O', '3', 'X', '5', '6', '7', '8', 'X'},
        {'1', '2', 'O', 'X', '5', 'X', 'O', 'X', 'O'},
        {'1', '2', 'O', 'X', '5', 'X', 'O', 'X', '9'},
        {'1', '2', 'O', 'X', '5', 'X', 'O', 'O', 'X'},
        {'1', '2', 'O', 'X', '5', 'X', 'O', '8', 'X'},
        {'1', '2', 'O', 'X', '5', 'X', 'O', '8', '9'},
        {'1', '2', 'O', 'X', '5', 'X', '7', 'O', 'X'},
        {'1', '2', 'O', 'X', '5', 'X', '7', 'O', '9'},
        {'1', '2', 'O', 'X', '5', 'X', '7', 'X', 'O'},
        {'1', '2', 'O', 'X', '5', 'X', '7', '8', 'O'},
        {'1', '2', 'O', 'X', '5', 'X', '7', '8', '9'},
        {'1', '2', 'O', 'X', '5', 'O', 'O', 'X', 'X'},
        {'1', '2', 'O', 'X', '5', '6', 'O', 'X', 'X'},
        {'1', '2', 'O', 'X', '5', '6', 'O', 'X', '9'},
        {'1', '2', 'O', 'X', '5', '6', 'O', '8', 'X'},
        {'1', '2', '3', 'X', '5', 'X', 'O', 'O', '9'},
        {'1', '2', '3', 'X', '5', 'X', 'O', 'X', 'O'},
        {'1', '2', '3', 'X', '5', 'X', 'O', '8', 'O'},
        {'1', '2', '3', 'X', '5', 'X', 'O', '8', '9'},
        {'1', '2', '3', 'X', '5', '6', 'O', 'X', 'O'},
        {'1', '2', '3', 'X', '5', '6', 'O', '8', 'X'},
        {'1', '2', '3', 'X', '5', 'X', '7', 'O', 'O'},
        {'1', '2', '3', 'X', '5', 'X', '7', 'O', '9'},
        {'1', '2', '3', 'X', '5', '6', '7', 'O', 'X'},
        {'1', '2', '3', 'X', '5', '6', '7', 'O', '9'},
        {'1', '2', '3', 'X', '5', 'X', '7', '8', 'O'},
        {'O', '2', '3', 'O', '5', 'X', 'X', 'X', 'O'},
        {'O', '2', '3', '4', '5', 'X', 'X', 'X', 'O'},
        {'O', '2', '3', '4', '5', 'X', 'X', '8', 'O'},
        {'O', '2', '3', '4', '5', 'X', '7', 'X', 'O'},
        {'1', 'O', '3', '4', '5', 'X', 'X', 'O', 'X'},
        {'1', 'O', '3', '4', '5', 'X', 'X', 'O', '9'},
        {'1', 'O', '3', '4', '5', 'X', 'X', '8', '9'},
        {'1', '2', 'O', '4', '5', 'X', 'O', 'X', 'X'},
        {'1', '2', 'O', '4', '5', 'X', 'O', 'X', '9'},
        {'1', '2', 'O', '4', '5', 'X', '7', 'X', 'O'},
        {'1', '2', 'O', '4', '5', 'X', 'O', '8', 'X'},
        {'1', '2', 'O', '4', '5', 'X', '7', 'O', 'X'},
        {'1', '2', '3', '4', '5', 'X', 'O', 'X', 'O'},
        {'1', '2', '3', '4', '5', 'X', 'X', 'O', '9'},
        {'1', '2', '3', '4', '5', 'X', '7', 'O', '9'},
        {'O', '2', '3', '4', '5', '6', 'X', 'X', 'O'},
        {'1', 'O', '3', '4', '5', '6', 'X', 'O', 'X'},
        {'1', '2', '3', 'O', '5', '6', 'X', 'X', 'O'},
        {'1', '2', '3', 'O', '5', '6', 'X', '8', '9'},
        {'1', '2', '3', '4', '5', '6', 'X', 'O', 'X'},
        {'1', '2', '3', '4', '5', '6', 'X', '8', '9'},
        {'1', '2', 'O', '4', '5', '6', 'O', 'X', 'X'},
        {'1', '2', '3', 'O', '5', '6', '7', 'X', '9'},
        {'1', '2', '3', '4', '5', 'O', 'O', 'X', 'X'},
        {'1', '2', '3', '4', '5', 'O', '7', 'X', '9'},
        {'1', '2', '3', '4', '5', 'O', '7', '8', 'X'},
        {'1', '2', '3', '4', '5', '6', '7', '8', 'X'}
    },
    {
        {'X', 'O', 'X', 'O', 'X', '6', 'O', 'X', 'O'},
        {'X', 'O', 'X', 'O', 'X', '6', '7', '8', '9'},
        {'X', 'O', 'X', 'O', 'O', '6', 'X', 'X', 'O'},
        {'X', 'O', 'X', 'O', 'O', '6', 'X', 'X', '9'},
        {'X', 'O', 'X', 'O', 'O', '6', 'X', '8', 'X'},
        {'X', 'O', 'X', 'O', 'O', '6', 'X', '8', '9'},
        {'X', 'O', 'X', 'O', 'O', '6', 'O', 'X', 'X'},
        {'X', 'O', 'X', 'O', 'O', '6', '7', 'X', 'X'},
        {'X', 'O', 'X', 'O', 'O', '6', '7', 'X', '9'},
        {'X', 'O', 'X', 'O', 'O', '6', '7', '8', 'X'},
        {'X', 'O', 'X', 'X', 'O', '6', 'O', 'X', 'O'},
        {'X', 'O', 'X', 'X', 'O', '6', 'O', 'X', '9'},
        {'X', 'O', 'X', '4', 'O', '6', 'O', 'X', 'X'},
        {'X', 'O', 'X', '4', 'O', '6', 'O', '8', 'X'},
        {'X', 'O', 'O', 'X', 'X', '6', 'O', 'X', 'O'},
        {'X', 'O', 'O', 'X', 'X', '6', 'O', 'X', '9'},
        {'X', 'O', 'O', 'X', 'X', '6', 'O', '8', '9'},
        {'X', 'O', 'O', 'X', 'X', '6', '7', 'O', '9'},
        {'X', 'O', 'O', 'X', 'X', '6', '7', 'X', 'O'},
        {'X', 'O', 'O', 'X', 'X', '6', '7', '8', 'O'},
        {'X', 'O', 'O', 'X', 'X', '6', '7', '8', '9'},
        {'X', 'O', '3', 'X', 'X', '6', 'O', 'O', '9'},
        {'X', 'O', '3', 'X', 'X', '6', 'O', 'X', 'O'},
        {'X', 'O', '3', 'X', 'X', '6', 'O', '8', 'O'},
        {'X', 'O', '3', 'X', 'X', '6', '7', 'O', 'O'},
        {'X', 'O', 'O', 'O', 'X', '6', 'X', 'X', 'O'},
        {'X', 'O', 'O', '4', 'X', '6', 'X', 'X', 'O'},
        {'X', 'O', 'O', '4', 'X', '6', '7', 'X', 'O'},
        {'X', 'X', 'O', 'O', 'X', '6', 'X', 'O', 'O'},
        {'X', 'X', 'O', 'O', 'X', '6', 'X', '8', 'O'},
        {'X', 'X', 'O', 'O', 'X', '6', '7', '8', '9'},
        {'X', 'X', 'O', 'O', 'O', '6', 'X', 'X', 'O'},
        {'X', 'X', 'O', 'O', 'O', '6', 'X', 'X', '9'},
        {'X', 'X', 'O', 'O', 'O', '6', 'X', 'O', 'X'},
        {'X', 'X', 'O', 'O', 'O', '6', 'X', '8', 'X'},
        {'X', 'X', 'O', 'O', 'O', '6', 'X', '8', '9'},
        {'X', 'X', 'O', 'O', '5', '6', 'X', 'X', 'O'},
        {'X', 'X', 'O', 'O', '5', '6', 'X', '8', 'O'},
        {'X', 'X', 'O', 'O', '5', '6', 'X', '8', '9'},
        {'X', 'X', 'O', 'O', 'O', '6', '7', 'X', 'X'},
        {'X', 'X', 'O', 'O', 'O', '6', '7', 'X', '9'},
        {'X', 'X', 'O', 'O', 'O', '6', '7', '8', 'X'},
        {'X', 'X', 'O', 'X', 'O', '6', '7', 'X', 'O'},
        {'X', 'X', 'O', '4', 'O', '6', 'X', 'X', 'O'},
        {'X', 'X', 'O', 'X', 'X', '6', 'O', '8', 'O'},
        {'X', 'X', 'O', 'X', 'X', '6', '7', 'O', 'O'},
        {'X', 'X', 'O', '4', 'X', '6', 'X', 'O', 'O'},
        {'X', 'X', 'O', 'X', '5', '6', '7', '8', 'O'},
        {'X', 'X', 'O', '4', 'X', '6', '7', '8', 'O'},
        {'X', 'X', 'O', '4', '5', '6', 'X', '8', 'O'},
        {'X', 'X', 'O', '4', '5', '6', '7', 'X', 'O'},
        {'X', 'X', 'O', '4', '5', '6', '7', '8', '9'},
        {'X', '2', 'O', 'X', 'X', '6', 'O', 'O', '9'},
        {'X', '2', 'O', 'X', 'X', '6', 'O', 'X', 'O'},
        {'X', '2', 'O', 'X', 'X', '6', 'O', '8', 'O'},
        {'X', '2', 'O', 'X', 'X', '6', '7', 'O', 'O'},
        {'X', '2', 'O', 'X', 'X', '6', '7', '8', 'O'},
        {'X', '2', 'O', 'X', '5', '6', '7', 'X', 'O'},
        {'X', '2', 'O', 'O', 'X', '6', 'X', 'X', 'O'},
        {'X', '2', 'O', 'O', 'X', '6', 'X', '8', 'O'},
        {'X', '2', 'O', '4', 'X', '6', 'X', '8', 'O'},
        {'X', '2', 'O', '4', 'X', '6', '7', 'X', 'O'},
        {'X', '2', 'O', '4', 'X', '6', '7', '8', 'O'},
        {'X', '2', 'O', 'O', '5', '6', 'X', 'X', 'O'},
        {'X', '2', 'O', '4', '5', '6', 'X', 'X', 'O'},
        {'X', '2', 'O', '4', '5', '6', '7', 'X', 'O'},
        {'X', 'X', '3', 'O', 'O', '6', 'X', 'O', 'X'},
        {'X', 'X', '3', 'O', 'O', '6', 'X', 'X', 'O'},
        {'X', 'X', '3', 'O', 'O', '6', 'X', '8', '9'},
        {'X', 'X', '3', 'O', 'O', '6', '7', 'X', '9'},
        {'X', 'X', '3', 'O', 'O', '6', '7', '8', 'X'},
        {'X', '2', 'X', 'O', 'O', '6', 'X', 'X', 'O'},
        {'X', '2', 'X', 'O', 'O', '6', 'X', '8', '9'},
        {'X', '2', 'X', 'O', 'O', '6', 'O', 'X', 'X'},
        {'X', '2', 'X', 'O', 'O', '6', '7', 'X', '9'},
        {'X', '2', 'X', 'O', 'O', '6', '7', '8', 'X'},
        {'X', '2', '3', 'O', 'O', '6', 'X', 'X', 'O'},
        {'X', '2', '3', 'O', 'O', '6', 'X', 'X', '9'},
        {'X', '2', '3', 'O', 'O', '6', 'X', '8', 'X'},
        {'X', '2', '3', 'O', 'O', '6', 'X', '8', '9'},
        {'X', '2', '3', 'O', '5', '6', 'X', 'X', 'O'},
        {'X', '2', '3', 'O', 'O', '6', '7', 'X', 'X'},
        {'X', '2', '3', 'O', 'O', '6', '7', 'X', '9'},
        {'X', '2', '3', 'O', 'O', '6', '7', '8', 'X'},
        {'O', 'X', 'X', 'O', 'O', '6', 'X', 'X', '9'},
        {'O', 'X', 'X', 'O', 'O', '6', 'X', 'O', 'X'},
        {'O', 'X', 'X', 'O', 'O', '6', 'X', '8', 'X'},
        {'O', 'X', 'X', 'O', 'O', '6', 'X', '8', '9'},
        {'O', 'X', 'X', 'O', 'O', '6', '7', 'X', 'X'},
        {'O', 'X', 'X', 'O', 'O', '6', '7', 'X', '9'},
        {'O', 'X', 'X', 'O', 'O', '6', '7', '8', 'X'},
        {'O', 'X', 'X', 'O', '5', '6', '7', 'O', 'X'},
        {'O', 'X', 'X', 'X', 'O', '6', 'O', 'O', 'X'},
        {'O', 'X', 'X', 'X', 'O', '6', 'O', '8', 'X'},
        {'O', 'X', 'X', 'X', 'O', '6', '7', 'O', 'X'},
        {'O', 'X', 'X', '4', 'O', '6', 'X', 'O', 'X'},
        {'O', 'X', 'X', '4', 'O', '6', 'O', '8', 'X'},
        {'O', 'X', 'X', '4', 'O', '6', '7', 'O', 'X'},
        {'O', 'X', 'X', '4', 'O', '6', '7', '8', 'X'},
        {'O', 'X', 'X', 'X', '5', '6', 'O', 'O', 'X'},
        {'O', 'X', 'X', '4', '5', '6', 'O', 'O', 'X'},
        {'O', 'X', 'X', '4', '5', '6', '7', 'O', 'X'},
        {'O', 'X', 'O', 'X', 'X', '6', 'O', 'O', 'X'},
        {'O', 'X', 'O', 'X', 'X', '6', 'O', '8', 'X'},
        {'O', 'X', 'O', 'X', 'X', '6', 'O', '8', '9'},
        {'O', 'X', 'O', 'X', 'X', '6', 'X', 'O', 'O'},
        {'O', 'X', 'O', 'X', 'X', '6', 'X', 'O', '9'},
        {'O', 'X', 'O', 'X', 'X', '6', '7', 'O', 'X'},
        {'O', 'X', 'O', 'X', 'X', '6', '7', 'O', '9'},
        {'O', 'X', 'O', 'X', 'X', '6', 'X', '8', 'O'},
        {'O', 'X', 'O', 'X', 'X', '6', '7', '8', 'O'},
        {'O', 'X', 'O', 'X', 'X', '6', '7', '8', '9'},
        {'O', 'X', 'O', 'X', 'O', '6', 'X', 'O', 'X'},
        {'O', 'X', 'O', 'X', 'O', '6', '7', 'X', '9'},
        {'O', 'X', '3', 'X', 'X', '6', 'O', 'O', 'X'},
        {'O', 'X', '3', 'X', 'X', '6', 'O', 'O', '9'},
        {'O', 'X', '3', 'X', 'X', '6', 'O', '8', 'O'},
        {'O', 'X', '3', 'X', '5', '6', 'O', 'O', 'X'},
        {'O', 'X', '3', 'X', 'X', '6', '7', 'O', 'O'},
        {'O', 'X', '3', 'X', 'X', '6', '7', 'O', '9'},
        {'O', 'X', 'O', 'O', 'X', '6', 'X', 'O', 'X'},
        {'1', 'X', 'O', 'X', 'X', '6', 'O', 'O', '9'},
        {'1', 'X', 'O', 'X', 'X', '6', 'O', '8', 'O'},
        {'1', 'X', 'O', 'X', 'X', '6', 'X', 'O', 'O'},
        {'1', 'X', 'O', 'X', 'X', '6', '7', 'O', 'O'},
        {'1', 'X', 'O', 'X', 'X', '6', '7', 'O', '9'},
        {'1', 'X', 'O', 'X', 'X', '6', '7', '8', 'O'},
        {'1', 'X', 'O', 'X', '5', '6', 'X', '8', 'O'},
        {'1', 'X', 'O', 'X', '5', '6', '7', 'X', 'O'},
        {'1', 'X', 'O', '4', 'X', '6', 'X', 'O', 'O'},
        {'1', 'X', 'O', '4', 'X', '6', 'X', '8', 'O'},
        {'1', 'X', 'O', '4', '5', '6', 'X', 'X', 'O'},
        {'1', 'X', 'O', '4', '5', '6', 'X', '8', 'O'},
        {'1', 'X', 'X', 'O', 'O', '6', 'X', 'O', 'X'},
        {'1', 'X', 'X', 'O', 'O', '6', 'X', '8', '9'},
        {'1', 'X', 'X', 'O', 'O', '6', '7', 'X', '9'},
        {'1', 'X', 'X', 'O', 'O', '6', '7', '8', 'X'},
        {'1', 'X', '3', 'O', 'O', '6', 'X', 'X', '9'},
        {'1', 'X', '3', 'O', 'O', '6', 'X', 'O', 'X'},
        {'1', 'X', '3', 'O', 'O', '6', 'X', '8', 'X'},
        {'1', 'X', '3', 'O', 'O', '6', 'X', '8', '9'},
        {'1', 'X', '3', 'O', 'O', '6', '7', 'X', 'X'},
        {'1', 'X', '3', 'O', 'O', '6', '7', '8', 'X'},
        {'O', 'O', 'X', 'X', 'X', '6', 'O', 'X', 'O'},
        {'O', 'O', 'X', 'X', 'X', '6', 'O', 'X', '9'},
        {'O', 'O', 'X', 'X', 'X', '6', 'O', 'O', 'X'},
        {'O', 'O', 'X', 'X', 'X', '6', 'O', '8', 'X'},
        {'O', 'O', 'X', 'X', 'X', '6', 'O', '8', '9'},
        {'O', 'O', 'X', 'X', 'X', '6', '7', 'O', 'X'},
        {'O', 'O', 'X', 'X', 'X', '6', '7', 'O', '9'},
        {'O', 'O', 'X', 'X', 'X', '6', '7', 'X', 'O'},
        {'O', 'O', 'X', 'X', 'X', '6', '7', '8', 'O'},
        {'O', 'O', 'X', 'X', 'X', '6', '7', '8', '9'},
        {'O', 'O', 'X', 'X', 'O', '6', 'X', '8', '9'},
        {'O', 'O', 'X', 'X', 'O', '6', 'O', 'X', 'X'},
        {'O', 'O', 'X', 'X', 'O', '6', '7', 'X', 'X'},
        {'O', 'O', 'X', 'X', '5', '6', 'O', 'X', 'X'},
        {'O', 'O', 'X', 'X', '5', '6', 'O', 'X', '9'},
        {'O', 'O', 'X', 'X', 'O', '6', '7', '8', 'X'},
        {'O', 'O', 'X', 'X', '5', '6', 'O', '8', 'X'},
        {'O', 'O', 'X', 'X', '5', '6', '7', 'O', 'X'},
        {'O', '2', 'X', 'X', 'O', '6', 'O', 'X', 'X'},
        {'O', '2', 'X', 'X', 'O', '6', 'O', '8', 'X'},
        {'O', '2', 'X', 'X', 'O', '6', '7', 'O', 'X'},
        {'O', '2', 'X', 'X', 'O', '6', '7', '8', 'X'},
        {'O', '2', 'X', 'X', 'X', '6', 'O', 'O', 'X'},
        {'O', '2', 'X', 'X', 'X', '6', 'O', 'O', '9'},
        {'O', '2', 'X', 'X', 'X', '6', 'O', '8', 'O'},
        {'O', '2', 'X', 'X', 'X', '6', 'O', '8', '9'},
        {'O', '2', 'X', 'X', '5', '6', 'O', 'O', 'X'},
        {'O', '2', 'X', 'X', '5', '6', 'O', '8', 'X'},
        {'O', '2', 'X', 'X', '5', '6', 'O', '8', '9'},
        {'O', '2', 'X', 'X', 'X', '6', '7', 'O', 'O'},
        {'O', '2', 'X', 'X', '5', '6', '7', 'O', 'X'},
        {'O', 'O', 'X', 'O', 'X', '6', '7', '8', 'X'},
        {'O', 'O', 'X', '4', 'X', '6', 'O', '8', 'X'},
        {'O', 'O', 'X', '4', 'X', '6', '7', 'O', 'X'},
        {'O', '2', 'X', 'O', 'X', '6', '7', 'O', 'X'},
        {'O', '2', 'X', '4', 'X', '6', 'O', 'O', 'X'},
        {'O', 'O', 'X', '4', 'O', '6', 'X', '8', 'X'},
        {'O', '2', 'X', 'O', 'O', '6', 'X', '8', 'X'},
        {'O', '2', 'X', '4', 'O', '6', 'X', 'O', 'X'},
        {'O', 'O', 'X', 'O', '5', '6', '7', 'X', 'X'},
        {'O', 'O', 'X', '4', 'O', '6', '7', 'X', 'X'},
        {'O', 'O', 'X', '4', '5', '6', 'O', 'X', 'X'},
        {'O', '2', 'X', 'O', 'O', '6', '7', 'X', 'X'},
        {'O', '2', 'X', '4', 'O', '6', 'O', 'X', 'X'},
        {'O', 'O', 'X', '4', '5', '6', '7', '8', 'X'},
        {'O', '2', 'X', 'O', '5', '6', '7', '8', 'X'},
        {'O', '2', 'X', '4', 'O', '6', '7', '8', 'X'},
        {'O', '2', 'X', '4', '5', '6', 'O', '8', 'X'},
        {'O', '2', 'X', '4', '5', '6', '7', 'O', 'X'},
        {'O', '2', 'X', '4', '5', '6', '7', '8', '9'},
        {'1', 'O', 'X', 'X', 'O', '6', 'O', 'X', 'X'},
        {'1', 'O', 'X', 'X', 'O', '6', 'O', '8', 'X'},
        {'1', 'O', 'X', 'X', 'X', '6', 'O', 'O', '9'},
        {'1', 'O', 'X', 'X', 'X', '6', 'O', 'X', 'O'},
        {'1', 'O', 'X', 'X', 'X', '6', 'O', '8', 'O'},
        {'1', 'O', 'X', 'X', 'X', '6', 'O', '8', '9'},
        {'1', 'O', 'X', 'X', '5', '6', 'O', 'O', 'X'},
        {'1', 'O', 'X', 'X', '5', '6', 'O', '8', 'X'},
        {'1', 'O', 'X', 'X', '5', '6', 'O', '8', '9'},
        {'1', 'O', 'X', 'X', 'X', '6', '7', 'O', 'O'},
        {'1', 'O', 'X', '4', 'X', '6', 'O', '8', '9'},
        {'1', 'O', 'X', 'O', 'O', '6', 'X', '8', 'X'},
        {'1', 'O', 'X', 'O', 'O', '6', '7', 'X', 'X'},
        {'1', 'O', 'X', 'O', '5', '6', 'O', 'X', 'X'},
        {'1', 'O', 'X', '4', 'O', '6', 'O', 'X', 'X'},
        {'1', 'O', 'X', '4', '5', '6', 'O', 'X', 'X'},
        {'1', 'O', 'X', 'O', '5', '6', '7', '8', 'X'},
        {'1', 'O', 'X', '4', 'O', '6', '7', '8', 'X'},
        {'1', 'O', 'X', '4', '5', '6', 'O', '8', 'X'},
        {'1', 'O', 'X', '4', '5', '6', '7', 'O', 'X'},
        {'1', '2', 'X', 'O', 'O', '6', 'X', 'X', '9'},
        {'1', '2', 'X', 'O', 'O', '6', 'X', 'O', 'X'},
        {'1', '2', 'X', 'O', 'O', '6', 'X', '8', 'X'},
        {'1', '2', 'X', 'O', 'O', '6', 'X', '8', '9'},
        {'1', '2', 'X', 'O', 'O', '6', 'O', 'X', 'X'},
        {'1', '2', 'X', 'O', 'O', '6', '7', 'X', 'X'},
        {'1', '2', 'X', 'O', 'O', '6', '7', 'X', '9'},
        {'1', '2', 'X', 'O', 'O', '6', '7', '8', 'X'},
        {'1', '2', 'X', 'O', '5', '6', 'O', '8', 'X'},
        {'1', '2', 'X', 'O', '5', '6', '7', 'O', 'X'},
        {'1', '2', 'X', 'X', 'O', '6', 'O', 'O', 'X'},
        {'1', '2', 'X', 'X', 'O', '6', 'O', '8', 'X'},
        {'1', '2', 'X', '4', 'O', '6', 'O', 'X', 'X'},
        {'1', '2', 'X', '4', 'O', '6', 'O', '8', 'X'},
        {'1', '2', 'X', '4', 'O', '6', '7', 'O', 'X'},
        {'1', '2', 'X', '4', 'O', '6', '7', '8', 'X'},
        {'1', '2', 'X', '4', '5', '6', 'O', 'O', 'X'},
        {'O', 'O', '3', 'X', 'X', '6', 'O', 'X', '9'},
        {'O', 'O', '3', 'X', 'X', '6', '7', 'X', 'O'},
        {'O', 'O', '3', 'X', 'X', '6', 'O', '8', 'X'},
        {'O', 'O', '3', 'X', 'X', '6', '7', 'O', 'X'},
        {'O', 'O', '3', 'X', 'X', '6', '7', '8', '9'},
        {'O', '2', 'O', 'X', 'X', '6', 'X', 'O', '9'},
        {'O', '2', 'O', 'X', 'X', '6', 'X', '8', 'O'},
        {'O', '2', 'O', 'X', 'X', '6', 'O', '8', 'X'},
        {'O', '2', 'O', 'X', 'X', '6', '7', 'O', 'X'},
        {'O', '2', 'O', 'X', 'X', '6', '7', '8', '9'},
        {'O', '2', '3', 'X', 'X', '6', 'O', 'O', 'X'},
        {'O', '2', '3', 'X', 'X', '6', 'O', '8', 'X'},
        {'O', '2', '3', 'X', 'X', '6', 'O', '8', '9'},
        {'O', '2', '3', 'X', 'X', '6', '7', 'O', 'X'},
        {'O', '2', '3', 'X', 'X', '6', '7', 'O', '9'},
        {'O', '2', '3', 'X', 'X', '6', '7', '8', 'O'},
        {'O', '2', '3', 'X', 'X', '6', '7', '8', '9'},
        {'O', '2', '3', 'X', '5', '6', 'O', '8', 'X'},
        {'O', '2', '3', 'X', '5', '6', '7', 'O', 'X'},
        {'1', 'O', 'O', 'X', 'X', '6', 'O', 'X', '9'},
        {'1', 'O', 'O', 'X', 'X', '6', '7', 'X', 'O'},
        {'1', 'O', 'O', 'X', 'X', '6', '7', '8', '9'},
        {'1', 'O', '3', 'X', 'X', '6', 'O', 'X', 'O'},
        {'1', 'O', '3', 'X', 'X', '6', 'O', 'X', '9'},
        {'1', 'O', '3', 'X', 'X', '6', 'O', '8', '9'},
        {'1', 'O', '3', 'X', 'X', '6', '7', 'O', '9'},
        {'1', 'O', '3', 'X', 'X', '6', '7', 'X', 'O'},
        {'1', 'O', '3', 'X', 'X', '6', '7', '8', 'O'},
        {'1', '2', 'O', 'X', 'X', '6', 'O', '8', '9'},
        {'1', '2', 'O', 'X', 'X', '6', '7', 'O', '9'},
        {'1', '2', 'O', 'X', 'X', '6', 'X', '8', 'O'},
        {'1', '2', 'O', 'X', 'X', '6', '7', 'X', 'O'},
        {'1', '2', 'O', 'X', 'X', '6', '7', '8', 'O'},
        {'1', '2', 'O', 'X', 'X', '6', '7', '8', '9'},
        {'1', '2', 'O', 'X', '5', '6', 'X', 'X', 'O'},
        {'1', '2', '3', 'X', 'X', '6', 'O', 'O', '9'},
        {'1', '2', '3', 'X', 'X', '6', 'O', '8', 'O'},
        {'1', '2', '3', 'X', 'X', '6', 'O', '8', '9'},
        {'1', '2', '3', 'X', 'X', '6', '7', 'O', 'O'},
        {'1', '2', '3', 'X', 'X', '6', '7', '8', 'O'},
        {'1', '2', 'O', '4', 'X', '6', 'X', 'X', 'O'},
        {'1', '2', 'O', '4', 'X', '6', 'X', '8', 'O'}
    },
    {
        {'X', 'O', 'X', 'O', 'X', 'O', '7', 'X', 'O'},
        {'X', 'O', 'X', 'O', 'X', 'O', '7', 'X', '9'},
        {'X', 'O', 'X', 'O', 'X', 'O', '7', '8', '9'},
        {'X', 'O', 'X', 'O', 'X', 'X', '7', 'O', 'O'},
        {'X', 'O', 'X', 'O', 'X', 'X', '7', 'O', '9'},
        {'X', 'O', 'X', 'O', 'X', '6', '7', 'O', '9'},
        {'X', 'O', 'X', 'O', 'X', 'X', '7', '8', 'O'},
        {'X', 'O', 'X', 'O', 'X', '6', '7', 'X', 'O'},
        {'X', 'O', 'X', 'O', 'X', '6', '7', '8', 'O'},
        {'X', 'O', 'X', 'O', 'O', 'X', '7', 'X', 'O'},
        {'X', 'O', 'X', 'X', 'O', 'O', '7', 'X', 'O'},
        {'X', 'O', 'X', 'X', 'O', 'O', '7', 'X', '9'},
        {'X', 'O', 'X', 'X', 'O', 'O', '7', '8', '9'},
        {'X', 'O', 'X', 'X', 'O', '6', '7', 'X', 'O'},
        {'X', 'O', 'X', 'X', 'O', '6', '7', '8', 'O'},
        {'X', 'O', 'X', 'X', 'X', 'O', '7', 'O', 'O'},
        {'X', 'O', 'X', 'X', 'X', 'O', '7', 'O', '9'},
        {'X', 'O', 'X', 'X', '5', 'O', '7', 'O', '9'},
        {'X', 'O', 'X', 'X', 'X', 'O', '7', '8', 'O'},
        {'X', 'O', 'X', 'X', '5', 'O', '7', 'X', 'O'},
        {'X', 'O', 'X', 'X', '5', 'O', '7', '8', 'O'},
        {'X', 'O', 'X', 'X', '5', 'O', '7', '8', '9'},
        {'X', 'O', 'X', '4', 'X', 'O', '7', 'O', '9'},
        {'X', 'O', 'X', '4', 'X', 'O', '7', 'X', 'O'},
        {'X', 'O', 'X', '4', 'X', 'O', '7', '8', 'O'},
        {'X', 'O', 'X', '4', '5', 'O', '7', 'X', 'O'},
        {'X', 'O', 'X', 'X', 'X', '6', '7', 'O', 'O'},
        {'X', 'O', 'X', 'X', '5', '6', '7', 'O', 'O'},
        {'X', 'O', 'X', '4', 'X', 'X', '7', 'O', 'O'},
        {'X', 'O', 'X', '4', 'X', '6', '7', 'O', 'O'},
        {'X', 'O', 'X', 'X', '5', '6', '7', '8', 'O'},
        {'X', 'O', 'X', '4', 'X', '6', '7', '8', 'O'},
        {'X', 'O', 'X', '4', '5', '6', '7', '8', 'O'},
        {'X', 'O', 'O', 'X', 'X', 'O', '7', '8', '9'},
        {'X', 'O', 'O', 'X', 'O', 'X', '7', 'X', 'O'},
        {'X', 'O', 'O', 'X', 'O', 'X', '7', 'X', '9'},
        {'X', 'O', 'O', 'X', 'O', 'X', '7', '8', 'X'},
        {'X', 'O', 'O', 'X', 'O', 'X', '7', '8', '9'},
        {'X', 'O', 'O', 'X', 'O', 'O', '7', 'X', 'X'},
        {'X', 'O', 'O', 'X', 'O', '6', '7', 'X', 'X'},
        {'X', 'O', 'O', 'X', 'O', '6', '7', 'X', '9'},
        {'X', 'O', 'O', 'X', '5', 'O', '7', 'X', '9'},
        {'X', 'O', 'O', 'X', '5', '6', '7', 'X', 'O'},
        {'X', 'O', 'O', 'X', 'O', '6', '7', '8', 'X'},
        {'X', 'O', 'O', 'X', '5', '6', '7', '8', '9'},
        {'X', 'O', '3', 'X', 'O', 'X', '7', 'X', 'O'},
        {'X', 'O', '3', 'X', 'O', 'X', '7', '8', 'O'},
        {'X', 'O', '3', 'X', 'O', 'O', '7', 'X', 'X'},
        {'X', 'O', '3', 'X', 'O', 'O', '7', 'X', '9'},
        {'X', 'O', '3', 'X', 'O', '6', '7', 'X', 'O'},
        {'X', 'O', '3', 'X', 'O', '6', '7', 'X', '9'},
        {'X', 'O', '3', 'X', 'O', 'O', '7', '8', 'X'},
        {'X', 'O', '3', 'X', 'O', '6', '7', '8', '9'},
        {'X', 'O', '3', 'X', 'X', 'O', '7', 'O', '9'},
        {'X', 'O', '3', 'X', 'X', 'O', '7', '8', 'O'},
        {'X', 'O', '3', 'X', '5', 'O', '7', 'X', 'O'},
        {'X', 'O', '3', 'X', '5', 'O', '7', 'X', '9'},
        {'X', 'O', '3', 'X', '5', 'O', '7', '8', '9'},
        {'X', 'O', '3', 'X', '5', '6', '7', 'O', '9'},
        {'X', 'O', '3', 'X', '5', '6', '7', 'X', 'O'},
        {'X', 'O', '3', 'X', '5', '6', '7', '8', 'O'},
        {'X', 'O', 'O', 'O', 'X', 'X', '7', 'X', 'O'},
        {'X', 'O', 'O', 'O', 'O', 'X', '7', 'X', 'X'},
        {'X', 'O', 'O', '4', 'O', 'X', '7', 'X', 'X'},
        {'X', 'O', 'O', '4', 'O', 'X', '7', 'X', '9'},
        {'X', 'O', '3', '4', 'O', 'X', '7', 'X', '9'},
        {'X', 'O', 'O', '4', 'O', '6', '7', 'X', 'X'},
        {'X', 'O', '3', 'O', 'O', '6', '7', 'X', 'X'},
        {'X', 'O', '3', '4', 'O', 'O', '7', 'X', 'X'},
        {'X', 'O', '3', '4', 'O', '6', '7', 'X', 'X'},
        {'X', 'O', '3', '4', 'O', '6', '7', 'X', '9'},
        {'X', 'O', '3', '4', '5', 'O', '7', 'X', '9'},
        {'X', 'O', '3', '4', '5', '6', '7', 'X', '9'},
        {'X', 'X', 'O', 'O', 'X', 'X', '7', 'O', 'O'},
        {'X', 'X', 'O', 'O', 'O', 'X', '7', 'X', 'O'},
        {'X', 'X', 'O', 'O', 'O', 'X', '7', 'X', '9'},
        {'X', 'X', 'O', 'O', 'O', 'X', '7', 'O', 'X'},
        {'X', 'X', 'O', 'O', 'O', 'X', '7', '8', 'X'},
        {'X', 'X', 'O', 'O', 'O', 'X', '7', '8', '9'},
        {'X', 'X', 'O', 'X', 'O', 'O', '7', 'X', '9'},
        {'X', 'X', 'O', 'X', 'O', 'O', '7', 'O', 'X'},
        {'X', 'X', 'O', 'X', 'O', 'O', '7', '8', 'X'},
        {'X', 'X', 'O', 'X', 'O', 'O', '7', '8', '9'},
        {'X', 'X', 'O', 'X', 'O', 'X', '7', 'O', 'O'},
        {'X', 'X', 'O', 'X', 'O', 'X', '7', 'O', '9'},
        {'X', 'X', 'O', 'X', 'O', '6', '7', 'O', 'X'},
        {'X', 'X', 'O', 'X', 'O', '6', '7', 'O', '9'},
        {'X', 'X', 'O', 'X', 'O', 'X', '7', '8', 'O'},
        {'X', 'X', 'O', 'X', 'O', '6', '7', '8', 'O'},
        {'X', 'X', 'O', 'X', 'O', '6', '7', '8', '9'},
        {'X', 'X', 'O', '4', 'O', 'X', '7', 'O', 'X'},
        {'X', 'X', 'O', '4', 'O', 'X', '7', 'O', '9'},
        {'X', 'X', 'O', '4', 'O', 'X', '7', 'X', 'O'},
        {'X', 'X', 'O', '4', 'O', 'X', '7', '8', 'O'},
        {'X', 'X', 'O', '4', 'O', 'X', '7', '8', '9'},
        {'X', 'X', 'O', '4', 'O', '6', '7', 'X', '9'},
        {'X', 'X', 'O', '4', 'O', '6', '7', 'O', 'X'},
        {'X', 'X', 'O', '4', 'O', '6', '7', '8', 'X'},
        {'X', 'X', 'O', '4', 'O', '6', '7', '8', '9'},
        {'X', 'X', 'O', 'X', '5', 'O', '7', 'O', '9'},
        {'X', 'X', 'O', 'X', '5', 'X', '7', 'O', 'O'},
        {'X', 'X', 'O', 'X', '5', '6', '7', 'O', 'O'},
        {'X', 'X', 'O', 'X', '5', '6', '7', 'O', '9'},
        {'X', 'X', 'O', '4', 'X', 'X', '7', 'O', 'O'},
        {'X', 'X', 'O', '4', '5', 'X', '7', 'O', 'O'},
        {'X', 'X', 'O', '4', '5', 'X', '7', 'O', '9'},
        {'X', 'X', 'O', '4', '5', '6', '7', 'O', '9'},
        {'X', 'X', 'O', '4', '5', 'X', '7', '8', 'O'},
        {'X', '2', 'O', 'X', 'O', 'X', '7', 'O', '9'},
        {'X', '2', 'O', 'X', 'O', 'X', '7', 'X', 'O'},
        {'X', '2', 'O', 'X', 'O', 'X', '7', '8', 'O'},
        {'X', '2', 'O', 'X', 'O', 'X', '7', '8', '9'},
        {'X', '2', 'O', 'X', 'O', 'O', '7', 'X', 'X'},
        {'X', '2', 'O', 'X', 'O', 'O', '7', 'X', '9'},
        {'X', '2', 'O', 'X', 'O', '6', '7', 'X', 'O'},
        {'X', '2', 'O', 'X', 'O', '6', '7', 'X', '9'},
        {'X', '2', 'O', 'X', 'O', 'O', '7', '8', 'X'},
        {'X', '2', 'O', 'X', 'O', '6', '7', 'O', 'X'},
        {'X', '2', 'O', 'X', 'O', '6', '7', '8', 'X'},
        {'X', '2', 'O', 'X', 'O', '6', '7', '8', '9'},
        {'X', '2', 'O', 'X', 'X', 'O', '7', 'O', '9'},
        {'X', '2', 'O', 'X', '5', 'O', '7', '8', '9'},
        {'X', '2', 'O', 'X', '5', '6', '7', 'O', '9'},
        {'X', '2', 'O', 'X', '5', '6', '7', '8', 'O'},
        {'X', '2', 'O', 'O', 'O', 'X', '7', 'X', 'X'},
        {'X', '2', 'O', 'O', 'O', 'X', '7', 'X', '9'},
        {'X', '2', 'O', 'O', 'O', 'X', '7', '8', 'X'},
        {'X', '2', 'O', '4', 'O', 'X', '7', 'X', 'O'},
        {'X', '2', 'O', '4', 'O', 'X', '7', 'X', '9'},
        {'X', '2', 'O', '4', 'O', 'X', '7', '8', 'X'},
        {'X', '2', 'O', '4', 'O', 'X', '7', '8', '9'},
        {'X', '2', 'O', 'O', 'O', '6', '7', 'X', 'X'},
        {'X', '2', 'O', '4', 'O', 'O', '7', 'X', 'X'},
        {'X', '2', 'O', '4', 'O', '6', '7', 'X', 'X'},
        {'X', '2', 'O', '4', 'O', '6', '7', 'X', '9'},
        {'X', '2', 'O', '4', 'O', '6', '7', '8', 'X'},
        {'X', 'X', '3', 'O', 'X', 'X', '7', 'O', 'O'},
        {'X', '2', 'X', 'O', 'X', 'X', '7', 'O', 'O'},
        {'X', '2', '3', 'O', 'X', 'X', '7', 'O', 'O'},
        {'X', 'X', '3', 'X', 'O', 'X', '7', 'O', 'O'},
        {'X', '2', '3', 'X', 'O', 'O', '7', 'X', 'O'},
        {'X', '2', '3', 'X', 'O', 'O', '7', 'X', '9'},
        {'X', '2', '3', 'X', 'O', 'O', '7', 'O', 'X'},
        {'X', '2', '3', 'X', 'O', 'O', '7', '8', 'X'},
        {'X', '2', '3', 'X', 'O', 'O', '7', '8', '9'},
        {'X', '2', '3', 'X', 'O', 'X', '7', 'O', 'O'},
        {'X', '2', '3', 'X', 'O', '6', '7', 'O', '9'},
        {'X', '2', '3', 'X', 'O', 'X', '7', '8', 'O'},
        {'X', '2', '3', 'X', 'O', '6', '7', 'X', 'O'},
        {'X', '2', '3', 'X', 'O', '6', '7', '8', 'O'},
        {'X', '2', '3', 'X', 'O', '6', '7', '8', '9'},
        {'X', '2', 'X', 'X', 'X', 'O', '7', 'O', 'O'},
        {'X', '2', '3', 'X', 'X', 'O', '7', 'O', 'O'},
        {'X', '2', '3', 'X', '5', 'O', '7', 'O', '9'},
        {'X', '2', '3', 'X', '5', 'O', '7', '8', 'O'},
        {'X', '2', '3', 'X', '5', 'O', '7', '8', '9'},
        {'X', 'X', '3', 'X', '5', '6', '7', 'O', 'O'},
        {'X', 'X', '3', '4', 'X', '6', '7', 'O', 'O'},
        {'X', 'X', '3', '4', '5', 'X', '7', 'O', 'O'},
        {'X', '2', 'X', 'X', '5', '6', '7', 'O', 'O'},
        {'X', '2', 'X', '4', 'X', '6', '7', 'O', 'O'},
        {'X', '2', 'X', '4', '5', 'X', '7', 'O', 'O'},
        {'X', '2', '3', 'X', 'X', '6', '7', 'O', 'O'},
        {'X', '2', '3', 'X', '5', 'X', '7', 'O', 'O'},
        {'X', '2', '3', 'X', '5', '6', '7', 'O', 'O'},
        {'X', '2', '3', 'X', '5', '6', '7', 'O', '9'},
        {'X', '2', '3', '4', 'X', 'X', '7', 'O', 'O'},
        {'X', '2', '3', '4', 'X', '6', '7', 'O', 'O'},
        {'X', '2', '3', '4', '5', 'X', '7', 'O', 'O'},
        {'X', '2', '3', 'X', '5', '6', '7', '8', 'O'},
        {'O', 'X', 'X', 'O', 'X', 'O', '7', 'O', 'X'},
        {'O', 'X', 'X', 'O', 'X', 'O', '7', '8', 'X'},
        {'O', 'X', 'X', 'O', 'X', 'O', '7', '8', '9'},
        {'O', 'X', 'X', 'O', 'X', 'X', '7', 'O', 'O'},
        {'O', 'X', 'X', 'O', 'X', 'X', '7', 'O', '9'},
        {'O', 'X', 'X', 'O', 'X', '6', '7', 'O', 'X'},
        {'O', 'X', 'X', 'O', 'X', '6', '7', 'O', '9'},
        {'O', 'X', 'X', 'O', 'X', 'X', '7', '8', 'O'},
        {'O', 'X', 'X', 'O', 'X', '6', '7', '8', 'O'},
        {'O', 'X', 'X', 'O', 'X', '6', '7', '8', '9'},
        {'O', 'X', 'X', 'O', 'O', 'X', '7', 'X', '9'},
        {'O', 'X', 'X', 'O', '5', 'X', '7', '8', '9'},
        {'O', 'X', 'X', 'O', '5', '6', '7', 'X', '9'},
        {'O', 'X', 'X', 'O', '5', '6', '7', '8', 'X'},
        {'O', 'X', 'X', 'X', 'O', 'O', '7', 'O', 'X'},
        {'O', 'X', 'X', 'X', 'O', 'O', '7', '8', 'X'},
        {'O', 'X', 'X', 'X', 'X', 'O', '7', 'O', 'O'},
        {'O', 'X', 'X', 'X', 'X', 'O', '7', 'O', '9'},
        {'O', 'X', 'X', 'X', 'X', 'O', '7', '8', 'O'},
        {'O', 'X', 'X', '4', 'X', 'O', '7', 'O', 'X'},
        {'O', 'X', 'X', '4', 'X', 'O', '7', 'O', '9'},
        {'O', 'X', 'X', '4', 'X', 'O', '7', '8', 'O'},
        {'O', 'X', 'X', 'X', 'X', '6', '7', 'O', 'O'},
        {'O', 'X', 'X', '4', 'X', 'X', '7', 'O', 'O'},
        {'O', 'X', 'X', '4', 'X', '6', '7', 'O', 'O'},
        {'O', 'X', 'X', '4', 'X', '6', '7', 'O', '9'},
        {'O', 'X', 'X', '4', '5', '6', '7', 'O', '9'},
        {'O', 'X', 'O', 'X', 'X', 'O', '7', 'O', 'X'},
        {'O', 'X', 'O', 'X', 'O', 'X', '7', 'X', '9'},
        {'O', 'X', 'O', 'X', 'O', 'X', '7', 'O', 'X'},
        {'O', 'X', 'O', 'X', 'O', 'X', '7', '8', 'X'},
        {'O', 'X', 'O', 'X', 'O', 'X', '7', '8', '9'},
        {'O', 'X', 'O', 'X', 'O', 'O', '7', 'X', 'X'},
        {'O', 'X', 'O', 'X', 'O', '6', '7', 'X', 'X'},
        {'O', 'X', 'O', 'X', 'O', '6', '7', '8', 'X'},
        {'O', 'X', '3', 'X', 'O', 'O', '7', 'X', 'X'},
        {'O', 'X', 'O', 'O', 'X', 'X', '7', 'O', 'X'},
        {'O', 'X', 'O', 'O', 'X', 'X', '7', '8', 'X'},
        {'O', 'X', '3', 'O', 'X', 'X', '7', 'O', 'X'},
        {'O', 'X', '3', 'O', 'X', 'X', '7', 'O', '9'},
        {'O', 'X', '3', 'O', 'X', 'X', '7', '8', '9'},
        {'O', 'X', '3', 'O', 'X', '6', '7', 'O', 'X'},
        {'O', 'X', '3', 'O', 'X', '6', '7', '8', 'X'},
        {'O', 'X', 'O', 'O', 'O', 'X', '7', 'X', 'X'},
        {'O', 'X', 'O', 'O', '5', 'X', '7', 'X', 'X'},
        {'O', 'X', 'O', '4', 'O', 'X', '7', 'X', 'X'},
        {'O', 'X', 'O', 'O', '5', 'X', '7', '8', 'X'},
        {'O', 'X', 'O', '4', 'O', 'X', '7', '8', 'X'},
        {'O', 'X', 'O', '4', '5', 'X', '7', '8', 'X'},
        {'O', 'X', '3', 'O', 'O', 'X', '7', 'X', 'X'},
        {'O', 'X', '3', 'O', '5', 'X', '7', 'X', '9'},
        {'O', 'X', '3', 'O', '5', 'X', '7', '8', 'X'},
        {'O', 'X', '3', 'O', '5', 'X', '7', '8', '9'},
        {'O', 'X', '3', '4', '5', 'X', '7', '8', '9'},
        {'O', 'X', 'O', '4', 'O', '6', '7', 'X', 'X'},
        {'O', 'X', '3', 'O', 'O', '6', '7', 'X', 'X'},
        {'O', 'X', '3', 'O', '5', '6', '7', 'X', 'X'},
        {'O', 'X', '3', '4', 'O', 'O', '7', 'X', 'X'},
        {'O', 'X', '3', '4', 'O', '6', '7', 'X', 'X'},
        {'O', 'X', '3', 'O', '5', '6', '7', '8', 'X'},
        {'1', 'X', 'O', 'X', 'O', 'X', '7', 'O', 'X'},
        {'1', 'X', 'O', 'X', 'O', 'X', '7', 'O', '9'},
        {'1', 'X', 'O', 'X', 'O', 'X', '7', '8', '9'},
        {'1', 'X', 'O', 'X', 'O', 'O', '7', 'X', 'X'},
        {'1', 'X', 'O', 'X', 'O', '6', '7', 'X', '9'},
        {'1', 'X', 'O', 'X', 'O', 'O', '7', '8', 'X'},
        {'1', 'X', 'O', 'X', 'O', '6', '7', 'O', 'X'},
        {'1', 'X', 'O', 'X', 'O', '6', '7', '8', 'X'},
        {'1', 'X', 'O', 'X', 'O', '6', '7', '8', '9'},
        {'1', 'X', 'O', 'O', 'O', 'X', '7', 'X', 'X'},
        {'1', 'X', 'O', 'O', 'O', 'X', '7', 'X', '9'},
        {'1', 'X', 'O', 'O', 'O', 'X', '7', '8', 'X'},
        {'1', 'X', 'O', 'O', '5', 'X', '7', '8', 'X'},
        {'1', 'X', 'O', '4', 'O', 'X', '7', 'X', '9'},
        {'1', 'X', 'O', '4', 'O', 'X', '7', 'O', 'X'},
        {'1', 'X', 'O', '4', 'O', 'X', '7', '8', 'X'},
        {'1', 'X', 'O', '4', 'O', 'X', '7', '8', '9'},
        {'1', 'X', 'O', 'O', 'O', '6', '7', 'X', 'X'},
        {'1', 'X', 'O', '4', 'O', 'O', '7', 'X', 'X'},
        {'1', 'X', 'O', '4', 'O', '6', '7', 'X', 'X'},
        {'1', 'X', 'O', '4', 'O', '6', '7', '8', 'X'},
        {'1', 'X', 'X', 'O', 'X', 'X', '7', 'O', 'O'},
        {'1', 'X', '3', 'O', 'X', 'X', '7', 'O', 'O'},
        {'1', 'X', '3', 'O', 'X', 'X', '7', 'O', '9'},
        {'1', 'X', '3', 'X', 'O', 'X', '7', 'O', '9'},
        {'1', 'X', 'X', 'X', 'X', 'O', '7', 'O', 'O'},
        {'1', 'X', 'X', 'X', '5', '6', '7', 'O', 'O'},
        {'1', 'X', 'X', '4', 'X', '6', '7', 'O', 'O'},
        {'1', 'X', 'X', '4', '5', 'X', '7', 'O', 'O'},
        {'1', 'X', '3', 'X', 'X', '6', '7', 'O', 'O'},
        {'1', 'X', '3', 'X', '5', 'X', '7', 'O', 'O'},
        {'1', 'X', '3', 'X', '5', '6', '7', 'O', 'O'},
        {'1', 'X', '3', '4', 'X', 'X', '7', 'O', 'O'},
        {'1', 'X', '3', '4', 'X', '6', '7', 'O', 'O'},
        {'1', 'X', '3', '4', '5', 'X', '7', 'O', 'O'},
        {'1', 'X', '3', '4', '5', 'X', '7', '8', 'O'},
        {'O', 'O', 'X', 'X', 'X', 'O', '7', 'X', 'O'},
        {'O', 'O', 'X', 'X', 'X', 'O', '7', 'X', '9'},
        {'O', 'O', 'X', 'X', 'X', 'O', '7', 'O', 'X'},
        {'O', 'O', 'X', 'X', 'X', 'O', '7', '8', 'X'},
        {'O', 'O', 'X', 'X', 'X', 'O', '7', '8', '9'},
        {'O', 'O', 'X', 'X', 'O', 'O', '7', 'X', 'X'},
        {'O', 'O', 'X', 'X', '5', 'O', '7', 'X', 'X'},
        {'O', 'O', 'X', 'X', '5', 'O', '7', 'X', '9'},
        {'O', 'O', 'X', 'X', '5', 'O', '7', '8', 'X'},
        {'O', '2', 'X', 'X', 'O', 'O', '7', 'X', 'X'},
        {'O', '2', 'X', 'X', 'X', 'O', '7', 'O', 'X'},
        {'O', '2', 'X', 'X', 'X', 'O', '7', 'O', '9'},
        {'O', '2', 'X', 'X', 'X', 'O', '7', '8', 'O'},
        {'O', '2', 'X', 'X', 'X', 'O', '7', '8', '9'},
        {'O', 'O', 'X', 'O', 'X', 'X', '7', 'X', 'O'},
        {'O', 'O', 'X', 'O', 'X', 'X', '7', 'X', '9'},
        {'O', 'O', 'X', 'O', 'X', 'X', '7', '8', '9'},
        {'O', 'O', 'X', 'O', 'X', 'O', '7', 'X', 'X'},
        {'O', 'O', 'X', 'O', 'X', '6', '7', 'X', 'X'},
        {'O', 'O', 'X', 'O', 'X', '6', '7', 'X', '9'},
        {'O', 'O', 'X', '4', 'X', 'O', '7', 'X', 'X'},
        {'O', 'O', 'X', '4', 'X', 'O', '7', 'X', '9'},
        {'O', 'O', 'X', '4', 'X', '6', '7', 'X', 'O'},
        {'O', 'O', 'X', '4', 'X', '6', '7', 'X', '9'},
        {'O', 'O', 'X', '4', 'X', 'O', '7', '8', 'X'},
        {'O', 'O', 'X', '4', 'X', '6', '7', '8', '9'},
        {'O', '2', 'X', 'O', 'X', 'X', '7', 'O', '9'},
        {'O', '2', 'X', 'O', 'X', 'X', '7', 'X', 'O'},
        {'O', '2', 'X', 'O', 'X', 'X', '7', '8', 'O'},
        {'O', '2', 'X', 'O', 'X', 'X', '7', '8', '9'},
        {'O', '2', 'X', 'O', 'X', 'O', '7', 'X', 'X'},
        {'O', '2', 'X', 'O', 'X', '6', '7', 'X', '9'},
        {'O', '2', 'X', 'O', 'X', 'O', '7', '8', 'X'},
        {'O', '2', 'X', 'O', 'X', '6', '7', '8', 'X'},
        {'O', '2', 'X', 'O', 'X', '6', '7', '8', '9'},
        {'O', '2', 'X', '4', 'X', 'O', '7', 'O', 'X'},
        {'O', '2', 'X', '4', 'X', 'O', '7', '8', 'X'},
        {'O', '2', 'X', '4', 'X', 'O', '7', '8', '9'},
        {'O', '2', 'X', '4', 'X', '6', '7', 'O', '9'},
        {'O', '2', 'X', '4', 'X', '6', '7', '8', 'O'},
        {'O', '2', 'X', '4', 'X', '6', '7', '8', '9'},
        {'O', '2', 'X', 'O', '5', 'X', '7', 'X', '9'},
        {'O', 'O', 'X', '4', '5', 'O', '7', 'X', 'X'},
        {'O', '2', 'X', 'O', '5', 'O', '7', 'X', 'X'},
        {'O', '2', 'X', 'O', '5', '6', '7', 'X', 'X'},
        {'O', '2', 'X', 'O', '5', '6', '7', 'X', '9'},
        {'O', '2', 'X', '4', 'O', 'O', '7', 'X', 'X'},
        {'O', '2', 'X', '4', '5', 'O', '7', 'X', 'X'},
        {'O', '2', 'X', '4', '5', '6', '7', 'X', '9'},
        {'O', '2', 'X', '4', '5', 'O', '7', '8', 'X'},
        {'1', 'O', 'X', 'X', 'O', 'O', '7', 'X', 'X'},
        {'1', 'O', 'X', 'X', 'O', 'O', '7', 'X', '9'},
        {'1', 'O', 'X', 'X', 'O', '6', '7', 'X', '9'},
        {'1', 'O', 'X', 'X', 'X', 'O', '7', 'O', '9'},
        {'1', 'O', 'X', 'X', 'X', 'O', '7', 'X', 'O'},
        {'1', 'O', 'X', 'X', 'X', 'O', '7', '8', 'O'},
        {'1', 'O', 'X', 'X', 'X', 'O', '7', '8', '9'},
        {'1', 'O', 'X', 'X', '5', 'O', '7', 'X', 'O'},
        {'1', 'O', 'X', 'X', '5', 'O', '7', 'X', '9'},
        {'1', 'O', 'X', 'X', '5', 'O', '7', '8', '9'},
        {'1', 'O', 'X', 'O', 'X', 'X', '7', 'O', '9'},
        {'1', 'O', 'X', 'O', 'X', 'X', '7', 'X', 'O'},
        {'1', 'O', 'X', 'O', 'X', 'X', '7', '8', 'O'},
        {'1', 'O', 'X', 'O', 'X', 'O', '7', 'X', '9'},
        {'1', 'O', 'X', 'O', 'X', '6', '7', 'X', 'O'},
        {'1', 'O', 'X', 'O', 'X', '6', '7', 'X', '9'},
        {'1', 'O', 'X', 'O', 'X', '6', '7', '8', '9'},
        {'1', 'O', 'X', '4', 'X', 'O', '7', 'X', 'O'},
        {'1', 'O', 'X', '4', 'X', 'O', '7', 'X', '9'},
        {'1', 'O', 'X', '4', 'X', 'O', '7', '8', '9'},
        {'1', 'O', 'X', '4', 'X', '6', '7', 'O', '9'},
        {'1', 'O', 'X', '4', 'X', '6', '7', 'X', 'O'},
        {'1', 'O', 'X', '4', 'X', '6', '7', '8', 'O'},
        {'1', 'O', 'X', 'O', '5', 'O', '7', 'X', 'X'},
        {'1', 'O', 'X', 'O', '5', '6', '7', 'X', '9'},
        {'1', 'O', 'X', '4', 'O', 'O', '7', 'X', 'X'},
        {'1', 'O', 'X', '4', '5', 'O', '7', 'X', 'X'},
        {'1', 'O', 'X', '4', '5', 'O', '7', 'X', '9'},
        {'1', 'O', 'X', '4', '5', '6', '7', 'X', '9'},
        {'1', '2', 'X', 'O', 'X', 'O', '7', '8', '9'},
        {'1', '2', 'X', 'O', 'X', 'X', '7', 'O', 'O'},
        {'1', '2', 'X', 'O', 'X', '6', '7', 'O', '9'},
        {'1', '2', 'X', 'O', 'X', 'X', '7', '8', 'O'},
        {'1', '2', 'X', 'O', 'X', '6', '7', '8', 'O'},
        {'1', '2', 'X', 'X', 'X', 'O', '7', 'O', 'O'},
        {'1', '2', 'X', 'X', 'X', 'O', '7', 'O', '9'},
        {'1', '2', 'X', 'X', 'X', 'O', '7', '8', 'O'},
        {'1', '2', 'X', '4', 'X', 'O', '7', 'O', '9'},
        {'1', '2', 'X', '4', 'X', 'O', '7', '8', 'O'},
        {'1', '2', 'X', 'X', 'X', '6', '7', 'O', 'O'},
        {'1', '2', 'X', 'X', '5', 'X', '7', 'O', 'O'},
        {'1', '2', 'X', 'X', '5', '6', '7', 'O', 'O'},
        {'1', '2', 'X', '4', 'X', 'X', '7', 'O', 'O'},
        {'1', '2', 'X', '4', 'X', '6', '7', 'O', 'O'},
        {'1', '2', 'X', 'X', '5', '6', '7', '8', 'O'},
        {'1', '2', 'X', '4', 'X', '6', '7', '8', 'O'},
        {'1', '2', 'X', '4', '5', 'X', '7', '8', 'O'},
        {'O', 'O', '3', 'X', 'O', '6', '7', 'X', 'X'},
        {'O', 'O', '3', 'X', '5', 'O', '7', 'X', 'X'},
        {'O', '2', 'O', 'X', 'O', '6', '7', 'X', 'X'},
        {'O', '2', 'O', 'X', '5', 'O', '7', 'X', 'X'},
        {'O', '2', '3', 'X', 'O', 'O', '7', 'X', 'X'},
        {'O', '2', '3', 'X', 'O', '6', '7', 'X', 'X'},
        {'O', '2', '3', 'X', '5', 'O', '7', 'X', 'X'},
        {'1', 'O', 'O', 'X', 'O', '6', '7', 'X', 'X'},
        {'1', 'O', 'O', 'X', '5', 'O', '7', 'X', 'X'},
        {'1', 'O', '3', 'X', 'O', 'O', '7', 'X', 'X'},
        {'1', 'O', '3', 'X', 'O', '6', '7', 'X', 'X'},
        {'1', 'O', '3', 'X', 'O', '6', '7', 'X', '9'},
        {'1', 'O', '3', 'X', '5', 'O', '7', 'X', 'X'},
        {'1', 'O', '3', 'X', '5', 'O', '7', 'X', '9'},
        {'1', 'O', '3', 'X', '5', '6', '7', 'X', '9'},
        {'1', '2', 'O', 'X', 'O', 'X', '7', 'X', '9'},
        {'1', '2', 'O', 'X', 'O', 'X', '7', '8', 'X'},
        {'1', '2', 'O', 'X', 'O', 'O', '7', 'X', 'X'},
        {'1', '2', 'O', 'X', 'O', '6', '7', 'X', 'X'},
        {'1', '2', 'O', 'X', 'O', '6', '7', 'X', '9'},
        {'1', '2', 'O', 'X', 'O', '6', '7', '8', 'X'},
        {'1', '2', '3', 'X', 'O', 'O', '7', 'X', 'X'},
        {'1', '2', '3', 'X', 'O', 'O', '7', 'X', '9'},
        {'1', '2', '3', 'X', 'O', 'O', '7', '8', 'X'},
        {'1', '2', '3', 'X', '5', '6', '7', '8', 'O'},
        {'O', '2', '3', 'O', 'X', 'X', '7', 'X', '9'},
        {'O', '2', '3', 'O', 'X', 'X', '7', '8', 'X'},
        {'O', '2', '3', 'O', 'X', 'X', '7', '8', '9'},
        {'O', 'O', '3', 'O', 'X', '6', '7', 'X', 'X'},
        {'O', 'O', '3', '4', 'X', 'O', '7', 'X', 'X'},
        {'O', '2', '3', 'O', 'X', '6', '7', 'X', 'X'},
        {'O', '2', '3', 'O', 'X', '6', '7', '8', 'X'},
        {'O', '2', '3', '4', 'X', 'O', '7', '8', 'X'},
        {'1', 'O', '3', 'O', 'X', '6', '7', 'X', '9'},
        {'1', 'O', '3', '4', 'X', 'O', '7', 'X', '9'},
        {'O', '2', 'O', 'O', '5', 'X', '7', 'X', 'X'},
        {'O', '2', 'O', '4', 'O', 'X', '7', 'X', 'X'},
        {'O', '2', '3', 'O', '5', 'X', '7', 'X', 'X'},
        {'O', '2', '3', 'O', '5', 'X', '7', 'X', '9'},
        {'1', 'O', 'O', 'O', '5', 'X', '7', 'X', 'X'},
        {'1', 'O', 'O', '4', 'O', 'X', '7', 'X', 'X'},
        {'1', 'O', '3', '4', '5', 'X', '7', 'X', '9'},
        {'1', '2', 'O', 'O', 'O', 'X', '7', 'X', 'X'},
        {'1', '2', 'O', 'O', '5', 'X', '7', 'X', 'X'},
        {'1', '2', 'O', '4', 'O', 'X', '7', 'X', 'X'},
        {'1', '2', 'O', '4', 'O', 'X', '7', 'X', '9'},
        {'1', '2', 'O', '4', 'O', 'X', '7', '8', 'X'},
        {'O', 'O', '3', '4', '5', '6', '7', 'X', 'X'},
        {'O', '2', 'O', '4', '5', '6', '7', 'X', 'X'},
        {'O', '2', '3', 'O', '5', '6', '7', 'X', 'X'},
        {'O', '2', '3', '4', 'O', '6', '7', 'X', 'X'},
        {'O', '2', '3', '4', '5', 'O', '7', 'X', 'X'},
        {'O', '2', '3', '4', '5', '6', '7', 'X', 'X'},
        {'O', '2', '3', '4', '5', '6', '7', 'X', '9'},
        {'1', 'O', 'O', '4', '5', '6', '7', 'X', 'X'},
        {'1', 'O', '3', 'O', '5', '6', '7', 'X', 'X'},
        {'1', 'O', '3', '4', 'O', '6', '7', 'X', 'X'},
        {'1', 'O', '3', '4', '5', 'O', '7', 'X', 'X'},
        {'1', 'O', '3', '4', '5', '6', '7', 'X', 'X'},
        {'1', '2', 'O', 'O', '5', '6', '7', 'X', 'X'},
        {'1', '2', 'O', '4', 'O', '6', '7', 'X', 'X'},
        {'1', '2', 'O', '4', '5', 'O', '7', 'X', 'X'},
        {'1', '2', '3', 'O', 'O', '6', '7', 'X', 'X'},
        {'1', '2', '3', 'O', '5', 'O', '7', 'X', 'X'},
        {'1', '2', '3', 'O', '5', '6', '7', 'X', 'X'},
        {'1', '2', '3', '4', 'O', 'O', '7', 'X', 'X'},
        {'1', '2', '3', '4', 'O', '6', '7', 'X', 'X'}
    },
    {
        {'X', 'O', 'X', 'O', 'X', 'X', 'O', '8', 'O'},
        {'X', 'O', 'X', 'O', 'O', 'X', 'X', '8', 'O'},
        {'X', 'O', 'X', 'O', 'O', 'X', 'X', '8', '9'},
        {'X', 'O', 'X', 'X', 'O', 'O', 'O', '8', 'X'},
        {'X', 'O', 'X', 'X', 'O', 'O', '7', '8', 'X'},
        {'X', 'O', 'X', 'X', 'O', 'X', 'O', '8', 'O'},
        {'X', 'O', 'X', 'X', 'O', 'X', 'O', '8', '9'},
        {'X', 'O', 'X', 'X', 'O', '6', 'O', '8', 'X'},
        {'X', 'O', 'X', 'X', 'O', '6', 'O', '8', '9'},
        {'X', 'O', 'X', 'X', 'O', 'X', '7', '8', 'O'},
        {'X', 'O', 'X', 'X', 'O', '6', '7', '8', '9'},
        {'X', 'O', 'X', '4', 'O', 'X', 'X', '8', 'O'},
        {'X', 'O', 'X', '4', 'O', 'X', '7', '8', 'O'},
        {'X', 'O', 'X', '4', 'O', 'X', '7', '8', '9'},
        {'X', 'O', 'X', '4', 'O', '6', 'X', '8', '9'},
        {'X', 'O', 'X', '4', 'O', '6', '7', '8', 'X'},
        {'X', 'O', 'X', '4', 'O', '6', '7', '8', '9'},
        {'X', 'O', 'X', 'X', 'X', 'O', 'O', '8', 'O'},
        {'X', 'O', 'X', 'X', 'X', '6', 'O', '8', 'O'},
        {'X', 'O', 'X', 'X', '5', 'X', 'O', '8', 'O'},
        {'X', 'O', 'X', 'X', '5', '6', 'O', '8', 'O'},
        {'X', 'O', 'X', 'X', '5', '6', 'O', '8', '9'},
        {'X', 'O', 'X', '4', 'X', 'X', 'O', '8', 'O'},
        {'X', 'O', 'X', '4', 'X', '6', 'O', '8', 'O'},
        {'X', 'O', 'X', '4', '5', 'X', 'O', '8', 'O'},
        {'X', 'O', 'X', '4', '5', 'X', '7', '8', 'O'},
        {'X', 'O', '3', 'X', 'O', 'X', '7', '8', '9'},
        {'X', 'O', '3', 'X', 'O', '6', '7', '8', 'X'},
        {'X', 'O', 'O', 'O', 'X', 'X', 'X', '8', 'O'},
        {'X', 'O', 'O', 'O', 'O', 'X', 'X', '8', 'X'},
        {'X', 'O', 'O', '4', 'O', 'X', 'X', '8', 'X'},
        {'X', 'O', '3', 'O', 'O', 'X', 'X', '8', 'X'},
        {'X', 'O', '3', 'O', 'O', 'X', 'X', '8', '9'},
        {'X', 'O', '3', '4', 'O', 'X', 'X', '8', '9'},
        {'X', 'O', '3', '4', 'O', 'X', '7', '8', 'X'},
        {'X', 'O', '3', '4', 'O', 'X', '7', '8', '9'},
        {'X', 'O', '3', 'O', 'O', '6', 'X', '8', 'X'},
        {'X', 'O', '3', '4', 'O', '6', 'X', '8', 'X'},
        {'X', 'O', '3', '4', 'O', '6', '7', '8', 'X'},
        {'X', 'X', 'O', 'O', 'X', 'O', '7', '8', '9'},
        {'X', 'X', 'O', 'O', 'X', 'X', 'O', '8', 'O'},
        {'X', 'X', 'O', 'O', 'X', 'X', 'O', '8', '9'},
        {'X', 'X', 'O', 'O', 'X', '6', 'O', '8', '9'},
        {'X', 'X', 'O', 'O', 'X', 'X', '7', '8', 'O'},
        {'X', 'X', 'O', 'O', 'X', '6', '7', '8', 'O'},
        {'X', 'X', 'O', 'O', 'O', 'X', 'X', '8', 'O'},
        {'X', 'X', 'O', 'O', 'O', 'X', 'X', '8', '9'},
        {'X', 'X', 'O', '4', 'X', 'O', 'O', '8', '9'},
        {'X', 'X', 'O', '4', 'X', 'X', 'O', '8', 'O'},
        {'X', 'X', 'O', '4', 'X', '6', 'O', '8', 'O'},
        {'X', '2', 'O', 'O', 'O', 'X', 'X', '8', 'X'},
        {'X', '2', 'O', 'O', 'O', '6', 'X', '8', 'X'},
        {'X', 'X', '3', 'O', 'X', 'X', 'O', '8', 'O'},
        {'X', '2', 'X', 'O', 'X', 'X', 'O', '8', 'O'},
        {'X', '2', '3', 'O', 'X', 'X', 'O', '8', 'O'},
        {'X', '2', 'X', 'X', 'O', 'X', 'O', '8', 'O'},
        {'X', '2', 'X', 'X', 'X', 'O', 'O', '8', 'O'},
        {'X', 'X', '3', 'X', '5', '6', 'O', '8', 'O'},
        {'X', 'X', '3', '4', 'X', '6', 'O', '8', 'O'},
        {'X', 'X', '3', '4', '5', 'X', 'O', '8', 'O'},
        {'X', '2', 'X', 'X', '5', '6', 'O', '8', 'O'},
        {'X', '2', 'X', '4', 'X', '6', 'O', '8', 'O'},
        {'X', '2', 'X', '4', '5', 'X', 'O', '8', 'O'},
        {'X', '2', '3', 'X', 'X', '6', 'O', '8', 'O'},
        {'X', '2', '3', 'X', '5', 'X', 'O', '8', 'O'},
        {'X', '2', '3', 'X', '5', '6', 'O', '8', '9'},
        {'X', '2', '3', '4', 'X', 'X', 'O', '8', 'O'},
        {'X', '2', '3', '4', 'X', '6', 'O', '8', 'O'},
        {'X', '2', '3', '4', '5', 'X', 'O', '8', 'O'},
        {'O', 'X', 'X', 'X', 'O', 'O', 'O', '8', 'X'},
        {'O', 'X', 'X', 'X', 'X', 'O', 'O', '8', 'O'},
        {'O', 'X', 'X', 'X', 'X', 'O', 'O', '8', '9'},
        {'O', 'X', 'X', '4', 'X', 'O', 'O', '8', '9'},
        {'O', 'X', 'X', 'X', 'X', '6', 'O', '8', 'O'},
        {'O', 'X', 'X', '4', 'X', '6', 'O', '8', 'O'},
        {'O', 'X', 'O', 'X', 'X', 'O', 'O', '8', 'X'},
        {'O', 'X', 'O', 'X', 'X', 'O', '7', '8', 'X'},
        {'O', 'X', 'O', 'X', 'X', 'O', '7', '8', '9'},
        {'O', 'X', 'O', 'X', 'O', 'O', 'X', '8', 'X'},
        {'O', 'X', 'O', 'X', 'O', '6', 'X', '8', 'X'},
        {'O', 'X', 'O', 'X', '5', 'O', 'X', '8', 'X'},
        {'O', 'X', 'O', 'X', '5', 'O', '7', '8', 'X'},
        {'O', 'X', '3', 'X', 'O', 'O', 'X', '8', 'X'},
        {'O', 'X', '3', 'X', 'X', 'O', 'O', '8', 'X'},
        {'O', 'X', '3', 'X', 'X', 'O', 'O', '8', '9'},
        {'O', 'X', '3', 'X', 'X', 'O', '7', '8', 'O'},
        {'O', 'X', '3', 'X', 'X', 'O', '7', '8', '9'},
        {'O', 'X', 'O', 'O', 'X', 'X', 'X', '8', 'O'},
        {'O', 'X', 'O', 'O', 'X', 'X', 'X', '8', '9'},
        {'O', 'X', 'O', 'O', 'X', 'X', '7', '8', '9'},
        {'O', 'X', 'O', 'O', 'X', 'O', 'X', '8', 'X'},
        {'O', 'X', 'O', 'O', 'X', '6', 'X', '8', 'X'},
        {'O', 'X', 'O', 'O', 'X', '6', 'X', '8', '9'},
        {'O', 'X', 'O', '4', 'X', 'O', 'X', '8', 'X'},
        {'O', 'X', 'O', '4', 'X', 'O', 'X', '8', '9'},
        {'O', 'X', 'O', '4', 'X', '6', 'X', '8', 'O'},
        {'O', 'X', 'O', '4', 'X', '6', 'X', '8', '9'},
        {'O', 'X', 'O', 'O', 'X', '6', '7', '8', 'X'},
        {'O', 'X', 'O', '4', 'X', 'O', '7', '8', 'X'},
        {'O', 'X', 'O', '4', 'X', '6', 'O', '8', 'X'},
        {'O', 'X', 'O', '4', 'X', '6', '7', '8', 'X'},
        {'O', 'X', 'O', '4', 'X', '6', '7', '8', '9'},
        {'O', 'X', '3', 'O', 'X', 'X', '7', '8', 'O'},
        {'O', 'X', '3', 'O', 'X', 'O', '7', '8', 'X'},
        {'O', 'X', '3', 'O', 'X', '6', '7', '8', '9'},
        {'O', 'X', '3', '4', 'X', 'O', 'O', '8', 'X'},
        {'O', 'X', '3', '4', 'X', 'O', '7', '8', 'X'},
        {'O', 'X', '3', '4', 'X', 'O', '7', '8', '9'},
        {'O', 'X', '3', '4', 'X', '6', 'O', '8', '9'},
        {'O', 'X', '3', '4', 'X', '6', '7', '8', 'O'},
        {'O', 'X', '3', '4', 'X', '6', '7', '8', '9'},
        {'O', 'X', 'O', 'O', 'O', 'X', 'X', '8', 'X'},
        {'O', 'X', 'O', 'O', '5', 'X', 'X', '8', 'X'},
        {'O', 'X', 'O', 'O', '5', 'X', 'X', '8', '9'},
        {'O', 'X', 'O', '4', 'O', 'X', 'X', '8', 'X'},
        {'O', 'X', 'O', 'O', '5', '6', 'X', '8', 'X'},
        {'O', 'X', 'O', '4', 'O', '6', 'X', '8', 'X'},
        {'O', 'X', 'O', '4', '5', 'O', 'X', '8', 'X'},
        {'O', 'X', 'O', '4', '5', '6', 'X', '8', 'X'},
        {'O', 'X', 'O', '4', '5', '6', 'X', '8', '9'},
        {'O', 'X', '3', 'O', 'O', '6', 'X', '8', 'X'},
        {'O', 'X', '3', 'O', '5', 'O', 'X', '8', 'X'},
        {'O', 'X', '3', '4', 'O', 'O', 'X', '8', 'X'},
        {'O', 'X', '3', '4', 'O', '6', 'X', '8', 'X'},
        {'O', 'X', '3', '4', '5', 'O', 'X', '8', 'X'},
        {'O', 'X', 'O', '4', '5', '6', '7', '8', 'X'},
        {'O', 'X', '3', '4', '5', 'O', '7', '8', 'X'},
        {'1', 'X', 'O', 'X', 'X', 'O', 'O', '8', '9'},
        {'1', 'X', 'O', 'O', 'X', 'X', 'O', '8', '9'},
        {'1', 'X', 'O', 'O', 'X', 'X', 'X', '8', 'O'},
        {'1', 'X', 'O', 'O', 'X', 'X', '7', '8', 'O'},
        {'1', 'X', 'O', 'O', 'X', 'X', '7', '8', '9'},
        {'1', 'X', 'O', 'O', 'X', 'O', 'X', '8', '9'},
        {'1', 'X', 'O', 'O', 'X', '6', 'X', '8', 'O'},
        {'1', 'X', 'O', 'O', 'X', '6', 'X', '8', '9'},
        {'1', 'X', 'O', 'O', 'X', '6', '7', '8', '9'},
        {'1', 'X', 'O', '4', 'X', 'O', '7', '8', '9'},
        {'1', 'X', 'O', '4', 'X', '6', 'O', '8', '9'},
        {'1', 'X', 'O', '4', 'X', '6', '7', '8', 'O'},
        {'1', 'X', 'O', '4', 'X', '6', '7', '8', '9'},
        {'1', 'X', 'O', 'O', 'O', 'X', 'X', '8', 'X'},
        {'1', 'X', 'O', 'O', 'O', '6', 'X', '8', 'X'},
        {'1', 'X', 'O', 'O', '5', 'O', 'X', '8', 'X'},
        {'1', 'X', 'O', 'O', '5', '6', 'X', '8', 'X'},
        {'1', 'X', 'O', 'O', '5', '6', 'X', '8', '9'},
        {'1', 'X', 'O', '4', 'O', 'O', 'X', '8', 'X'},
        {'1', 'X', 'O', '4', 'O', '6', 'X', '8', 'X'},
        {'1', 'X', '3', 'O', 'X', 'O', '7', '8', '9'},
        {'1', 'X', '3', 'O', 'X', 'X', 'O', '8', 'O'},
        {'1', 'X', '3', 'O', 'X', '6', 'O', '8', '9'},
        {'1', 'X', '3', 'O', 'X', 'X', '7', '8', 'O'},
        {'1', 'X', '3', 'O', 'X', '6', '7', '8', 'O'},
        {'1', 'X', 'X', 'X', 'X', 'O', 'O', '8', 'O'},
        {'1', 'X', '3', 'X', 'X', 'O', 'O', '8', 'O'},
        {'1', 'X', '3', 'X', 'X', 'O', 'O', '8', '9'},
        {'1', 'X', '3', '4', 'X', 'O', 'O', '8', '9'},
        {'1', 'X', '3', '4', 'X', 'O', '7', '8', 'O'},
        {'1', 'X', 'X', 'X', '5', '6', 'O', '8', 'O'},
        {'1', 'X', 'X', '4', 'X', '6', 'O', '8', 'O'},
        {'1', 'X', 'X', '4', '5', 'X', 'O', '8', 'O'},
        {'1', 'X', '3', 'X', 'X', '6', 'O', '8', 'O'},
        {'1', 'X', '3', 'X', '5', 'X', 'O', '8', 'O'},
        {'1', 'X', '3', '4', 'X', 'X', 'O', '8', 'O'},
        {'1', 'X', '3', '4', 'X', '6', 'O', '8', 'O'},
        {'1', 'X', '3', '4', 'X', '6', 'O', '8', '9'},
        {'1', 'X', '3', '4', 'X', '6', '7', '8', 'O'},
        {'O', 'O', 'X', 'X', 'X', 'O', 'O', '8', 'X'},
        {'O', 'O', 'X', 'X', 'O', 'X', 'X', '8', '9'},
        {'O', 'O', 'X', 'X', 'O', 'O', 'X', '8', 'X'},
        {'O', 'O', 'X', 'X', 'O', '6', 'X', '8', 'X'},
        {'O', '2', 'X', 'X', 'O', 'O', 'X', '8', 'X'},
        {'O', '2', 'X', '4', 'O', 'O', 'X', '8', 'X'},
        {'1', 'O', 'X', 'X', 'O', 'X', '7', '8', '9'},
        {'1', 'O', 'X', 'X', 'O', 'O', 'X', '8', 'X'},
        {'1', 'O', 'X', 'X', 'O', '6', 'X', '8', '9'},
        {'1', 'O', 'X', 'X', 'O', 'O', '7', '8', 'X'},
        {'1', 'O', 'X', 'X', 'O', '6', '7', '8', 'X'},
        {'1', 'O', 'X', 'X', 'O', '6', '7', '8', '9'},
        {'1', 'O', 'X', '4', 'O', 'X', 'X', '8', '9'},
        {'1', 'O', 'X', '4', 'O', 'O', 'X', '8', 'X'},
        {'1', 'O', 'X', '4', 'O', '6', 'X', '8', 'X'},
        {'1', 'O', 'X', '4', 'O', '6', 'X', '8', '9'},
        {'1', '2', 'X', 'X', 'X', 'O', 'O', '8', 'O'},
        {'1', '2', 'X', 'X', 'X', '6', 'O', '8', 'O'},
        {'1', '2', 'X', 'X', '5', 'X', 'O', '8', 'O'},
        {'1', '2', 'X', 'X', '5', '6', 'O', '8', 'O'},
        {'1', '2', 'X', '4', 'X', 'X', 'O', '8', 'O'},
        {'1', '2', 'X', '4', 'X', '6', 'O', '8', 'O'},
        {'O', 'O', '3', 'X', 'O', '6', 'X', '8', 'X'},
        {'O', 'O', '3', 'X', '5', 'O', 'X', '8', 'X'},
        {'O', '2', 'O', 'X', 'O', '6', 'X', '8', 'X'},
        {'O', '2', 'O', 'X', '5', 'O', 'X', '8', 'X'},
        {'O', '2', '3', 'X', 'O', 'O', 'X', '8', 'X'},
        {'O', '2', '3', 'X', 'O', '6', 'X', '8', 'X'},
        {'O', '2', '3', 'X', '5', 'O', 'X', '8', 'X'},
        {'1', 'O', '3', 'X', 'O', 'X', 'X', '8', '9'},
        {'1', 'O', '3', 'X', 'O', 'X', '7', '8', 'X'},
        {'1', 'O', '3', 'X', 'O', '6', 'X', '8', 'X'},
        {'1', 'O', '3', 'X', 'O', '6', '7', '8', 'X'},
        {'O', '2', 'O', 'O', 'X', '6', 'X', '8', 'X'},
        {'O', '2', 'O', '4', 'X', 'O', 'X', '8', 'X'},
        {'1', '2', 'O', 'O', 'X', '6', 'X', '8', '9'},
        {'O', '2', 'O', 'O', '5', 'X', 'X', '8', 'X'},
        {'O', '2', 'O', '4', 'O', 'X', 'X', '8', 'X'},
        {'1', 'O', 'O', 'O', '5', 'X', 'X', '8', 'X'},
        {'1', 'O', 'O', '4', 'O', 'X', 'X', '8', 'X'},
        {'1', 'O', '3', '4', 'O', 'X', 'X', '8', 'X'},
        {'1', 'O', '3', '4', 'O', 'X', 'X', '8', '9'},
        {'1', '2', 'O', 'O', 'O', 'X', 'X', '8', 'X'},
        {'1', '2', 'O', 'O', '5', 'X', 'X', '8', 'X'},
        {'1', '2', 'O', '4', 'O', 'X', 'X', '8', 'X'},
        {'O', 'O', '3', '4', '5', '6', 'X', '8', 'X'},
        {'O', '2', 'O', '4', '5', '6', 'X', '8', 'X'},
        {'O', '2', '3', 'O', '5', '6', 'X', '8', 'X'},
        {'O', '2', '3', '4', 'O', '6', 'X', '8', 'X'},
        {'O', '2', '3', '4', '5', 'O', 'X', '8', 'X'},
        {'1', 'O', 'O', '4', '5', '6', 'X', '8', 'X'},
        {'1', 'O', '3', 'O', '5', '6', 'X', '8', 'X'},
        {'1', 'O', '3', '4', 'O', '6', 'X', '8', 'X'},
        {'1', 'O', '3', '4', '5', 'O', 'X', '8', 'X'},
        {'1', '2', 'O', 'O', '5', '6', 'X', '8', 'X'},
        {'1', '2', 'O', '4', 'O', '6', 'X', '8', 'X'},
        {'1', '2', 'O', '4', '5', 'O', 'X', '8', 'X'},
        {'1', '2', '3', 'O', 'O', '6', 'X', '8', 'X'},
        {'1', '2', '3', 'O', '5', 'O', 'X', '8', 'X'},
        {'1', '2', '3', '4', 'O', 'O', 'X', '8', 'X'},
        {'1', '2', '3', '4', 'O', '6', 'X', '8', 'X'}
    },
    {
        {'X', 'O', 'X', 'O', 'X', 'O', 'O', 'X', '9'},
        {'X', 'O', 'X', 'O', 'X', 'X', 'O', 'O', '9'},
        {'X', 'O', 'X', 'O', 'X', 'X', 'O', '8', '9'},
        {'X', 'O', 'X', 'O', 'X', '6', 'O', 'X', '9'},
        {'X', 'O', 'X', 'O', 'X', '6', 'O', '8', '9'},
        {'X', 'O', 'X', 'O', 'O', 'X', 'O', 'X', '9'},
        {'X', 'O', 'X', 'O', 'O', 'X', '7', 'X', '9'},
        {'X', 'O', 'X', 'O', 'O', 'X', '7', '8', '9'},
        {'X', 'O', 'X', 'O', '5', 'X', 'O', 'X', '9'},
        {'X', 'O', 'X', 'O', '5', 'X', 'O', '8', '9'},
        {'X', 'O', 'X', 'O', '5', 'X', '7', 'O', '9'},
        {'X', 'O', 'X', 'O', '5', 'X', '7', '8', '9'},
        {'X', 'O', 'X', 'O', '5', '6', 'O', 'X', '9'},
        {'X', 'O', 'X', 'X', 'O', 'O', 'O', 'X', '9'},
        {'X', 'O', 'X', '4', 'O', 'X', 'O', 'X', '9'},
        {'X', 'O', 'X', '4', 'O', 'X', 'O', '8', '9'},
        {'X', 'O', 'X', 'X', 'X', 'O', 'O', 'O', '9'},
        {'X', 'O', 'X', 'X', 'X', 'O', 'O', '8', '9'},
        {'X', 'O', 'X', '4', 'X', 'O', 'O', 'X', '9'},
        {'X', 'O', 'X', '4', 'X', 'O', 'O', '8', '9'},
        {'X', 'O', 'X', 'X', 'X', '6', 'O', 'O', '9'},
        {'X', 'O', 'X', '4', 'X', 'X', 'O', 'O', '9'},
        {'X', 'O', 'X', '4', 'X', '6', 'O', 'O', '9'},
        {'X', 'O', 'X', '4', 'X', '6', 'O', '8', '9'},
        {'X', 'O', 'X', '4', '5', 'X', 'O', 'O', '9'},
        {'X', 'O', 'X', '4', '5', 'X', 'O', '8', '9'},
        {'X', 'O', 'X', '4', '5', '6', 'O', '8', '9'},
        {'X', 'O', 'O', 'X', 'X', 'O', 'O', 'X', '9'},
        {'X', 'O', 'O', 'X', 'X', 'O', '7', 'X', '9'},
        {'X', 'O', '3', 'X', 'X', 'O', 'O', 'X', '9'},
        {'X', 'O', '3', 'X', 'X', 'O', 'O', '8', '9'},
        {'X', 'O', 'O', 'O', 'X', 'X', 'X', 'O', '9'},
        {'X', 'O', 'O', 'O', 'X', 'X', 'X', '8', '9'},
        {'X', 'O', 'O', 'O', 'X', 'X', 'O', 'X', '9'},
        {'X', 'O', 'O', 'O', 'X', 'X', '7', 'X', '9'},
        {'X', 'O', 'O', 'O', 'X', 'X', '7', '8', '9'},
        {'X', 'O', 'O', 'O', 'X', 'O', 'X', 'X', '9'},
        {'X', 'O', 'O', 'O', 'X', '6', 'X', 'X', '9'},
        {'X', 'O', 'O', 'O', 'X', '6', 'X', '8', '9'},
        {'X', 'O', 'O', '4', 'X', 'O', 'X', 'X', '9'},
        {'X', 'O', 'O', 'O', 'X', '6', '7', 'X', '9'},
        {'X', 'O', 'O', '4', 'X', 'O', '7', 'X', '9'},
        {'X', 'O', 'O', '4', 'X', '6', 'O', 'X', '9'},
        {'X', 'O', 'O', '4', 'X', '6', '7', 'X', '9'},
        {'X', 'O', 'O', '4', 'X', '6', '7', '8', '9'},
        {'X', 'O', '3', 'O', 'X', 'X', 'O', 'X', '9'},
        {'X', 'O', '3', 'O', 'X', 'X', 'O', '8', '9'},
        {'X', 'O', '3', 'O', 'X', 'X', '7', 'O', '9'},
        {'X', 'O', '3', 'O', 'X', 'X', '7', '8', '9'},
        {'X', 'O', '3', 'O', 'X', 'O', '7', 'X', '9'},
        {'X', 'O', '3', 'O', 'X', '6', 'O', 'X', '9'},
        {'X', 'O', '3', 'O', 'X', '6', '7', 'X', '9'},
        {'X', 'O', '3', 'O', 'X', '6', '7', '8', '9'},
        {'X', 'O', '3', '4', 'X', 'O', 'O', 'X', '9'},
        {'X', 'O', '3', '4', 'X', 'O', '7', 'X', '9'},
        {'X', 'O', '3', '4', 'X', 'O', '7', '8', '9'},
        {'X', 'O', '3', '4', 'X', '6', 'O', 'X', '9'},
        {'X', 'O', '3', '4', 'X', '6', 'O', '8', '9'},
        {'X', 'O', '3', '4', 'X', '6', '7', 'O', '9'},
        {'X', 'O', 'O', 'O', 'O', 'X', 'X', 'X', '9'},
        {'X', 'O', 'O', 'O', '5', 'X', 'X', 'X', '9'},
        {'X', 'O', 'O', 'O', '5', 'X', 'X', '8', '9'},
        {'X', 'O', 'O', 'O', '5', 'X', '7', 'X', '9'},
        {'X', 'O', '3', 'O', 'O', 'X', 'X', 'X', '9'},
        {'X', 'O', '3', 'O', 'O', 'X', '7', 'X', '9'},
        {'X', 'O', '3', 'O', '5', 'X', 'O', 'X', '9'},
        {'X', 'O', '3', 'O', '5', 'X', '7', 'X', '9'},
        {'X', 'O', '3', 'O', '5', 'X', '7', '8', '9'},
        {'X', 'O', 'O', 'O', '5', '6', 'X', 'X', '9'},
        {'X', 'O', '3', 'O', 'O', '6', 'X', 'X', '9'},
        {'X', 'O', '3', 'O', '5', 'O', 'X', 'X', '9'},
        {'X', 'O', '3', 'O', '5', '6', 'X', 'X', '9'},
        {'X', 'O', '3', 'O', '5', '6', '7', 'X', '9'},
        {'X', 'X', 'O', 'O', 'X', 'O', 'X', 'O', '9'},
        {'X', 'X', 'O', 'O', 'X', 'O', 'X', '8', '9'},
        {'X', 'X', 'O', 'O', 'X', 'X', 'O', 'O', '9'},
        {'X', 'X', 'O', 'O', 'X', 'X', '7', 'O', '9'},
        {'X', 'X', 'O', 'O', 'X', '6', 'X', 'O', '9'},
        {'X', 'X', 'O', 'O', 'X', '6', '7', 'O', '9'},
        {'X', 'X', 'O', 'O', 'O', 'X', 'X', 'O', '9'},
        {'X', 'X', 'O', 'X', 'X', 'O', 'O', 'O', '9'},
        {'X', 'X', 'O', 'X', 'X', 'O', 'O', '8', '9'},
        {'X', 'X', 'O', 'X', 'X', 'O', '7', 'O', '9'},
        {'X', 'X', 'O', 'X', '5', 'O', '7', '8', '9'},
        {'X', 'X', 'O', '4', 'X', 'O', 'X', 'O', '9'},
        {'X', 'X', 'O', '4', 'X', 'O', '7', 'O', '9'},
        {'X', 'X', 'O', '4', 'X', 'O', '7', '8', '9'},
        {'X', 'X', 'O', '4', '5', 'O', 'X', '8', '9'},
        {'X', 'X', 'O', '4', '5', 'O', '7', 'X', '9'},
        {'X', 'X', 'O', 'X', 'X', '6', 'O', 'O', '9'},
        {'X', 'X', 'O', '4', 'X', 'X', 'O', 'O', '9'},
        {'X', 'X', 'O', '4', 'X', '6', 'O', 'O', '9'},
        {'X', 'X', 'O', '4', 'X', '6', '7', 'O', '9'},
        {'X', '2', 'O', 'X', 'X', 'O', 'O', 'X', '9'},
        {'X', '2', 'O', 'X', 'X', 'O', 'O', '8', '9'},
        {'X', '2', 'O', 'X', 'X', 'O', '7', '8', '9'},
        {'X', '2', 'O', 'X', '5', 'O', '7', 'X', '9'},
        {'X', '2', 'O', 'O', 'X', 'X', 'O', '8', '9'},
        {'X', '2', 'O', 'O', 'X', 'X', 'X', 'O', '9'},
        {'X', '2', 'O', 'O', 'X', 'X', '7', 'O', '9'},
        {'X', '2', 'O', 'O', 'X', 'X', '7', '8', '9'},
        {'X', '2', 'O', 'O', 'X', 'O', 'X', 'X', '9'},
        {'X', '2', 'O', 'O', 'X', 'O', 'X', '8', '9'},
        {'X', '2', 'O', 'O', 'X', '6', 'X', 'O', '9'},
        {'X', '2', 'O', 'O', 'X', '6', 'X', '8', '9'},
        {'X', '2', 'O', 'O', 'X', '6', '7', '8', '9'},
        {'X', '2', 'O', '4', 'X', 'O', 'X', '8', '9'},
        {'X', '2', 'O', '4', 'X', 'O', '7', 'X', '9'},
        {'X', '2', 'O', '4', 'X', 'O', '7', '8', '9'},
        {'X', '2', 'O', '4', 'X', '6', 'O', '8', '9'},
        {'X', '2', 'O', '4', 'X', '6', '7', 'O', '9'},
        {'X', '2', 'O', '4', 'X', '6', '7', '8', '9'},
        {'X', '2', 'O', 'O', 'O', 'X', 'X', 'X', '9'},
        {'X', '2', 'O', 'O', 'O', '6', 'X', 'X', '9'},
        {'X', '2', 'O', 'O', '5', 'O', 'X', 'X', '9'},
        {'X', '2', 'O', 'O', '5', '6', 'X', 'X', '9'},
        {'X', '2', 'O', 'O', '5', '6', 'X', '8', '9'},
        {'X', '2', 'O', '4', '5', 'O', 'X', 'X', '9'},
        {'X', '2', 'O', '4', '5', 'O', '7', 'X', '9'},
        {'X', '2', 'O', '4', '5', '6', '7', 'X', '9'},
        {'X', 'X', '3', 'O', 'X', 'X', 'O', 'O', '9'},
        {'X', '2', 'X', 'O', 'X', 'X', 'O', 'O', '9'},
        {'X', '2', '3', 'O', 'X', 'O', '7', '8', '9'},
        {'X', '2', '3', 'O', 'X', 'X', 'O', 'O', '9'},
        {'X', '2', '3', 'O', 'X', 'X', 'O', '8', '9'},
        {'X', '2', '3', 'O', 'X', '6', 'O', '8', '9'},
        {'X', '2', '3', 'O', 'X', 'X', '7', 'O', '9'},
        {'X', '2', '3', 'O', 'X', '6', '7', 'O', '9'},
        {'X', 'X', '3', 'X', 'X', 'O', 'O', 'O', '9'},
        {'X', '2', 'X', 'X', 'X', 'O', 'O', 'O', '9'},
        {'X', '2', '3', 'X', 'X', 'O', 'O', 'O', '9'},
        {'X', '2', '3', 'X', 'X', 'O', 'O', '8', '9'},
        {'X', '2', '3', '4', 'X', 'O', 'O', '8', '9'},
        {'X', '2', '3', '4', 'X', 'O', '7', 'O', '9'},
        {'X', 'X', '3', 'X', '5', '6', 'O', 'O', '9'},
        {'X', 'X', '3', '4', 'X', '6', 'O', 'O', '9'},
        {'X', 'X', '3', '4', '5', 'X', 'O', 'O', '9'},
        {'X', '2', 'X', 'X', '5', '6', 'O', 'O', '9'},
        {'X', '2', 'X', '4', 'X', '6', 'O', 'O', '9'},
        {'X', '2', 'X', '4', '5', 'X', 'O', 'O', '9'},
        {'X', '2', '3', 'X', 'X', '6', 'O', 'O', '9'},
        {'X', '2', '3', 'X', '5', 'X', 'O', 'O', '9'},
        {'X', '2', '3', '4', 'X', 'X', 'O', 'O', '9'},
        {'X', '2', '3', '4', 'X', '6', 'O', 'O', '9'},
        {'X', '2', '3', '4', 'X', '6', 'O', '8', '9'},
        {'X', '2', '3', '4', '5', 'X', 'O', 'O', '9'},
        {'X', '2', '3', '4', '5', 'X', 'O', '8', '9'},
        {'O', 'X', 'X', 'O', 'O', 'X', 'X', 'O', '9'},
        {'O', 'X', 'X', 'O', 'O', 'X', 'X', '8', '9'},
        {'O', 'X', 'X', 'O', 'O', 'X', '7', '8', '9'},
        {'O', 'X', 'X', 'O', '5', 'X', '7', 'O', '9'},
        {'O', 'X', 'X', 'X', 'O', 'O', 'X', 'O', '9'},
        {'O', 'X', 'X', 'X', 'O', 'O', 'X', '8', '9'},
        {'O', 'X', 'X', 'X', 'O', 'O', 'O', 'X', '9'},
        {'O', 'X', 'X', 'X', 'O', 'O', '7', 'X', '9'},
        {'O', 'X', 'X', 'X', 'O', 'O', '7', '8', '9'},
        {'O', 'X', 'X', 'X', 'O', 'X', 'O', 'O', '9'},
        {'O', 'X', 'X', 'X', 'O', 'X', 'O', '8', '9'},
        {'O', 'X', 'X', 'X', 'O', '6', 'O', 'X', '9'},
        {'O', 'X', 'X', 'X', 'O', '6', 'O', '8', '9'},
        {'O', 'X', 'X', 'X', 'O', 'X', '7', 'O', '9'},
        {'O', 'X', 'X', 'X', 'O', '6', 'X', 'O', '9'},
        {'O', 'X', 'X', 'X', 'O', '6', '7', 'O', '9'},
        {'O', 'X', 'X', 'X', 'O', '6', '7', '8', '9'},
        {'O', 'X', 'X', '4', 'O', 'X', 'O', '8', '9'},
        {'O', 'X', 'X', '4', 'O', 'X', 'X', 'O', '9'},
        {'O', 'X', 'X', '4', 'O', 'X', '7', 'O', '9'},
        {'O', 'X', 'X', '4', 'O', 'X', '7', '8', '9'},
        {'O', 'X', 'X', '4', 'O', '6', 'X', 'O', '9'},
        {'O', 'X', 'X', '4', 'O', '6', 'X', '8', '9'},
        {'O', 'X', 'X', '4', 'O', '6', '7', 'X', '9'},
        {'O', 'X', 'X', '4', 'O', '6', '7', '8', '9'},
        {'O', 'X', 'X', 'X', 'X', 'O', 'O', 'O', '9'},
        {'O', 'X', 'X', 'X', 'X', '6', 'O', 'O', '9'},
        {'O', 'X', 'X', 'X', '5', 'X', 'O', 'O', '9'},
        {'O', 'X', 'X', 'X', '5', '6', 'O', 'O', '9'},
        {'O', 'X', 'X', 'X', '5', '6', 'O', '8', '9'},
        {'O', 'X', 'X', '4', '5', 'X', 'O', 'O', '9'},
        {'O', 'X', 'X', 'X', '5', '6', '7', 'O', '9'},
        {'O', 'X', 'X', '4', '5', 'X', '7', 'O', '9'},
        {'O', 'X', 'O', 'X', 'X', 'O', 'X', 'O', '9'},
        {'O', 'X', 'O', 'X', 'X', 'O', 'X', '8', '9'},
        {'O', 'X', 'O', 'X', 'O', 'X', 'X', 'O', '9'},
        {'O', 'X', 'O', 'X', 'O', 'X', 'X', '8', '9'},
        {'O', 'X', 'O', 'X', 'O', 'O', 'X', 'X', '9'},
        {'O', 'X', 'O', 'X', 'O', '6', 'X', 'X', '9'},
        {'O', 'X', 'O', 'X', 'O', '6', 'X', '8', '9'},
        {'O', 'X', 'O', 'X', '5', 'O', 'X', 'X', '9'},
        {'O', 'X', 'O', 'X', '5', 'O', 'X', '8', '9'},
        {'O', 'X', 'O', 'X', '5', '6', 'X', '8', '9'},
        {'O', 'X', '3', 'X', 'O', 'X', 'X', 'O', '9'},
        {'O', 'X', '3', 'X', 'O', 'X', '7', 'O', '9'},
        {'O', 'X', '3', 'X', 'O', 'X', '7', '8', '9'},
        {'O', 'X', '3', 'X', 'O', 'O', 'X', 'X', '9'},
        {'O', 'X', '3', 'X', 'O', 'O', 'X', '8', '9'},
        {'O', 'X', '3', 'X', 'O', '6', 'X', 'O', '9'},
        {'O', 'X', '3', 'X', 'O', '6', 'X', '8', '9'},
        {'O', 'X', '3', 'X', 'O', 'O', '7', 'X', '9'},
        {'O', 'X', '3', 'X', 'O', '6', '7', 'X', '9'},
        {'O', 'X', '3', 'X', 'O', '6', '7', '8', '9'},
        {'O', 'X', '3', 'X', '5', 'O', 'X', '8', '9'},
        {'O', 'X', 'O', 'O', 'X', 'X', 'X', 'O', '9'},
        {'O', 'X', 'O', 'O', 'O', 'X', 'X', 'X', '9'},
        {'O', 'X', 'O', '4', 'O', 'X', 'X', 'X', '9'},
        {'O', 'X', 'O', '4', 'O', 'X', 'X', '8', '9'},
        {'O', 'X', '3', 'O', 'O', 'X', 'X', 'X', '9'},
        {'O', 'X', '3', 'O', 'O', 'X', 'X', '8', '9'},
        {'O', 'X', '3', '4', 'O', 'X', 'X', 'O', '9'},
        {'O', 'X', '3', '4', 'O', 'X', 'X', '8', '9'},
        {'O', 'X', '3', '4', 'O', 'X', '7', 'X', '9'},
        {'O', 'X', '3', '4', 'O', 'X', '7', '8', '9'},
        {'O', 'X', 'O', '4', 'O', '6', 'X', 'X', '9'},
        {'O', 'X', '3', 'O', 'O', '6', 'X', 'X', '9'},
        {'O', 'X', '3', '4', 'O', 'O', 'X', 'X', '9'},
        {'O', 'X', '3', '4', 'O', '6', 'X', 'X', '9'},
        {'O', 'X', '3', '4', 'O', '6', 'X', '8', '9'},
        {'1', 'X', 'O', 'X', 'O', 'O', 'X', 'X', '9'},
        {'1', 'X', 'O', 'X', 'X', 'O', 'X', 'O', '9'},
        {'1', 'X', 'O', 'X', 'X', 'O', '7', 'O', '9'},
        {'1', 'X', 'O', 'X', 'X', 'O', '7', '8', '9'},
        {'1', 'X', 'O', 'X', '5', 'O', 'X', '8', '9'},
        {'1', 'X', 'O', 'X', '5', 'O', '7', 'X', '9'},
        {'1', 'X', 'O', 'X', '5', 'O', '7', '8', '9'},
        {'1', 'X', 'O', 'X', '5', '6', '7', '8', '9'},
        {'1', 'X', 'O', '4', 'X', 'O', 'X', 'O', '9'},
        {'1', 'X', 'O', '4', 'X', 'O', 'X', '8', '9'},
        {'1', 'X', 'O', 'O', 'O', 'X', 'X', 'X', '9'},
        {'1', 'X', 'O', 'O', 'O', '6', 'X', 'X', '9'},
        {'1', 'X', 'O', '4', 'O', 'O', 'X', 'X', '9'},
        {'1', 'X', 'O', '4', 'O', '6', 'X', 'X', '9'},
        {'1', 'X', 'O', '4', '5', 'O', 'X', 'X', '9'},
        {'1', 'X', 'O', '4', '5', 'O', 'X', '8', '9'},
        {'1', 'X', 'X', 'X', 'O', 'X', 'O', 'O', '9'},
        {'1', 'X', 'X', 'X', 'X', 'O', 'O', 'O', '9'},
        {'1', 'X', '3', 'X', 'X', 'O', 'O', 'O', '9'},
        {'1', 'X', '3', 'X', 'X', 'O', '7', 'O', '9'},
        {'1', 'X', 'X', 'X', '5', '6', 'O', 'O', '9'},
        {'1', 'X', 'X', '4', 'X', '6', 'O', 'O', '9'},
        {'1', 'X', 'X', '4', '5', 'X', 'O', 'O', '9'},
        {'1', 'X', '3', 'X', 'X', '6', 'O', 'O', '9'},
        {'1', 'X', '3', 'X', '5', 'X', 'O', 'O', '9'},
        {'1', 'X', '3', 'X', '5', '6', 'O', 'O', '9'},
        {'1', 'X', '3', 'X', '5', '6', 'O', '8', '9'},
        {'1', 'X', '3', '4', 'X', 'X', 'O', 'O', '9'},
        {'1', 'X', '3', '4', 'X', '6', 'O', 'O', '9'},
        {'1', 'X', '3', '4', '5', 'X', 'O', 'O', '9'},
        {'O', 'O', 'X', 'X', 'X', 'O', 'O', 'X', '9'},
        {'O', 'O', 'X', 'X', 'O', 'X', 'O', 'X', '9'},
        {'O', 'O', 'X', 'X', 'O', 'X', '7', 'X', '9'},
        {'O', 'O', 'X', 'X', 'O', 'X', '7', '8', '9'},
        {'O', 'O', 'X', 'X', 'O', 'O', 'X', 'X', '9'},
        {'O', 'O', 'X', 'X', 'O', '6', 'X', 'X', '9'},
        {'O', 'O', 'X', 'X', 'O', '6', '7', 'X', '9'},
        {'O', '2', 'X', 'X', 'O', 'X', 'O', 'X', '9'},
        {'O', '2', 'X', 'X', 'O', 'X', 'O', '8', '9'},
        {'O', '2', 'X', 'X', 'O', 'X', '7', 'O', '9'},
        {'O', '2', 'X', 'X', 'O', 'X', '7', '8', '9'},
        {'O', '2', 'X', 'X', 'O', 'O', 'X', 'X', '9'},
        {'O', '2', 'X', 'X', 'O', 'O', 'X', '8', '9'},
        {'O', '2', 'X', 'X', 'O', '6', 'X', '8', '9'},
        {'O', '2', 'X', 'X', 'O', 'O', '7', 'X', '9'},
        {'O', '2', 'X', 'X', 'O', '6', 'O', 'X', '9'},
        {'O', '2', 'X', 'X', 'O', '6', '7', 'X', '9'},
        {'O', '2', 'X', 'X', 'O', '6', '7', '8', '9'},
        {'O', 'O', 'X', 'O', 'O', 'X', 'X', 'X', '9'},
        {'O', 'O', 'X', '4', 'O', 'X', 'X', 'X', '9'},
        {'O', 'O', 'X', '4', 'O', 'X', 'X', '8', '9'},
        {'O', 'O', 'X', 'O', '5', 'X', '7', 'X', '9'},
        {'O', 'O', 'X', '4', 'O', 'X', '7', 'X', '9'},
        {'O', 'O', 'X', '4', '5', 'X', 'O', 'X', '9'},
        {'O', 'O', 'X', '4', '5', 'X', '7', '8', '9'},
        {'O', '2', 'X', 'O', 'O', 'X', 'X', 'X', '9'},
        {'O', '2', 'X', 'O', 'O', 'X', 'X', '8', '9'},
        {'O', '2', 'X', 'O', 'O', 'X', '7', 'X', '9'},
        {'O', '2', 'X', 'O', '5', 'X', '7', '8', '9'},
        {'O', '2', 'X', '4', 'O', 'X', 'X', 'O', '9'},
        {'O', '2', 'X', '4', 'O', 'X', 'X', '8', '9'},
        {'O', '2', 'X', '4', 'O', 'X', 'O', 'X', '9'},
        {'O', '2', 'X', '4', 'O', 'X', '7', 'X', '9'},
        {'O', '2', 'X', '4', 'O', 'X', '7', '8', '9'},
        {'O', '2', 'X', '4', '5', 'X', 'O', '8', '9'},
        {'O', '2', 'X', '4', '5', 'X', '7', 'O', '9'},
        {'O', 'O', 'X', '4', 'O', '6', 'X', 'X', '9'},
        {'O', '2', 'X', 'O', 'O', '6', 'X', 'X', '9'},
        {'O', '2', 'X', '4', 'O', 'O', 'X', 'X', '9'},
        {'O', '2', 'X', '4', 'O', '6', 'X', 'X', '9'},
        {'O', '2', 'X', '4', 'O', '6', 'X', '8', '9'},
        {'O', '2', 'X', '4', 'O', '6', '7', 'X', '9'},
        {'1', 'O', 'X', 'X', 'O', 'X', 'O', 'X', '9'},
        {'1', 'O', 'X', 'X', 'O', 'X', 'O', '8', '9'},
        {'1', 'O', 'X', 'O', 'X', 'X', 'O', '8', '9'},
        {'1', 'O', 'X', 'O', 'O', 'X', 'X', 'X', '9'},
        {'1', 'O', 'X', 'O', 'O', 'X', 'X', '8', '9'},
        {'1', 'O', 'X', 'O', 'O', 'X', '7', 'X', '9'},
        {'1', 'O', 'X', 'O', '5', 'X', 'O', 'X', '9'},
        {'1', 'O', 'X', 'O', '5', 'X', '7', 'X', '9'},
        {'1', 'O', 'X', 'O', '5', 'X', '7', '8', '9'},
        {'1', 'O', 'X', '4', 'O', 'X', 'O', 'X', '9'},
        {'1', 'O', 'X', '4', 'O', 'X', '7', 'X', '9'},
        {'1', 'O', 'X', '4', 'O', 'X', '7', '8', '9'},
        {'1', 'O', 'X', '4', '5', 'X', 'O', 'X', '9'},
        {'1', 'O', 'X', '4', '5', 'X', 'O', '8', '9'},
        {'1', 'O', 'X', '4', '5', 'X', '7', 'O', '9'},
        {'1', 'O', 'X', 'O', 'O', '6', 'X', 'X', '9'},
        {'1', 'O', 'X', '4', 'O', 'O', 'X', 'X', '9'},
        {'1', 'O', 'X', '4', 'O', '6', 'X', 'X', '9'},
        {'1', 'O', 'X', '4', 'O', '6', '7', 'X', '9'},
        {'1', '2', 'X', 'O', 'X', 'X', 'O', 'O', '9'},
        {'1', '2', 'X', 'O', 'O', 'X', 'X', 'O', '9'},
        {'1', '2', 'X', 'O', 'O', 'X', 'X', '8', '9'},
        {'1', '2', 'X', 'O', 'O', 'X', 'O', 'X', '9'},
        {'1', '2', 'X', 'O', 'O', 'X', '7', 'X', '9'},
        {'1', '2', 'X', 'O', 'O', 'X', '7', '8', '9'},
        {'1', '2', 'X', 'O', '5', 'X', 'O', '8', '9'},
        {'1', '2', 'X', 'O', '5', 'X', '7', 'O', '9'},
        {'1', '2', 'X', 'O', '5', 'X', '7', '8', '9'},
        {'1', '2', 'X', 'X', 'O', 'X', 'O', 'O', '9'},
        {'1', '2', 'X', 'X', 'O', 'X', 'O', '8', '9'},
        {'1', '2', 'X', '4', 'O', 'X', 'O', 'X', '9'},
        {'1', '2', 'X', '4', 'O', 'X', 'O', '8', '9'},
        {'1', '2', 'X', '4', 'O', 'X', '7', 'O', '9'},
        {'1', '2', 'X', '4', 'O', 'X', '7', '8', '9'},
        {'1', '2', 'X', 'X', 'X', 'O', 'O', 'O', '9'},
        {'1', '2', 'X', 'X', 'X', '6', 'O', 'O', '9'},
        {'1', '2', 'X', 'X', '5', 'X', 'O', 'O', '9'},
        {'1', '2', 'X', 'X', '5', '6', 'O', 'O', '9'},
        {'1', '2', 'X', '4', 'X', 'X', 'O', 'O', '9'},
        {'1', '2', 'X', '4', 'X', '6', 'O', 'O', '9'},
        {'1', '2', 'X', '4', '5', 'X', 'O', 'O', '9'},
        {'1', '2', 'X', '4', '5', 'X', 'O', '8', '9'},
        {'1', '2', 'X', '4', '5', 'X', '7', 'O', '9'},
        {'O', '2', '3', 'X', 'O', 'X', 'X', '8', '9'},
        {'O', '2', '3', 'X', 'O', 'X', '7', 'X', '9'},
        {'O', 'O', '3', 'X', 'O', '6', 'X', 'X', '9'},
        {'O', 'O', '3', 'X', '5', 'O', 'X', 'X', '9'},
        {'O', '2', 'O', 'X', 'O', '6', 'X', 'X', '9'},
        {'O', '2', 'O', 'X', '5', 'O', 'X', 'X', '9'},
        {'O', '2', '3', 'X', 'O', 'O', 'X', 'X', '9'},
        {'O', '2', '3', 'X', 'O', '6', 'X', 'X', '9'},
        {'O', '2', '3', 'X', 'O', '6', 'X', '8', '9'},
        {'O', '2', '3', 'X', '5', 'O', 'X', 'X', '9'},
        {'O', '2', '3', 'X', 'O', '6', '7', 'X', '9'},
        {'1', '2', 'O', 'X', 'X', 'O', 'X', '8', '9'},
        {'1', '2', 'O', 'X', 'X', 'O', '7', 'X', '9'},
        {'1', '2', 'O', 'X', 'X', 'O', '7', '8', '9'},
        {'1', '2', 'O', 'X', '5', 'O', 'X', 'X', '9'},
        {'1', '2', 'O', 'X', '5', 'O', '7', 'X', '9'},
        {'1', 'O', 'O', 'O', 'X', '6', 'X', 'X', '9'},
        {'1', 'O', 'O', '4', 'X', 'O', 'X', 'X', '9'},
        {'1', '2', 'O', '4', 'X', 'O', 'X', 'X', '9'},
        {'1', '2', 'O', '4', 'X', 'O', 'X', '8', '9'},
        {'O', 'O', '3', 'O', '5', 'X', 'X', 'X', '9'},
        {'O', 'O', '3', '4', 'O', 'X', 'X', 'X', '9'},
        {'O', '2', 'O', 'O', '5', 'X', 'X', 'X', '9'},
        {'O', '2', 'O', '4', 'O', 'X', 'X', 'X', '9'},
        {'O', '2', '3', 'O', 'O', 'X', 'X', 'X', '9'},
        {'O', '2', '3', '4', 'O', 'X', 'X', 'X', '9'},
        {'O', '2', '3', '4', 'O', 'X', 'X', '8', '9'},
        {'O', '2', '3', '4', 'O', 'X', '7', 'X', '9'},
        {'1', 'O', 'O', 'O', '5', 'X', 'X', 'X', '9'},
        {'1', 'O', 'O', '4', 'O', 'X', 'X', 'X', '9'},
        {'1', 'O', '3', 'O', 'O', 'X', 'X', 'X', '9'},
        {'1', 'O', '3', 'O', '5', 'X', 'X', 'X', '9'},
        {'1', 'O', '3', '4', 'O', 'X', 'X', 'X', '9'},
        {'1', 'O', '3', 'O', '5', 'X', '7', 'X', '9'},
        {'1', 'O', '3', '4', 'O', 'X', '7', 'X', '9'},
        {'1', '2', 'O', 'O', 'O', 'X', 'X', 'X', '9'},
        {'1', '2', 'O', 'O', '5', 'X', 'X', 'X', '9'},
        {'1', '2', 'O', '4', 'O', 'X', 'X', 'X', '9'},
        {'1', '2', '3', 'O', 'O', 'X', 'X', 'X', '9'},
        {'1', '2', '3', 'O', 'O', 'X', 'X', '8', '9'},
        {'1', '2', '3', 'O', 'O', 'X', '7', 'X', '9'},
        {'1', '2', '3', '4', '5', 'X', 'O', '8', '9'},
        {'O', 'O', '3', '4', '5', '6', 'X', 'X', '9'},
        {'O', '2', 'O', '4', '5', '6', 'X', 'X', '9'},
        {'O', '2', '3', 'O', '5', '6', 'X', 'X', '9'},
        {'O', '2', '3', '4', 'O', '6', 'X', 'X', '9'},
        {'O', '2', '3', '4', '5', 'O', 'X', 'X', '9'},
        {'1', 'O', 'O', '4', '5', '6', 'X', 'X', '9'},
        {'1', 'O', '3', 'O', '5', '6', 'X', 'X', '9'},
        {'1', 'O', '3', '4', 'O', '6', 'X', 'X', '9'},
        {'1', 'O', '3', '4', '5', 'O', 'X', 'X', '9'},
        {'1', 'O', '3', '4', '5', '6', 'X', 'X', '9'},
        {'1', '2', 'O', 'O', '5', '6', 'X', 'X', '9'},
        {'1', '2', 'O', '4', 'O', '6', 'X', 'X', '9'},
        {'1', '2', 'O', '4', '5', 'O', 'X', 'X', '9'},
        {'1', '2', 'O', '4', '5', '6', 'X', 'X', '9'},
        {'1', '2', '3', 'O', 'O', '6', 'X', 'X', '9'},
        {'1', '2', '3', 'O', '5', 'O', 'X', 'X', '9'},
        {'1', '2', '3', '4', 'O', 'O', 'X', 'X', '9'},
        {'1', '2', '3', '4', 'O', '6', 'X', 'X', '9'},
        {'1', '2', '3', '4', '5', 'O', 'X', 'X', '9'},
        {'1', '2', 'O', '4', '5', '6', '7', 'X', '9'}
    }
}});

bool xInBoard(std::array<char, 9> board)
{
    for (char val : board)
    {
        if (val == 'X')
        {
            return true;
        }
    }
    return false;
}

std::uint8_t ai::strategyDumb(ai::Board board)
{
    return util::firstOpenCell(board);
}

std::uint8_t ai::strategyRandom(ai::Board board)
{
    std::vector<std::uint8_t> openCells = util::openCells(board);

    std::random_device rd;
    std::default_random_engine engine(rd());
    std::uniform_int_distribution<> dist(0, openCells.size() - 1);

    std::uint8_t choice = dist(engine);
    return openCells[choice];
}

std::uint8_t ai::strategyOracle(ai::Board board)
{
    std::array<char, 9> board1D = util::get1dFrom2dBoard(board);

    if (!xInBoard(board1D))
    {
        return ai::strategyRandom(board);
    }
    for (std::uint8_t i = 0; i < (*MODEL).size(); i++)
    {
        for (char place : board1D)
        {
            if (util::isDigit(place))
            {
                for (std::uint16_t j = 0; j < (*MODEL)[i].size(); j++)
                {
                    if (board1D == (*MODEL)[i][j])
                    {
                        return i + 1;
                    }
                }
            }
        }
    }
    std::print("If you see this, the model isn't finding the current state.\n");
    return 0;
}
