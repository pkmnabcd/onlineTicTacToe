...
namespace critical
{
    ...
    void invalidateLobby(std::array<Lobby, arraySize>& lobbies, Lobby lobby, std::mutex& mut);
    void invalidateLobbyIfOtherPlayerDisconnected(std::array<Lobby, arraySize>& lobbies, Lobby lobby, std::mutex& dataMut, std::mutex& disconnectMut);
