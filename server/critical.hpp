...
namespace critical
{
    ...
    void invalidateLobby(std::array<Lobby, arraySize>& lobbies, std::array<GameState, arraySize>& gamestates, std::uint8_t hostID, std::mutex& mut);
    void invalidateLobbyIfOtherPlayerDisconnected(std::array<Lobby, arraySize>& lobbies, std::array<GameState, arraySize>& gamestates, std::uint8_t hostID, std::mutex& dataMut, std::mutex& disconnectMut);
